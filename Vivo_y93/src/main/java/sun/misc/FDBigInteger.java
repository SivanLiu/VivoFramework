...
public class FDBigInteger {
    ...
    static final /* synthetic */ boolean f251-assertionsDisabled = (FDBigInteger.class.desiredAssertionStatus() ^ 1);
    ...
    private int[] data;
    private boolean isImmutable = f251-assertionsDisabled;
    private int nWords;
    private int offset;
    ...
    public static FDBigInteger valueOfMulPow52(long value, int p5, int p2) {
        if (!f251-assertionsDisabled && p5 < 0) {
            throw new AssertionError(Integer.valueOf(p5));
        } else if (f251-assertionsDisabled || p2 >= 0) {
            int v0 = (int) value;
            int v1 = (int) (value >>> 32);
            int wordcount = p2 >> 5;
            int bitcount = p2 & 31;
    ...
    public int quoRemIteration(FDBigInteger S) throws IllegalArgumentException {
        if (f251-assertionsDisabled || !this.isImmutable) {
            int thSize = size();
            int sSize = S.size();
            int p;
            if (thSize < sSize) {
                ...
                if (multDiffMe(q, S) != 0) {
                    ...
                    while (sum == 0) {
                        ...
                        if (f251-assertionsDisabled || sum == 0 || sum == 1) {
                            q--;
                        } else {
                ...
                p = multAndCarryBy10(this.data, this.nWords, this.data);
                if (f251-assertionsDisabled || p == 0) {
                    trimLeadingZeros();
                    return (int) q;
                }
    ...
    public FDBigInteger leftInplaceSub(FDBigInteger subtrahend) {
        if (f251-assertionsDisabled || size() >= subtrahend.size()) {
            ...
            if (f251-assertionsDisabled || borrow == 0) {
