...
public class StreamEncoder extends Writer {
    ...
    static final /* synthetic */ boolean f8-assertionsDisabled = (StreamEncoder.class.desiredAssertionStatus() ^ 1);
    ...
    public void close() throws IOException {
        synchronized (this.lock) {
            if (this.isOpen) {
                implClose();
                this.isOpen = f8-assertionsDisabled;
                return;
            }
        }
    }
    ...
    private StreamEncoder(OutputStream out, Object lock, CharsetEncoder enc) {
        super(lock);
        this.isOpen = true;
        this.haveLeftoverChar = f8-assertionsDisabled;
        this.lcb = null;
        this.out = out;
        this.ch = null;
        this.cs = enc.charset();
    ...
    private void writeBytes() throws IOException {
        ...
        if (f8-assertionsDisabled || pos <= lim) {
            int rem = pos <= lim ? lim - pos : 0;
            if (rem > 0) {
                if (this.ch == null) {
                    this.out.write(this.bb.array(), this.bb.arrayOffset() + pos, rem);
                } else if (!(this.ch.write(this.bb) == rem || f8-assertionsDisabled)) {
        ...
        if (this.haveLeftoverChar || (endOfInput ^ 1) == 0) {
            ...
            while (true) {
                ...
                if (cr.isUnderflow()) {
                    ...
                    cr.throwException();
                } else if (f8-assertionsDisabled || this.bb.position() > 0) {
                    writeBytes();
                } else {
                    throw new AssertionError();
                }
    ...
    void implWrite(char[] cbuf, int off, int len) throws IOException {
        ...
        if (this.haveLeftoverChar) {
            flushLeftoverChar(cb, f8-assertionsDisabled);
        }
        while (cb.hasRemaining()) {
            CoderResult cr = this.encoder.encode(cb, this.bb, f8-assertionsDisabled);
            if (cr.isUnderflow()) {
                if (!f8-assertionsDisabled && cb.remaining() > 1) {
