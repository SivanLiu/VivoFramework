...
abstract class AbstractWatchKey implements WatchKey {
    ...
    static final /* synthetic */ boolean f272-assertionsDisabled = (AbstractWatchKey.class.desiredAssertionStatus() ^ 1);
    ...
    final void signalEvent(Kind<?> kind, Object context) {
        boolean isModify = kind == StandardWatchEventKinds.ENTRY_MODIFY ? true : f272-assertionsDisabled;
        synchronized (this) {
            ...
            if (size > 0) {
                ...
                if (!this.lastModifyEvents.isEmpty()) {
                    if (isModify) {
                        ...
                        if (ev != null) {
                            if (f272-assertionsDisabled || ev.kind() == StandardWatchEventKinds.ENTRY_MODIFY) {
                                ((Event) ev).increment();
                                return;
                            }
                ...
                if (size >= 512) {
                    ...
                    isModify = f272-assertionsDisabled;
                    context = null;
                }
            }
