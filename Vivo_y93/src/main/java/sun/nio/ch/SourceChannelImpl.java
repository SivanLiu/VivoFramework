...
class SourceChannelImpl extends SourceChannel implements SelChImpl {
    ...
    static final /* synthetic */ boolean f269-assertionsDisabled = (SourceChannelImpl.class.desiredAssertionStatus() ^ 1);
    ...
    public void kill() throws IOException {
        synchronized (this.stateLock) {
            if (this.state == 1) {
                ...
                this.state = 1;
            } else if (f269-assertionsDisabled || !(isOpen() || isRegistered())) {
                nd.close(this.fd);
                this.state = 1;
            } else {
        ...
        synchronized (this.lock) {
            try {
                ...
                if (isOpen()) {
                    ...
                    if (f269-assertionsDisabled || IOStatus.check(n)) {
                    } else {
                        throw new AssertionError();
                    }
                }
                this.thread = 0;
                end(f269-assertionsDisabled);
                if (f269-assertionsDisabled || IOStatus.check(0)) {
                } else {
                    throw new AssertionError();
                }
            } finally {
