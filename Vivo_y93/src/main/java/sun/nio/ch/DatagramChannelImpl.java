...
class DatagramChannelImpl extends DatagramChannel implements SelChImpl {
    ...
    static final /* synthetic */ boolean f261-assertionsDisabled = (DatagramChannelImpl.class.desiredAssertionStatus() ^ 1);
    ...
    public DatagramChannelImpl(SelectorProvider sp) throws IOException {
        ...
        try {
            ...
            this.fd = Net.socket(this.family, f261-assertionsDisabled);
    ...
    public DatagramChannelImpl(SelectorProvider sp, ProtocolFamily family) throws IOException {
        ...
        if (family == StandardProtocolFamily.INET || family == StandardProtocolFamily.INET6) {
            if (family != StandardProtocolFamily.INET6 || Net.isIPv6Available()) {
                this.family = family;
                this.fd = Net.socket(family, f261-assertionsDisabled);
                this.fdVal = IOUtil.fdVal(this.fd);
                this.state = 0;
                if (this.fd != null && this.fd.valid()) {
                    this.guard.open("close");
        ...
        if (dst.isReadOnly()) {
            ...
            synchronized (this.readLock) {
                ...
                try {
                    ...
                    if (isOpen()) {
                        ...
                        if (isConnected() || security == null) {
                            ...
                            if (n == -2) {
                                this.readerThread = 0;
                                if (n <= 0 && n != -2) {
                                    z = f261-assertionsDisabled;
                                }
                                end(z);
                                if (f261-assertionsDisabled || IOStatus.check(n)) {
                                } else {
                    ...
                    this.readerThread = 0;
                    end(f261-assertionsDisabled);
                    if (f261-assertionsDisabled || IOStatus.check(0)) {
                    } else {
                        throw new AssertionError();
                    }
    ...
    private int sendFromNativeBuffer(FileDescriptor fd, ByteBuffer bb, InetSocketAddress target) throws IOException {
        ...
        if (f261-assertionsDisabled || pos <= lim) {
            ...
            int rem = pos <= lim ? lim - pos : 0;
            try {
                written = send0(this.family != StandardProtocolFamily.INET ? true : f261-assertionsDisabled, fd, ((DirectBuffer) bb).address() + ((long) pos), rem, target.getAddress(), target.getPort());
    ...
    public long read(ByteBuffer[] dsts, int offset, int length) throws IOException {
        ...
        synchronized (this.readLock) {
            ...
            try {
                ...
                this.readerThread = 0;
                boolean z = (n > 0 || n == -2) ? true : f261-assertionsDisabled;
                end(z);
                if (f261-assertionsDisabled || IOStatus.check(n)) {
                } else {
                    throw new AssertionError();
                ...
                this.readerThread = 0;
                boolean z2 = (0 > 0 || 0 == -2) ? true : f261-assertionsDisabled;
                end(z2);
                if (!f261-assertionsDisabled && !IOStatus.check(0)) {
                    AssertionError assertionError = new AssertionError();
                }
                ...
                synchronized (this.stateLock) {
                    ...
                    synchronized (blockingLock()) {
                        try {
                            ...
                            if (blocking) {
                                configureBlocking(f261-assertionsDisabled);
                            }
                            do {
                                tmpBuf.clear();
            ...
            synchronized (this.writeLock) {
                synchronized (this.stateLock) {
                    if (isConnected() && (isOpen() ^ 1) == 0) {
                        ...
                        disconnect0(this.fd, this.family == StandardProtocolFamily.INET6 ? true : f261-assertionsDisabled);
    ...
    void drop(MembershipKeyImpl key) {
        if (f261-assertionsDisabled || key.channel() == this) {
            synchronized (this.stateLock) {
                if (key.isValid()) {
                    try {
    ...
    void block(MembershipKeyImpl key, InetAddress source) throws IOException {
        if (!f261-assertionsDisabled && key.channel() != this) {
            throw new AssertionError();
        } else if (f261-assertionsDisabled || key.sourceAddress() == null) {
            synchronized (this.stateLock) {
                if (!key.isValid()) {
    ...
    public void kill() throws IOException {
        synchronized (this.stateLock) {
            if (this.state == 2) {
                ...
                this.state = 2;
            } else if (f261-assertionsDisabled || !(isOpen() || isRegistered())) {
                nd.close(this.fd);
                this.state = 2;
            } else {
                throw new AssertionError();
    ...
    public boolean translateReadyOps(int ops, int initialOps, SelectionKeyImpl sk) {
        ...
        if ((Net.POLLNVAL & ops) != 0) {
            return f261-assertionsDisabled;
        }
    ...
    int poll(int events, long timeout) throws IOException {
        boolean z = f261-assertionsDisabled;
        if (f261-assertionsDisabled || (Thread.holdsLock(blockingLock()) && !isBlocking())) {
            synchronized (this.readLock) {
                long isOpen;
                try {
                    begin();
