...
class SinkChannelImpl extends SinkChannel implements SelChImpl {
    ...
    static final /* synthetic */ boolean f263-assertionsDisabled = (SinkChannelImpl.class.desiredAssertionStatus() ^ 1);
    ...
    public void kill() throws IOException {
        synchronized (this.stateLock) {
            if (this.state == 1) {
                ...
                this.state = 1;
            } else if (f263-assertionsDisabled || !(isOpen() || isRegistered())) {
                nd.close(this.fd);
                this.state = 1;
            } else {
        ...
        synchronized (this.lock) {
            try {
                ...
                if (isOpen()) {
                    ...
                    if (f263-assertionsDisabled || IOStatus.check(n)) {
                    } else {
                        throw new AssertionError();
                    }
                }
                this.thread = 0;
                end(f263-assertionsDisabled);
                if (f263-assertionsDisabled || IOStatus.check(0)) {
                } else {
                    throw new AssertionError();
                }
            } finally {
