...
public class QcRilHook implements IQcRilHook {
    ...
    private static RegistrantList mRegistrants = new RegistrantList();
    ...
    public byte[] qcRilGetOemVersionOfIDVivo(String config_id, int mbnType) {
        if (config_id.isEmpty() || config_id.length() > MAX_PDC_ID_LEN) {
            Log.w(LOG_TAG, "invalid config_id");
            return null;
        }
        if (!(mbnType == 0 || 1 == mbnType)) {
            Log.d(LOG_TAG, "unsupport mbnType: " + mbnType);
        }
        byte[] payload = new byte[((this.mHeaderSize + 4) + config_id.length())];
        ByteBuffer buf = createBufferWithNativeByteOrder(payload);
        addQcRilHookHeader(buf, IQcRilHook.QCRIL_EVT_HOOK_GET_OEM_VERSION_OF_ID_VIVO, config_id.length() + 4);
        try {
            buf.putInt(mbnType);
            buf.put(config_id.getBytes("ISO-8859-1"));
            Log.d(LOG_TAG, "qcRilGetOemVersionOfIDVivo payload: " + IccUtils.bytesToHexString(payload));
            AsyncResult ar = sendRilOemHookMsg(IQcRilHook.QCRIL_EVT_HOOK_GET_OEM_VERSION_OF_ID_VIVO, payload);
            if (ar.exception != null) {
                Log.w(LOG_TAG, "QCRIL_EVT_HOOK_GET_OEM_VERSION_OF_ID_VIVO failed w/ " + ar.exception);
                return null;
            } else if (ar.result == null) {
                Log.w(LOG_TAG, "QCRIL_EVT_HOOK_GET_OEM_VERSION_OF_ID_VIVO failed w/ null result");
                return null;
            } else {
                Log.v(LOG_TAG, "QCRIL_EVT_HOOK_GET_OEM_VERSION_OF_ID_VIVO returned w/ " + ((byte[]) ar.result));
                return (byte[]) ar.result;
            }
        } catch (UnsupportedEncodingException e) {
    ...
    @VivoHook(hookType = VivoHookType.NEW_METHOD)
    public boolean qcRilGetAllConfigsVivo(int mbnType) {
        if (mbnType == 0 || 1 == mbnType) {
            byte[] payload = new byte[(this.mHeaderSize + 4)];
            ByteBuffer buf = createBufferWithNativeByteOrder(payload);
            addQcRilHookHeader(buf, IQcRilHook.QCRIL_EVT_HOOK_GET_AVAILABLE_CONFIGS_VIVO, 4);
            buf.putInt(mbnType);
            Log.d(LOG_TAG, "qcRilGetAllConfigsVivo payload: " + IccUtils.bytesToHexString(payload));
            AsyncResult ar = sendRilOemHookMsg(IQcRilHook.QCRIL_EVT_HOOK_GET_AVAILABLE_CONFIGS_VIVO, payload);
            if (ar.exception == null) {
                return DBG;
            }
            Log.w(LOG_TAG, "QCRIL_EVT_HOOK_GET_AVAILABLE_CONFIGS failed w/ " + ar.exception);
    ...
    public boolean qcRilSetSarPower(byte powerState) {
        Log.v(LOG_TAG, "qcRilSetSarPower: powerState value will be set to " + powerState);
        byte[] requestData = new byte[8];
        ByteBuffer reqBuffer = createBufferWithNativeByteOrder(requestData);
        reqBuffer.putInt(powerState);
        reqBuffer.putInt(0);
        AsyncResult ar = sendQcRilHookMsg((int) IQcRilHook.QCRIL_EVT_HOOK_SET_TRANSMIT_POWER, requestData);
        if (ar.exception == null) {
            return DBG;
        }
        Log.e(LOG_TAG, "qcRilSetSarPower: Exception " + ar.exception);
        return false;
    }
