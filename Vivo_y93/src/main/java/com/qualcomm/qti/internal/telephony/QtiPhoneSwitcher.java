...
public class QtiPhoneSwitcher extends PhoneSwitcher {
    ...
    private boolean mTryInformDdsNextTime = false;
    ...
    protected void onEvaluate(boolean requestsChanged, String reason) {
        ...
        if ((this.mSubscriptionController.isActiveSubId(dataSubId) && dataSubId != this.mDefaultDataSubscription) || !(ddsPhoneId == -1 || ddsPhoneId == this.mDefaultDataPhoneId)) {
            sb.append(" default ").append(this.mDefaultDataSubscription).append("->").append(dataSubId);
            ...
            this.mPhoneStates[this.mDefaultDataPhoneId].active = false;
            diffDetected = true;
        }
        ...
        boolean isDdsPhoneValid = SubscriptionManager.isValidPhoneId(ddsPhoneId);
        if (diffDetected || (this.mTryInformDdsNextTime && isDdsPhoneValid)) {
            if (isDdsPhoneValid) {
                log("set mTryInformDdsNextTime = false");
                this.mTryInformDdsNextTime = false;
                ...
                log("can not informDdsToRil when dds invalid. set mTryInformDdsNextTime = true");
                this.mTryInformDdsNextTime = true;
            }
        }
    }
    ...
    protected void activate(int phoneId) {
        if (phoneId == -1) {
            log("Don't active phone for invalid phone id");
            return;
        }
        PhoneState state = this.mPhoneStates[phoneId];
    ...
    private boolean isPhoneIdValidForRetry(int phoneId) {
        ...
        for (int i = 0; i < this.mMaxActivePhones; i++) {
            ...
            if (dcRequest != null && phoneIdForRequest(dcRequest.networkRequest, dcRequest.apnId) == phoneId) {
                isValid = true;
            }
        }
        return isValid;
