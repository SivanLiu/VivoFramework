...
public class QtiSubscriptionController extends SubscriptionController {
    ...
    private void handleDataPreference(int nextActiveSubId) {
        ...
        if (subInfoList == null || subInfoList.size() == 0) {
            logd("handleDataPreference subInfoList is empty!");
            return;
        }
        String tag;
        for (SubscriptionInfo subInfo : subInfoList) {
            logd("handleDataPreference subInfo = " + subInfo);
        }
        SharedPreferences pref = this.mContext.getSharedPreferences("com.android.phone_preferences", 0);
        if (pref.getInt("DEFAULT_DATA_SLOT_ID", 0) == 0) {
            tag = "sim0iccid";
        } else {
            tag = "sim1iccid";
        }
        String prefIccid = pref.getString(tag, "");
        int sim0UserPreference = getUiccCardProvisioningUserPreference(0);
        int sim1UserPreference = getUiccCardProvisioningUserPreference(1);
        logd("handleDataPreference sim0UserPreference = " + sim0UserPreference + " sim1UserPreference = " + sim1UserPreference);
        boolean isSetDds = true;
        if (!(sim0UserPreference == -2 || sim1UserPreference == -2)) {
            boolean[] mSimOk = new boolean[]{false, false};
            for (SubscriptionInfo subInfo2 : subInfoList) {
                if (subInfo2.getSimSlotIndex() == 0 || subInfo2.getSimSlotIndex() == 1) {
                    mSimOk[subInfo2.getSimSlotIndex()] = true;
                }
            }
            if (!(mSimOk[0] && (mSimOk[1] ^ 1) == 0)) {
                isSetDds = false;
            }
        }
        if (isSetDds) {
            int ddsDataId = -1;
            for (SubscriptionInfo subInfo22 : subInfoList) {
                if (prefIccid.equals(subInfo22.getIccId())) {
                    if (getUiccCardProvisioningUserPreference(subInfo22.getSimSlotIndex()) == 1) {
                        ddsDataId = subInfo22.getSubscriptionId();
                        break;
                    }
                }
                if (subInfo22.getSimSlotIndex() == 0 && getUiccCardProvisioningUserPreference(0) == 1) {
                    ddsDataId = subInfo22.getSubscriptionId();
                } else if (subInfo22.getSimSlotIndex() == 1 && ddsDataId == -1 && getUiccCardProvisioningUserPreference(1) == 1) {
                    ddsDataId = subInfo22.getSubscriptionId();
                }
            }
            logd("handleDataPreference ddsDataId = " + ddsDataId);
            if (ddsDataId != -1) {
                setDefaultDataSubId(ddsDataId);
                Intent intent = new Intent("android.intent.action.SET_DDS_NW_FLAG");
                intent.addFlags(536870912);
                intent.putExtra("reason", "setDDS");
                this.mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
            }
            return;
        }
        logd("handleDataPreference sim not ready");
    }
