...
public class QtiTelephonyComponentFactory extends TelephonyComponentFactory {
    ...
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    private static HandlerThread mRilOemHandlerThread;
    ...
    public QtiTelephonyComponentFactory() {
        if (mRilOemHandlerThread == null) {
            mRilOemHandlerThread = new HandlerThread("QtiRilOemHandlerThread");
            mRilOemHandlerThread.start();
        }
        if (mHandlerThread == null) {
            mHandlerThread = new HandlerThread("QtiCommonHandlerThread");
            mHandlerThread.start();
        }
    }

    @VivoHook(hookType = VivoHookType.NEW_METHOD)
    public static Looper getRilOemLooper() {
        if (mRilOemHandlerThread != null) {
            return mRilOemHandlerThread.getLooper();
        }
        mRilOemHandlerThread = new HandlerThread("QtiRilOemHandlerThread");
        mRilOemHandlerThread.start();
        return mRilOemHandlerThread.getLooper();
    ...
    public void makeExtTelephonyClasses(Context context, Phone[] phones, CommandsInterface[] commandsInterfaces) {
        ...
        QtiUiccCardProvisioner.make(context, getCommonLooper());
