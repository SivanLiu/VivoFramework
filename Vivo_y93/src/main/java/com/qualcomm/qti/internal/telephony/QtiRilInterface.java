...
public class QtiRilInterface implements BaseRilInterface {
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            Integer instanceId = Integer.valueOf(intent.getIntExtra("instance_id", -1));
            ...
            if ("qualcomm.intent.action.ACTION_ADN_INIT_DONE".equals(intent.getAction())) {
                ...
                QtiRilInterface.this.mAdnRecordsInfoRegistrantList.notifyRegistrants(new AsyncResult(null, QtiRilInterface.this.responseAdnRecords(intent.getByteArrayExtra("adn_records"), instanceId.intValue()), null));
            }
        }
    };
    ...
    public AsyncResult sendQcRilHookMsg(int msgId, byte[] requestData, int phoneId) {
        return this.mQcRilHook.sendQcRilHookMsg(msgId, requestData, phoneId);
    }
    ...
    public String getUiccIccId(int phoneId) {
        ...
        String normalMode = SystemProperties.get("persist.sys.factory.mode", "null");
        if (normalMode != "" && (normalMode.equals("null") ^ 1) != 0 && normalMode.equals("yes") && phoneId == 0 && virtualSIMFlagIsTrue()) {
            logd("getUiccIccId sim1 is virtualSim!!! prop");
            str = "89860000000000000001";
        }
    ...
    private Object responseAdnRecords(byte[] data, int instanceId) {
        ...
        for (int i = 0; i < numRecords; i++) {
            ...
            AdnRecordsInfoGroup[i].mInstanceId = instanceId;
        }
