...
public class QtiPrimaryCardController extends Handler {
    ...
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (QtiPrimaryCardController.ACTION_RADIO_CAPABILITY_UPDATED.equals(action)) {
                QtiPrimaryCardController.this.triggerPrimaryCardIfRequired();
            }
        }
    };
    ...
    private void triggerPrimaryCardIfRequired() {
        if (this.mQtiRilInterface.isServiceReady() && this.mIsLpluslSupport != this.mQtiRilInterface.getLpluslSupportStatus()) {
            this.mPriorityHandler.reloadPriorityConfig();
            setPrimaryCardIfRequired(DBG);
            this.mIsLpluslSupport = this.mQtiRilInterface.getLpluslSupportStatus();
        }
    }
        ...
        switch (msg.what) {
            ...
            case 8:
                ...
                if (this.mQtiRilInterface.isServiceReady()) {
                    this.mIsLpluslSupport = this.mQtiRilInterface.getLpluslSupportStatus();
                    if (!this.mIsLpluslSupport) {
                        this.mContext.registerReceiver(this.mReceiver, new IntentFilter(ACTION_RADIO_CAPABILITY_UPDATED));
                    }
                }
                logd("on EVENT_OEM_HOOK_SERVICE_READY and mIsLpluslSupport = " + this.mIsLpluslSupport);
                return;
            default:
                return;
        }
