...
public class QtiGsmCdmaPhone extends GsmCdmaPhone {
    private static final int EVENT_GET_PREFERED_NETWORK_TYPE = 47;
    private static final int EVENT_OEM_HOOK_SERVICE_READY = 46;
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    protected static final int EVENT_UI_NOT_READY = 100;
    ...
    public QtiGsmCdmaPhone(Context context, CommandsInterface ci, PhoneNotifier notifier, boolean unitTestMode, int phoneId, int precisePhoneType, TelephonyComponentFactory telephonyComponentFactory) {
        ...
        this.mCi.setUiNotReadyEvent(this, 100, null);
    }
    ...
    public void dispose() {
        ...
        this.mCi.unSetUiNotReadyEvent(this);
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_GET_PREFERED_NETWORK_TYPE /*47*/:
                ar = (AsyncResult) msg.obj;
                if (SubscriptionManager.from(this.mContext).isActiveSubId(getSubId()) && ar != null && ar.result != null) {
                    int type = PhoneFactory.calculatePreferredNetworkType(this.mContext, getSubId());
                    int modemNetworkMode = ((int[]) ar.result)[0];
                    Rlog.d(LOG_TAG, "EVENT_GET_PREFERED_NETWORK_TYPE type:" + type + " modemNetworkMode = " + modemNetworkMode);
                    if (type != modemNetworkMode) {
                ...
                return;
            case 100:
                Rlog.d(LOG_TAG, "EVENT_UI_NOT_READY mIsPhoneReadySent:" + this.mIsPhoneReadySent);
                if (this.mIsPhoneReadySent) {
                    this.mIsPhoneReadySent = false;
                    updatePhoneReady(this.mPhoneId);
                    return;
                }
    ...
    public void sendSubscriptionSettings(boolean restoreNetworkSelection) {
        ...
        logd("sendSubscriptionSettings not do!");
    }
    ...
    protected void setPreferredNetworkTypeIfSimLoaded() {
        int subId = getSubId();
        logd("setPreferredNetworkTypeIfSimLoaded!");
        if (SubscriptionManager.from(this.mContext).isActiveSubId(subId)) {
            int type = PhoneFactory.calculatePreferredNetworkType(this.mContext, getSubId());
            getPreferredNetworkType(obtainMessage(EVENT_GET_PREFERED_NETWORK_TYPE));
        }
    }
