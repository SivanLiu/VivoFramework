...
public class BootReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (!intent.getAction().equals("android.intent.action.BOOT_COMPLETED") && !intent.getAction().equals("android.intent.action.LOCKED_BOOT_COMPLETED")) {
            ...
            if (context.startService(new Intent(context, QtiTelephonyService.class)) == null) {
                Log.e(TAG, "Could not start service");
            } else {
                Log.d(TAG, "Successfully started service");
            }
            if (context.startService(new Intent(context, LowLatencyService.class)) == null) {
                Log.e(TAG, "Could not start LowLatencyService service");
            } else {
                Log.d(TAG, "Successfully LowLatencyService started service");
            }
        }
    }
