...
public interface IUimRemoteSimlockServiceCallback extends IInterface {
    ...
    public static abstract class Stub extends Binder implements IUimRemoteSimlockServiceCallback {
        ...
        private static class Proxy implements IUimRemoteSimlockServiceCallback {
            ...
            public void uimRemoteSimlockGenerateRequestResponse(int token, int responseCode, byte[] requestData) throws RemoteException {
                Parcel _data = Parcel.obtain();
                Parcel _reply = Parcel.obtain();
                try {
                    _data.writeInterfaceToken(Stub.DESCRIPTOR);
                    ...
                    this.mRemote.transact(Stub.TRANSACTION_uimRemoteSimlockGenerateRequestResponse, _data, _reply, 0);
                    _reply.readException();
                } finally {
                    _reply.recycle();
                    _data.recycle();
            ...
            public void uimRemoteSimlockUnlockTimerResponse(int token, int responseCode, int timerValue) throws RemoteException {
                Parcel _data = Parcel.obtain();
                Parcel _reply = Parcel.obtain();
                try {
                    _data.writeInterfaceToken(Stub.DESCRIPTOR);
                    _data.writeInt(token);
                    _data.writeInt(responseCode);
                    _data.writeInt(timerValue);
                    this.mRemote.transact(Stub.TRANSACTION_uimRemoteSimlockUnlockTimerResponse, _data, _reply, 0);
                    _reply.readException();
                } finally {
                    _reply.recycle();
                    _data.recycle();
        ...
        public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            switch (code) {
                ...
                case TRANSACTION_uimRemoteSimlockGenerateRequestResponse /*6*/:
                    data.enforceInterface(DESCRIPTOR);
                    uimRemoteSimlockGenerateRequestResponse(data.readInt(), data.readInt(), data.createByteArray());
                    reply.writeNoException();
                    return true;
                case TRANSACTION_uimRemoteSimlockUnlockTimerResponse /*7*/:
                    data.enforceInterface(DESCRIPTOR);
                    uimRemoteSimlockUnlockTimerResponse(data.readInt(), data.readInt(), data.readInt());
                    reply.writeNoException();
                    return true;
                case 1598968902:
