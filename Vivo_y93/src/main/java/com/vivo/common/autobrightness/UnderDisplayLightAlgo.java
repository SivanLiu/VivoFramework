...
public class UnderDisplayLightAlgo {
    ...
    private static final String TAG = "UnderDisplayLightAlgo";
    ...
    private SensorEventListener mStepCountListener = new SensorEventListener() {
        public void onSensorChanged(SensorEvent event) {
            ...
            UnderDisplayLightAlgo.this.mStepCount = step_value;
            UnderDisplayLightAlgo.this.mRgbCureAlgoData.pushStepCountData(UnderDisplayLightAlgo.this.mStepCount);
            Slog.e(UnderDisplayLightAlgo.TAG, "RGB mStepCountListener step_count = " + step_value);
        }
    ...
    private class AdjustBarHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (msg == null) {
                Slog.e(UnderDisplayLightAlgo.TAG, "handleMessage msg is NULL");
                return;
            }
            switch (msg.what) {
                case 1:
                    UnderDisplayLightAlgo.this.modifyBrightness();
                    break;
                case 2:
                    UnderDisplayLightAlgo.this.modifyBrightnessMode();
                    break;
                case 3:
                    UnderDisplayLightAlgo.this.restore_param();
                    break;
                case 4:
                    UnderDisplayLightAlgo.this.setInertiaSensorEnabledInner(true);
                    break;
                case 5:
                    UnderDisplayLightAlgo.this.setInertiaSensorEnabledInner(false);
                    break;
            }
        }
    }

    public UnderDisplayLightAlgo(Context context, SensorManager sensorManager, Looper looper) {
    ...
    private boolean isNeedChange(int lux) {
        return this.mLastChangeLux > 30 ? lux < this.mLastChangeLux + -20 || lux > this.mLastChangeLux + 20 : this.mLastChangeLux > 10 ? lux < this.mLastChangeLux + -10 || lux > this.mLastChangeLux + 10 : lux < this.mLastChangeLux + -5 || lux > this.mLastChangeLux + 5;
    }

    public AutobrightInfo getAutoBrightness(boolean waitFirst, boolean mProximityStatus, int lux, AutobrightInfo info, boolean videogameflag, boolean mFirstLightPersist) {
        ...
        int changeTime = 0;
        int temp = lux;
        boolean tempvideogameflag = !videogameflag ? this.mIsPortrait ^ 1 : true;
        ...
        } else if (AblConfig.isNeedRegMotion()) {
            this.mMontionStatus = this.mRgbCureAlgoData.getPhoneMontionStatus();
        } else {
            this.mMontionStatus = abInfo.mMotionState;
        }
        if (t - this.mChangeTime > 360000) {
            this.mChangeCount = 0;
        }
        if (!this.mPriveVideoGameFlag && tempvideogameflag) {
            this.mChangeDownLux = vivoCalacSidewardsChangeDownTheashAls(info.mLightLux, tempvideogameflag);
            Slog.d(TAG, "vivo getAutoBrightness dlux " + this.mChangeDownLux + " vg=" + tempvideogameflag);
        ...
        } else if ((this.mPriveMontionStatus != this.mMontionStatus && this.mMontionStatus == 0) || (this.mPriveVideoGameFlag && (tempvideogameflag ^ 1) != 0)) {
            if (this.mTimePeriod.getTimePeriod() == PeriodType.DAY_TIME) {
                this.mChangeDownLux = this.mPhoneStatusChangeDownLux[0];
            } else {
                this.mChangeDownLux = this.mPhoneStatusChangeDownLux[0];
            }
            this.mChangeCount = 0;
        ...
        int brightness;
        int delayTime;
        if (waitFirst || (mFirstLightPersist && isNeedChange(lux))) {
            ...
            if (t - this.mChangeUpTime > ((long) changeTime) || (this.mChangeCount == 0 && (this.mOffVideoGameFlag ^ 1) != 0)) {
                ...
                if (AblConfig.isUse2048GrayScaleBacklight()) {
                    ...
                    if (t - this.mChangUpStartTimeStamp < 1000) {
                ...
                if (t - this.mAdjustBrightnessTimeStamp > ((long) delayTime)) {
                    ...
                    this.mOffVideoGameFlag = false;
                    if (tempvideogameflag || this.mMontionStatus == 1) {
                        if (this.mMontionStatus == 1) {
                            tempLux = vivoCalacMotionChangeDownTheashAls(info.mLightLux);
                        } else {
                            tempLux = 5001;
                        }
                        if (tempvideogameflag) {
                            tempVideoLux = vivoCalacSidewardsChangeDownTheashAls(info.mLightLux, tempvideogameflag);
                        } else {
                            tempVideoLux = 5001;
                        }
                        ...
                        if (tempLux >= lux) {
                            ...
                            if (videogameflag) {
                                tempVideoLux = vivoCalacVideoGameChangeDownAls(lux);
                            } else {
                                tempVideoLux = 0;
                            }
                            if (tempLux < tempVideoLux) {
                            ...
                            if (this.mChangeCount <= 0) {
                                tempOftenLux = 0;
                            } else if (videogameflag) {
                                tempOftenLux = vivoCalacOftenChangeDownAls(lux, true);
                            } else {
                                tempOftenLux = vivoCalacOftenChangeDownAls(lux, false);
                            }
                        ...
                        mLightLuxApplied = true;
                        this.nCurrentBrightness = calcBrightness(tempOftenLux, waitFirst);
                        lux = tempOftenLux;
                        this.mChangeDownFlag = true;
                        abInfo.mPrivBrightness = this.mPrivBrightness;
                    }
    ...
    private int vivoCalacChangeUpAls(int lux) {
        int tempLux = lux;
        if (lux > lux) {
            tempLux = lux;
        }
    ...
    private int vivoChangeDownAls(int lux, int phoneStatus) {
        int templux = lux;
        if (this.mTimePeriod.getTimePeriod() == PeriodType.DAY_TIME) {
            if (lux <= 8) {
                templux = lux + 2;
    ...
    private int vivoCalacVideoGameChangeDownAls(int lux) {
        int templux = lux;
        if (this.mTimePeriod.getTimePeriod() == PeriodType.DAY_TIME) {
            if (lux <= 10) {
                templux = lux + 5;
    ...
    private int vivoCalacOftenChangeDownAls(int lux, boolean vgmflag) {
        int templux = lux;
    ...
    private void vivoCalacUpDownBrightness(int current_brightness) {
        if (current_brightness <= 12) {
            ...
            this.mChangeUpBrightness = (int) ((mUpDownParam[4][0] * ((double) current_brightness)) + mUpDownParam[4][1]);
        }
        if (current_brightness <= 33) {
    ...
    private void vivoCalacOrigUpDowmGm(int current_brightness, int up_brightness, int down_brightness) {
        ...
        while (i < 4) {
            ...
            if (AblConfig.isUse2048GrayScaleBacklight()) {
                if (down_brightness <= 2 && current_brightness <= 7 && this.mTimePeriod.getTimePeriod() == PeriodType.DAY_TIME) {
                    temp_flag_down = false;
                    this.mChangeDownLux = -1;
    ...
    private void vivoCalacUpDownGm(int x_gm, int current_brightness) {
        ...
        if (this.bUserSettingBrightness) {
            ...
            if (temp_als >= 2 && temp_als <= 10 && this.mChangeUpLux < ((int) Math.ceil((((double) temp_als) * 2.7d) + 2.9d))) {
                this.mChangeUpLux = (int) Math.ceil((((double) temp_als) * 2.7d) + 2.9d);
            }
        } else {
            vivoCalacUpDownBrightness(current_brightness);
    ...
    public void setLigtSensorEnable(boolean enable) {
        if (enable) {
            ...
            if (this.mBacklightMode != 0 && AblConfig.isNeedRegMotion()) {
    ...
    public void dump(PrintWriter pw) {
        if (AblConfig.isDebug()) {
            pw.println("  ======== UnderDisplayLightAlgo");
