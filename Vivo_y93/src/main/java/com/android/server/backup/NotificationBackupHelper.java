...
public class NotificationBackupHelper extends BlobBackupHelper {
    ...
    protected byte[] getBackupPayload(String key) {
        ...
        try {
            return Stub.asInterface(ServiceManager.getService(ThreadsColumns.NOTIFICATION)).getBackupPayload(0);
        } catch (Exception e) {
    ...
    protected void applyRestoredPayload(String key, byte[] payload) {
        ...
        if (KEY_NOTIFICATIONS.equals(key)) {
            try {
                Stub.asInterface(ServiceManager.getService(ThreadsColumns.NOTIFICATION)).applyRestore(payload, 0);
            } catch (Exception e) {
                Slog.e(TAG, "Couldn't communicate with notification manager");
            }
        }
