...
public final class LockPatternChecker {
    ...
    public static AsyncTask<?, ?, ?> checkPatternNoCounting(final LockPatternUtils utils, final String pattern, final int userId, final OnCheckCallback callback) {
        AsyncTask<Void, Void, Boolean> task = new AsyncTask<Void, Void, Boolean>() {
            private byte[] mKind = new byte[10];
            private int mThrottleTimeout;

            protected Boolean doInBackground(Void... args) {
                try {
                    return Boolean.valueOf(utils.checkPatternNoCounting(pattern, userId, this.mKind));
                } catch (Exception e) {
                    this.mThrottleTimeout = 0;
                    return Boolean.valueOf(false);
                }
            }
            ...
            protected void onPostExecute(Boolean result) {
                if (result.booleanValue()) {
                    if (new String(this.mKind).toString().contains("visit")) {
                        Log.i("patternChecker", "pattern same as visit mode.");
                        this.mThrottleTimeout = -1;
                    } else if (new String(this.mKind).toString().contains("user")) {
                        Log.i("patternChecker", "pattern same as user mode.");
                        this.mThrottleTimeout = -2;
                    }
                }
                callback.onChecked(result.booleanValue(), this.mThrottleTimeout);
            }
    ...
    @VivoHook(hookType = VivoHookType.PUBLIC_API_METHOD)
    public static AsyncTask<?, ?, ?> checkPasswordNoCounting(final LockPatternUtils utils, final String pattern, final int userId, final OnCheckCallback callback) {
        AsyncTask<Void, Void, Boolean> task = new AsyncTask<Void, Void, Boolean>() {
            private byte[] mKind = new byte[10];
            private int mThrottleTimeout;
            ...
            protected Boolean doInBackground(Void... args) {
                try {
                    return Boolean.valueOf(utils.checkPasswordNoCounting(pattern, userId, this.mKind));
                } catch (Exception e) {
                    this.mThrottleTimeout = 0;
                    return Boolean.valueOf(false);
                }
            ...
            protected void onPostExecute(Boolean result) {
                if (result.booleanValue()) {
                    if (new String(this.mKind).toString().contains("visit")) {
                        Log.i("patternChecker", "password same as visit mode.");
                        this.mThrottleTimeout = -1;
                    } else if (new String(this.mKind).toString().contains("user")) {
                        Log.i("patternChecker", "password same as user mode.");
                        this.mThrottleTimeout = -2;
                    }
                }
                callback.onChecked(result.booleanValue(), this.mThrottleTimeout);
