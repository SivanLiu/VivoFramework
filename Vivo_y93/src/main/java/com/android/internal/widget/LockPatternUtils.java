...
public class LockPatternUtils {
    ...
    private final boolean isOverseaType;
    ...
    public LockPatternUtils(Context context) {
        ...
        this.isOverseaType = SystemProperties.get("ro.vivo.product.overseas", "").equals("yes");
    }
    ...
    private byte[] verifyCredential(String credential, int type, long challenge, int userId) throws RequestThrottledException {
        try {
            ...
            if (response == null) {
                Log.e(TAG, "Sorry, VerifyCredentialResponse == null in verifyCredential, return null.");
                return null;
    ...
    private boolean checkCredential(String credential, int type, int userId, CheckCredentialProgressCallback progressCallback) throws RequestThrottledException {
        try {
            ...
            if (response == null) {
                Log.e(TAG, "Sorry, VerifyCredentialResponse == null in checkCredential, return false.");
                return false;
            } else if (response.getResponseCode() == 0) {
                return true;
            } else {
    ...
    public byte[] verifyTiedProfileChallenge(String password, boolean isPattern, long challenge, int userId) throws RequestThrottledException {
        ...
        try {
            ...
            if (response == null) {
                Log.e(TAG, "Sorry, VerifyCredentialResponse == null in verifyTiedProfileChallenge, return null.");
                return null;
            } else if (response.getResponseCode() == 0) {
                return response.getPayload();
            } else {
    ...
    public void saveLockPattern(List<Cell> pattern, String savedPattern, int userId) {
        System.putInt(this.mContext.getContentResolver(), "vivo_invalidity_password", 0);
        if (pattern != null) {
            try {
                if (pattern.size() >= 4) {
    ...
    public void saveLockPassword(String password, String savedPassword, int requestedQuality, int userHandle) {
        if (Secure.getInt(this.mContentResolver, "vivo_save_password", 0) == 0) {
            System.putInt(this.mContext.getContentResolver(), "vivo_invalidity_password", 1);
        }
        if (password != null) {
            try {
    ...
    public boolean isSecure(int userId) {
        ...
        return (isLockPatternEnabled(mode, userId) || isLockPasswordEnabled(mode, userId)) ? true : FtFindPhoneLockUtil.isFindPhoneLocked(this.mContext);
    }
    ...
    @VivoHook(hookType = VivoHookType.PUBLIC_API_METHOD)
    public boolean checkPasswordNoCounting(String password, int userId, byte[] buf) {
        try {
            VerifyCredentialResponse response = getLockSettings().checkPasswordNoCounting(password, userId);
            if (response == null) {
                Log.e(TAG, "Sorry, VerifyCredentialResponse == null in checkPasswordNoCounting, return false.");
                return false;
            } else if (response.getResponseCode() != 0) {
                return false;
            } else {
                if (buf != null) {
                    Log.d(TAG, "already have the same password.");
                    byte[] kind = response.getPayload();
                    System.arraycopy(kind, 0, buf, 0, kind.length);
                }
                return true;
            }
        } catch (RemoteException e) {
    ...
    @VivoHook(hookType = VivoHookType.PUBLIC_API_METHOD)
    public boolean checkPatternNoCounting(String pattern, int userId, byte[] buf) {
        try {
            VerifyCredentialResponse response = getLockSettings().checkPatternNoCounting(pattern, userId);
            if (response == null) {
                Log.e(TAG, "Sorry, VerifyCredentialResponse == null in checkPatternNoCounting, return false.");
                return false;
            } else if (response.getResponseCode() != 0) {
                return false;
            } else {
                Log.d(TAG, "already have the same pattern.");
                if (buf != null) {
                    byte[] kind = response.getPayload();
                    System.arraycopy(kind, 0, buf, 0, kind.length);
                }
