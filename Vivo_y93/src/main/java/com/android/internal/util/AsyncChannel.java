...
public class AsyncChannel {
    ...
    private static class SyncMessenger {
        ...
        private static Message sendMessageSynchronouslyWithTimeout(Messenger dstMessenger, Message msg, int timeout) {
            SyncMessenger sm = obtain();
            if (dstMessenger == null || msg == null) {
                sm.mHandler.mResultMsg = null;
            } else {
                try {
                    ...
                    synchronized (sm.mHandler.mLockObject) {
                        dstMessenger.send(msg);
                        sm.mHandler.mLockObject.wait((long) timeout);
                    }
                } catch (InterruptedException e) {
                    sm.mHandler.mResultMsg = null;
                } catch (RemoteException e2) {
    ...
    public Message sendMessageSynchronouslyWithTimeout(int what, int timeout) {
        Message msg = Message.obtain();
        msg.what = what;
        return SyncMessenger.sendMessageSynchronouslyWithTimeout(this.mDstMessenger, msg, timeout);
    }
