...
public class GsmAlphabet {
    ...
    public static byte[] stringToAlphaField(String s) {
        if (s != null) {
            try {
                byte[] u16Buf = s.getBytes("UTF-16BE");
                byte[] alphaBuf = ucs2ToAlphaField(u16Buf, 0, u16Buf.length, 0);
                if (alphaBuf == null) {
                    alphaBuf = new byte[0];
                }
                return alphaBuf;
            } catch (UnsupportedEncodingException e) {
                Rlog.w(TAG, "encoding tag failed : UnsupportedEncodingException");
            }
        }
        return new byte[0];
    }
    ...
    @VivoHook(hookType = VivoHookType.NEW_METHOD)
    public static byte[] ucs2ToAlphaField(byte[] src, int srcOff, int srcLen, int destOff) {
        int i;
        int min = SelectionEvent.OUT_OF_BOUNDS;
        int max = 0;
        int sCSTCount = 0;
        ...
        if (srcLen > 2) {
            for (i = 0; i < srcLen; i += 2) {
                ...
                if (charToShiftTable.get((char) temp, -1) != -1) {
                    sCSTCount++;
                }
                if (temp < 0) {
                    max = min + 130;
        ...
        if (max - min < 128) {
            int outOff;
            if (((byte) (min & 128)) == ((byte) (max & 128))) {
                dest = new byte[(((srcLen / 2) + 3) + sCSTCount)];
                dest[destOff + 1] = (byte) ((srcLen / 2) + sCSTCount);
                dest[destOff] = (byte) -127;
                min &= 32640;
                dest[destOff + 2] = (byte) ((min >> 7) & 255);
                outOff = destOff + 3;
            } else {
                dest = new byte[(((srcLen / 2) + 4) + sCSTCount)];
                dest[destOff + 1] = (byte) ((srcLen / 2) + sCSTCount);
            ...
            while (true) {
                ...
                if (src[srcOff + i] == (byte) 0 && ((byte) (src[(srcOff + i) + 1] & 128)) == (byte) 0) {
                    ...
                    if (isGsmSeptets(c)) {
                        ...
                        if (v == -1) {
                            ...
                            if (v == -1) {
                                ...
                                outOff = outOff2 + 1;
                                dest[outOff2] = GSM_EXTENDED_ESCAPE;
                            }
                        } else {
                            outOff = outOff2;
                        }
                        ...
                        dest[outOff2] = (byte) (src[(srcOff + i) + 1] & 127);
                        outOff = outOff2;
                    }
                } else {
                    dest[outOff2] = (byte) (((((src[srcOff + i] << 8) & 65280) | (src[(srcOff + i) + 1] & 255)) - min) | 128);
                    outOff = outOff2;
                }
                outOff++;
                i += 2;
    ...
    public static Boolean isStringToGsm8Bit(String s) {
        ...
        for (int i = 0; i < sz; i++) {
            ...
            if (v == sCharsToGsmTables[0].get(32, 32) && c != ' ') {
