...
public final class BearerData {
    ...
    private static void encodeMsgCenterTimeStamp(BearerData bData, BitwiseOutputStream outStream) throws AccessException {
        outStream.write(8, 6);
        int year = 0;
        String longYear = String.valueOf(bData.msgCenterTimeStamp.year);
        String limitYear = "";
        if (!TextUtils.isEmpty(longYear)) {
            int strLength = longYear.length();
            limitYear = longYear.substring(strLength > 2 ? strLength - 2 : 0, strLength);
        }
        if (!TextUtils.isEmpty(limitYear)) {
            year = Integer.parseInt(limitYear);
        }
        outStream.write(8, intToByte(year));
        outStream.write(8, intToByte(bData.msgCenterTimeStamp.month + 1));
        outStream.write(8, intToByte(bData.msgCenterTimeStamp.monthDay));
        outStream.write(8, intToByte(bData.msgCenterTimeStamp.hour));
        outStream.write(8, intToByte(bData.msgCenterTimeStamp.minute));
        outStream.write(8, intToByte(bData.msgCenterTimeStamp.second));
    }
    ...
    public static byte[] encode(BearerData bData) {
        boolean z = true;
        boolean z2 = bData.userData != null ? bData.userData.userDataHeader != null : false;
        bData.hasUserDataHeader = z2;
        try {
            BitwiseOutputStream outStream = new BitwiseOutputStream(250);
            outStream.write(8, 0);
            ...
            String str = LOG_TAG;
            StringBuilder append = new StringBuilder().append("bData.msgCenterTimeStamp != null  ");
