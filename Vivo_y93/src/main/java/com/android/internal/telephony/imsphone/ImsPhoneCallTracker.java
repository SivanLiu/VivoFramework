...
public class ImsPhoneCallTracker extends CallTracker implements ImsPullCall {
    ...
    private Listener mImsCallListener = new Listener() {
        ...
        public void onCallProgressing(ImsCall imsCall) {
            ...
            if (imsCall == null || imsCall.getState() == 2) {
                ImsPhoneCallTracker.this.processCallStateChange(imsCall, State.ALERTING, 0);
            } else {
                ImsPhoneCallTracker.this.processCallStateChange(imsCall, State.DIALING, 0);
            }
        }
        ...
        public void onCallUpdated(ImsCall imsCall) {
            ...
            if (imsCall != null) {
                ...
                if (conn != null) {
                    if (imsCall != null && imsCall.getState() == 2 && conn.getCall().mState == State.DIALING) {
                        ImsPhoneCallTracker.this.processCallStateChange(imsCall, State.ALERTING, 0);
                    } else {
        ...
        public void onCallTerminated(ImsCall imsCall, ImsReasonInfo reasonInfo) {
            ...
            State oldState = ImsPhoneCallTracker.this.mForegroundCall.getState();
            ImsPhoneConnection conn = ImsPhoneCallTracker.this.findConnection(imsCall);
            if (conn != null) {
                callState = conn.getState();
            } else {
            ...
            ImsPhoneCallTracker.this.processCallStateChange(imsCall, State.DISCONNECTED, cause);
            if (reasonInfo.getCode() == RadioError.OEM_ERROR_1 && oldState == State.DISCONNECTING && ImsPhoneCallTracker.this.mForegroundCall.getState() == State.IDLE && ImsPhoneCallTracker.this.mBackgroundCall.getState() == State.HOLDING) {
            ...
            if (ImsPhoneCallTracker.this.mSwitchingFgAndBgCalls) {
                ...
                if (imsCall == ImsPhoneCallTracker.this.mCallExpectedToResume && ImsPhoneCallTracker.this.mRingingCall.getState() != State.WAITING) {
    ...
    /* renamed from: lambda$-com_android_internal_telephony_imsphone_ImsPhoneCallTracker_35464 */
    /* synthetic */ void m27x31588f77() {
        try {
    ...
    /* renamed from: lambda$-com_android_internal_telephony_imsphone_ImsPhoneCallTracker_36536 */
    /* synthetic */ int m28x3159073c() {
    ...
    public Connection dial(String dialString, int videoState, Bundle intentExtras) throws CallStateException {
        ...
        if (this.mSharedPreferenceProxy == null || this.mPhone.getDefaultPhone() == null) {
            ...
            oirMode = this.mPhone.getDefaultPhone().getClirSetting(0);
        }
    ...
    public boolean canDial() {
        ...
        if (this.mForegroundCall.getState().isAlive()) {
            return this.mBackgroundCall.getState().isAlive() ^ 1;
        }
        return true;
    }
    ...
    private void updatePhoneState() {
        ...
        if (this.mState != PhoneConstants.State.IDLE) {
            GsmCdmaPhone phone = this.mPhone.mDefaultPhone;
            if (phone != null) {
                phone.mCT.checkStateWhenImsCallNotIdle();
            }
        }
    }
    ...
    public void broadcastImsStateChange() {
        Context context = this.mPhone.getContext();
        int phoneId = this.mPhone.getPhoneId();
        int state = (isVolteEnabled() || isVideoCallEnabled()) ? 1 : 0;
        boolean isOperatorCT = GsmCdmaPhone.isOperatorCT(phoneId);
        boolean uiVolteSwitch = ImsManager.getVolteSwitchUIState(context, phoneId);
        log("broadcastImsStateChange() state= " + state + ", uiVolteSwitch=" + uiVolteSwitch);
        if (!isOperatorCT || !uiVolteSwitch || state != 0 || !onNotifyCtccImsDeregister()) {
            if (state == 1 && (uiVolteSwitch ^ 1) != 0) {
                state = 0;
                ImsManager instance = ImsManager.getInstance(context, phoneId);
                if (instance != null) {
                    instance.setEnhanced4gLteModeSettingForSlot(false);
                }
                log("broadcastImsStateChange() 0 to trun off phoneId=" + phoneId);
            }
            if (state == 1) {
                GsmCdmaPhone.onImsRegistered(phoneId);
            }
            Intent intent = new Intent(FtTelephonyAdapter.getFtTelephony(context).getImsStateChangeBroadcast());
            if (TelephonyManager.getDefault().getPhoneCount() == 1) {
                intent.addFlags(536870912);
            }
            intent.putExtra("regState", state);
            intent.putExtra("is_volte_support", isVolteEnabled());
            intent.putExtra("is_vilte_support", isVideoCallEnabled());
            intent.putExtra("is_operator_ct", isOperatorCT);
            intent.putExtra("is_switch_on", uiVolteSwitch);
            intent.putExtra("VoLTE", this.mImsFeatureEnabled[0]);
            intent.putExtra("ViLTE", this.mImsFeatureEnabled[1]);
            intent.putExtra("VoWiFi", this.mImsFeatureEnabled[2]);
            intent.putExtra("ViWiFi", this.mImsFeatureEnabled[3]);
            intent.putExtra("UTLTE", this.mImsFeatureEnabled[4]);
            intent.putExtra("UTWiFi", this.mImsFeatureEnabled[5]);
    ...
    @VivoHook(hookType = VivoHookType.NEW_METHOD)
    private boolean onNotifyCtccImsDeregister() {
        ...
        if (!SystemProperties.get("ro.vivo.product.overseas", "no").equals("yes")) {
            try {
                ...
                VivoTelephonyApiParams param = new VivoTelephonyApiParams("API_TAG_onNotifyCtccImsDeregister");
                param.put("phoneId", Integer.valueOf(this.mPhone.getPhoneId()));
                ret = iTelephony.vivoTelephonyApi(param);
            } catch (RemoteException e) {
                ret = null;
                log("onNotifyCtccImsDeregister()  catch RemoteException!");
            } catch (NullPointerException e2) {
                ret = null;
                log("onNotifyCtccImsDeregister()  catch NullPointerException!");
            } catch (Exception e3) {
                ret = null;
                log("onNotifyCtccImsDeregister()  catch Exception!");
            }
        }
        if (ret != null) {
            return true;
    ...
    void notifySrvccState(SrvccState state) {
        ...
        if (this.mSrvccState == SrvccState.COMPLETED) {
            ...
            if (this.mPendingMO != null) {
                log("notifySrvccState notify Disconnect and remove mPendingMO");
                this.mPendingMO.update(null, State.DISCONNECTED);
                this.mPendingMO.onDisconnect();
                removeConnection(this.mPendingMO);
                updatePhoneState();
            ...
            log("notifySrvccState clear mPendingMO =" + this.mPendingMO);
            this.mPendingMO = null;
        }
    }
    ...
    protected void log(String msg) {
        Rlog.d(LOG_TAG, "[ImsPhoneCallTracker][" + (this.mPhone == null ? -1 : this.mPhone.getPhoneId()) + "] " + msg);
    }

    @VivoHook(hookType = VivoHookType.CHANGE_CODE)
    protected void loge(String msg) {
        Rlog.e(LOG_TAG, "[ImsPhoneCallTracker][" + (this.mPhone == null ? -1 : this.mPhone.getPhoneId()) + "] " + msg);
    }
