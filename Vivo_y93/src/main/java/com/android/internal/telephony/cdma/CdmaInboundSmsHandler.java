...
public class CdmaInboundSmsHandler extends InboundSmsHandler {
    ...
    private void dispatchCtccReg(SmsMessageBase smsb) {
        log("vivo.intent.action.REG_CONFIRM,REG: " + smsb.getUserData());
        Intent intent = new Intent("vivo.intent.action.REG_CONFIRM");
        intent.putExtra("pdus", smsb.getUserData());
        intent.setComponent(new ComponentName("com.vivo.devicereg", "com.vivo.devicereg.receiver.RegReceiver"));
        intent.addFlags(134217728);
        this.mContext.sendOrderedBroadcast(intent, "android.permission.RECEIVE_SMS", 16, null, getHandler(), -1, null, null);
    }
