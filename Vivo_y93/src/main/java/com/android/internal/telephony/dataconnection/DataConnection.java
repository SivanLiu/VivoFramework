...
public class DataConnection extends StateMachine {
    ...
    private static int mDataErrRetryTime = 0;
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    private static int mDataOtherErrRetryTime = 0;
    private static AtomicInteger mInstanceNumber = new AtomicInteger(0);
    private static String[] sCmdToString = new String[18];
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    private static int testPropFlag = SystemProperties.getInt(VivoNetLowlatency.SYSTEMPROPERTIES_PRINT_LOG_FLAG, 0);
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    private int ERROR_UNSPECIFIED_NO_PDP = 65535;
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    private String cid = "";
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    ...
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    private int mRetryDataCallForPdpRejectNetConut = 0;
    ...
    private class DcActivatingState extends State {
        ...
        private static /* synthetic */ int[] m32xa73f2fb6() {
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case DataConnection.EVENT_SETUP_DATA_CONNECTION_DONE /*262145*/:
                    ...
                    boolean myResultNeedReport = false;
                    switch (m32xa73f2fb6()[result.ordinal()]) {
                        case 1:
                        ...
                        case 2:
                            ...
                            DataConnection.this.mInactiveState.setEnterNotificationParams(cp, result.mFailCause);
                            DataConnection.this.transitionTo(DataConnection.this.mInactiveState);
                            DataConnection.mDataErrRetryTime = DataConnection.mDataErrRetryTime + 1;
                            if (DataConnection.mDataErrRetryTime >= 5) {
                                myResultNeedReport = true;
                                DataConnection.mDataErrRetryTime = 0;
                                break;
                            }
                            break;
                        case 3:
                        ...
                        case 5:
                            ...
                            DataConnection.mDataErrRetryTime = 0;
                            DataConnection.mDataOtherErrRetryTime = 0;
                            DataConnection.this.mRetryDataCallForPdpRejectNetConut = 0;
                            if (!(cp.mApnContext == null || DataConnection.this.mApnSetting == null || !cp.mApnContext.getApnType().equalsIgnoreCase("default") || (DataConnection.this.mApnSetting.apn.equalsIgnoreCase("ctvpdn") ^ 1) == 0)) {
                                DataConnection.this.sendbrocastPingTest(1);
                                break;
                            }
                        default:
                            throw new RuntimeException("Unknown SetupResult, should not happen");
                    }
                    if (!(result == SetupResult.ERR_RilError || result == SetupResult.SUCCESS)) {
                        if (DataConnection.mDataOtherErrRetryTime < 2) {
                            DataConnection.mDataOtherErrRetryTime = DataConnection.mDataOtherErrRetryTime + 1;
                        } else {
                            DataConnection.mDataOtherErrRetryTime = 0;
                            myResultNeedReport = true;
                        }
                    }
                    DataConnection.this.loge("datacollect myResultNeedReport=" + myResultNeedReport + ",mDataErrRetryTime=" + DataConnection.mDataErrRetryTime + ",mDataOtherErrRetryTime=" + DataConnection.mDataOtherErrRetryTime);
                    if (myResultNeedReport || DataConnection.testPropFlag == 1) {
                        DataConnection.this.getCellInfoState();
                        DataCallResponse responseResult = ar.result;
                        if (!(responseResult == null || DataConnection.this.ERROR_UNSPECIFIED_NO_PDP == responseResult.status)) {
                            try {
                                if (DataConnection.this.mPhone != null) {
                                    int phoneid = DataConnection.this.mPhone.getPhoneId();
                                    int subid = DataConnection.this.mPhone.getSubId();
                                    DataConnection.this.mTelephonyManager = (TelephonyManager) DataConnection.this.mPhone.getContext().getSystemService("phone");
                                    if (DataConnection.this.mTelephonyManager != null) {
                                        DataConnection.this.mTelephonyManager;
                                        cstypestr = TelephonyManager.getNetworkTypeName(DataConnection.this.csType);
                                        DataConnection.this.mTelephonyManager;
                                        pstypestr = TelephonyManager.getNetworkTypeName(DataConnection.this.psType);
                                        plmnstr = DataConnection.this.mTelephonyManager.getSimOperatorNumericForPhone(phoneid);
                                    }
                                    phoneidstr = phoneid + "";
                                    String strCauseCode = SystemProperties.get("ril.pdp.end.cause", "0");
                                    DataConnection.this.sendBrocastToCollect(phoneidstr, responseResult.status + "_" + strCauseCode, DataConnection.this.lac + "", DataConnection.this.cid + "", cstypestr, pstypestr, plmnstr);
                                    DataConnection.this.log("datacollect  DcFailCause = " + responseResult.status + "_" + strCauseCode);
                                } else {
                                    cstypestr = DataConnection.this.csType + "";
                                    pstypestr = DataConnection.this.psType + "";
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
    ...
    private class DcDefaultState extends State {
        ...
        public void exit() {
            ...
            if (DataConnection.this.mPhone.getServiceStateTracker() != null) {
    ...
    private void checkSetMtu(ApnSetting apn, LinkProperties lp) {
        if (lp != null && apn != null && lp != null) {
            if (lp.getMtu() != 0) {
                ...
                int mtu = this.mPhone.getContext().getResources().getInteger(17694813);
                if (mtu != 0) {
                    lp.setMtu(mtu);
    ...
    private SetupResult onSetupConnectionCompleted(AsyncResult ar) {
        ...
        if (cp.mTag != this.mTag) {
            ...
            result.mFailCause = DcFailCause.fromInt(response.status);
            return result;
        } else if (TextUtils.isEmpty(response.ifname)) {
            result = SetupResult.ERR_RilError;
            result.mFailCause = DcFailCause.fromInt(response.status);
            return result;
        } else {
    ...
    private boolean isDefaultDataSubPhone(Phone phone) {
        int defaultDataPhoneId = SubscriptionController.getInstance().getPhoneId(SubscriptionController.getInstance().getDefaultDataSubId());
    ...
    NetworkCapabilities getNetworkCapabilities() {
        ...
        if (this.mDct.mChildrenMode) {
            isDataEnabled = this.mDct.mChildrenModeDataOn;
        } else {
            isDataEnabled = this.mDct.getDataEnabled();
        }
            ...
            for (String type : this.mApnSetting.types) {
                if (!this.mRestrictedNetworkOverride && this.mConnectionParams != null && this.mConnectionParams.mUnmeteredUseOnly && ApnSetting.isMeteredApnType(type, this.mPhone)) {
                    ...
                    if (isDataEnabled && isDefaultDataSubPhone(this.mPhone)) {
                        log("should make internet cp");
                        result.addCapability(12);
                    } else {
                        log("shouldn't make internet cp");
                    }
    ...
    private boolean updateNetworkInfoSuspendState() {
        ...
        if (sst.getCurrentDataConnectionState() != 0) {
            log("updateNetworkInfoSuspendState01 mNetworkInfo.getExtraInfo() = " + this.mNetworkInfo.getExtraInfo());
            ...
            log("updateNetworkInfoSuspendState03 mNetworkInfo.getExtraInfo() = " + this.mNetworkInfo.getExtraInfo());
            this.mNetworkInfo.setDetailedState(DetailedState.CONNECTED, null, this.mNetworkInfo.getExtraInfo());
        } else {
            log("updateNetworkInfoSuspendState02 mNetworkInfo.getExtraInfo() = " + this.mNetworkInfo.getExtraInfo());
            this.mNetworkInfo.setDetailedState(DetailedState.SUSPENDED, null, this.mNetworkInfo.getExtraInfo());
    ...
    private void getCellInfoState() {
        ...
        try {
            if (this.mPhone != null) {
                ...
                this.csType = servicestate.getNetworkType();
                this.psType = servicestate.getDataNetworkType();
                CellLocation cellLocation = this.mPhone.getCellLocation();
                if (cellLocation instanceof GsmCellLocation) {
                    GsmCellLocation myloc = (GsmCellLocation) cellLocation;
                    this.lac = myloc.getLac();
                    this.cid = myloc.getCid() + "";
                } else if (cellLocation instanceof CdmaCellLocation) {
                    CdmaCellLocation myloc2 = (CdmaCellLocation) cellLocation;
                    int sid = myloc2.getSystemId();
                    String nid = myloc2.getNetworkId() + "," + myloc2.getBaseStationId();
                    this.lac = sid;
                    this.cid = nid;
                }
            }
    ...
    @VivoHook(hookType = VivoHookType.NEW_METHOD)
    private void sendBrocastToCollect(String sim_id, String fail_cause, String lac1, String cid1, String cs1, String ps1, String plmn) {
        Intent intent = new Intent("vivo.intent.action.vivocloudfourpdpfail");
        intent.setPackage("com.vivo.networkimprove");
        intent.putExtra("SIM_ID", sim_id);
        intent.putExtra("FAIL_CAUSE", fail_cause);
        intent.putExtra("LAC", lac1);
        intent.putExtra("CID", cid1);
        intent.putExtra("CS", cs1);
        intent.putExtra("PS", ps1);
        intent.putExtra("PLMN", plmn);
        this.mPhone.getContext().sendBroadcast(intent);
    }

    private void sendbrocastPingTest(int flag) {
        Intent intent1 = new Intent();
        intent1.setAction("com.vivo.networkimprove.pingfail.datastatus");
        ...
        log(" pingtest 1  sendbrocast data ok  flag=" + flag);
    }
