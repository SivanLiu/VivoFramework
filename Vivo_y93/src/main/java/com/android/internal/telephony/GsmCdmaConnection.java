...
public class GsmCdmaConnection extends Connection {
    ...
    private static /* synthetic */ int[] m8xd9c92f69() {
        ...
        switch (causeCode) {
            ...
            default:
                ...
                if (!this.mIsEmergencyCall) {
                    ...
                    if (uiccCard != null && ((uiccCard.getCardState() == CardState.CARDSTATE_ABSENT || uiccCard.getCardState() == CardState.CARDSTATE_ERROR) && (isPhoneTypeGsm() || phone.mCdmaSubscriptionSource == 0))) {
                        return 19;
                    }
    ...
    private boolean processPostDialChar(char c) {
        if (PhoneNumberUtils.is12Key(c)) {
            if (isPhoneTypeGsm()) {
                ...
                this.mOwner.mCi.sendBurstDtmf(String.valueOf(c), 0, 0, this.mHandler.obtainMessage(1));
            }
        } else if (isPause(c)) {
            if (!isPhoneTypeGsm()) {
    ...
    private GsmCdmaCall parentFromDCState(State state) {
        switch (m8xd9c92f69()[state.ordinal()]) {
            case 1:
            case 2:
            case 3:
