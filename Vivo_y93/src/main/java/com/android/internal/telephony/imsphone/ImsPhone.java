...
public class ImsPhone extends ImsPhoneBase {
    ...
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    private static final List<String> SPECIAL_CALL_WAITING_LIST = Arrays.asList(new String[]{"52000", "52004"});
        ...
        this.mResultReceiver = new BroadcastReceiver() {
            public void onReceive(Context context, Intent intent) {
                if (getResultCode() == -1) {
                    ...
                    ((NotificationManager) ImsPhone.this.mContext.getSystemService("notification")).notify("wifi_calling", 1, new Builder(ImsPhone.this.mContext).setSmallIcon(17301642).setContentTitle(title).setContentText(messageNotification).setAutoCancel(true).setContentIntent(PendingIntent.getActivity(ImsPhone.this.mContext, 0, resultIntent, 134217728)).setStyle(new BigTextStyle().bigText(messageNotification)).setChannelId("others").build());
                }
            }
        };
    ...
    public boolean handleUssdRequest(String ussdRequest, ResultReceiver wrappedCallback) throws CallStateException {
        ...
        try {
            Connection conn = dialInternal(ussdRequest, 0, null, wrappedCallback);
            if (conn != null) {
    ...
    private void openAutoCallBbklog(int flag) {
        ...
        Rlog.d(LOG_TAG, "autolog openAutoCallBbklog  ,,bbkAutoLogFlag=" + bbkAutoLogFlag + ",,calllog_flag=" + calllog_flag + ",,,,flag = " + flag);
    }
    ...
    private Connection dialInternal(String dialString, int videoState, Bundle intentExtras, ResultReceiver wrappedCallback) throws CallStateException {
        ...
        if (this.mDefaultPhone.getPhoneType() == 2) {
            if (this.mDefaultPhone.isUtEnabled() && GsmCdmaPhone.isSimOnceRegisteredOnIms(this.mDefaultPhone.getPhoneId())) {
                i = GsmCdmaPhone.isOperatorSupportUT(this.mDefaultPhone.getPhoneId());
            }
            if ((i ^ 1) != 0) {
    ...
    private boolean isSpecialCallWaitingOperator() {
        String plmn = TelephonyManager.getDefault().getSimOperatorNumeric();
        Rlog.d(LOG_TAG, "isSpecialCallWaitingOperator plmn is: " + plmn);
        return SPECIAL_CALL_WAITING_LIST.contains(plmn);
    }

    @VivoHook(hookType = VivoHookType.CHANGE_CODE)
    public void setCallWaiting(boolean enable, Message onComplete) {
        int serviceClass = 1;
        if (isSpecialCallWaitingOperator()) {
            serviceClass = 0;
        }
    ...
    public void getCallBarring(String facility, Message onComplete) {
        getCallBarring(facility, 1, onComplete);
    }
    ...
    public void setCallBarring(String facility, boolean lockState, String password, Message onComplete) {
        setCallBarring(facility, lockState, 1, password, onComplete);
    }
            ...
            for (int i = 0; i < wfcOperatorErrorCodes.length; i++) {
                ...
                if (codes.length != 2) {
                    ...
                    if (!Character.isLetterOrDigit(codes[0].charAt(codeStringLength - 1)) || imsReasonInfo.mExtraMessage.length() <= codeStringLength || !Character.isLetterOrDigit(imsReasonInfo.mExtraMessage.charAt(codeStringLength))) {
                        CharSequence title = this.mContext.getText(17041510);
    ...
    public void notifySubInfoReadyForIMS() {
        this.mCT.broadcastImsStateChange();
    }
}
