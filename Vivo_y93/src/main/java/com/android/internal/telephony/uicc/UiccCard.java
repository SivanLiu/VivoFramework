...
public class UiccCard {
    ...
    private void promptForRestart(boolean isAdded) {
        synchronized (this.mLock) {
            ...
            if (isAdded) {
                title = r.getString(17041302);
            } else {
                title = r.getString(17041305);
            }
            if (isAdded) {
                message = r.getString(17041301);
            } else {
                message = r.getString(17041304);
            }
            AlertDialog dialog = new Builder(this.mContext).setTitle(title).setMessage(message).setPositiveButton(r.getString(17041306), listener).create();
    ...
    private void promptInstallCarrierApp(String pkgName) {
        ...
        Resources r = Resources.getSystem();
        String message = r.getString(17039619);
        AlertDialog dialog = new Builder(this.mContext).setMessage(message).setNegativeButton(r.getString(17039620), listener).setPositiveButton(r.getString(17039618), listener).create();
        dialog.getWindow().setType(2003);
        dialog.show();
    }
