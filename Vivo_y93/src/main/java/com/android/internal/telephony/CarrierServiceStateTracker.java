...
public class CarrierServiceStateTracker extends Handler {
    ...
    public class EmergencyNetworkNotification implements NotificationType {
        ...
        public Builder getNotificationBuilder() {
            ...
            return new Builder(context).setContentTitle(title).setStyle(new BigTextStyle().bigText(details)).setContentText(details).setChannel("others");
        }
    }
            ...
            if (extTelephony != null) {
                try {
                    if (extTelephony.isVendorApkAvailable("com.qualcomm.qti.networksetting")) {
                        ...
                        return new Builder(context).setContentTitle(title).setStyle(new BigTextStyle().bigText(details)).setContentText(details).setChannel("others").setContentIntent(settingsIntent);
                    }
    ...
    private boolean isGlobalMode() {
        boolean z = true;
        try {
            if (Global.getInt(this.mPhone.getContext().getContentResolver(), "preferred_network_mode" + this.mPhone.getSubId(), 22) != 10) {
                z = false;
            }
            return z;
        } catch (Exception e) {
    ...
    public void sendNotification(NotificationType notificationType) {
        if (evaluateSendingMessage(notificationType)) {
            ...
            builder.setWhen(System.currentTimeMillis()).setAutoCancel(true).setSmallIcon(17301642).setColor(context.getResources().getColor(17170765));
