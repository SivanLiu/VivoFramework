...
class ZygoteConnection {
    ...
    private Runnable handleChildProc(Arguments parsedArgs, FileDescriptor[] descriptors, FileDescriptor pipeFd) {
        if (parsedArgs.niceName != null) {
            Build.resetBuild(parsedArgs.niceName);
        }
        closeSocket();
        if (descriptors != null) {
            try {
    ...
    private boolean doVivoCmd(Arguments parsedArgs) {
        if (this.peer == null || this.peer.getUid() != 1000 || !VIVO_CMD_PREFIX.equals(parsedArgs.niceName)) {
            return false;
        }
        if (UPDATE_FONT.equals(parsedArgs.instructionSet)) {
            reloadVivoFont();
        }
        return true;
    }

    private void reloadVivoFont() {
        Typeface.reloadVivoFont(true);
    }
}
