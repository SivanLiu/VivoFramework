...
public class DockedDividerUtils {
    ...
    public static int calculateAlienMiddlePosition(boolean isHorizontalDivision, Rect insets, int displayWidth, int displayHeight, int dividerSize) {
        int end;
        int midPoint;
        int start = isHorizontalDivision ? insets.top : insets.left;
        if (isHorizontalDivision) {
        ...
        if (isHorizontalDivision) {
            if (isHomeIndicatorOn()) {
                midPoint = (displayHeight / 2) - (dividerSize / 2);
            } else {
                midPoint = (end / 2) - (dividerSize / 2);
            }
        } else if (isHomeIndicatorOn()) {
            midPoint = (((end - start) / 2) + start) - (dividerSize / 2);
        } else {
            midPoint = (((end - start) / 2) + start) - (dividerSize / 2);
        }
        Slog.d("multiwinowsnap", "calculateMiddlePosition midpoint is " + midPoint);
        return midPoint;
    }
    ...
    static boolean isHomeIndicatorOn() {
        String navBarOverride = SystemProperties.get("qemu.hw.mainkeys", "1");
        Slog.d("multiwinowsnap", "DEBUG_ALIENSCREEN:isHomeIndicatorOn=" + navBarOverride);
