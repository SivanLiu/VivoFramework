...
public class BackdropFrameRenderer extends Thread implements FrameCallback {
    ...
    Drawable mVivoResizeBgDrawable = null;
    boolean mVivoResizingBgSupport = false;
    ...
    public void initVivoBgResizing(Context context) {
        int vivoResizeBgForceColor = 0;
        try {
            if (context instanceof Activity) {
                String fullActivityName = ((Activity) context).getIntent().getComponent().flattenToShortString();
                if (fullActivityName.contains("com.tencent.qqlive") || fullActivityName.contains("com.youku.phone") || (fullActivityName.contains("com.qiyi.video") && (SystemProperties.getBoolean("persist.vivo.multiwindow_resizebg_enable_all", false) ^ 1) != 0)) {
                    this.mVivoResizingBgSupport = true;
                    vivoResizeBgForceColor = 0;
                } else {
                    this.mVivoResizingBgSupport = SystemProperties.getBoolean("persist.vivo.multiwindow_resizebg_enable_force", false);
                    vivoResizeBgForceColor = SystemProperties.getInt("persist.vivo.multiwindow_resizebg_color_force", -1);
                }
            }
        } catch (Exception e) {
            Log.e("BackdropFrameRenderer", "initVivoBgResizing", e);
            this.mVivoResizingBgSupport = false;
        }
        if (this.mVivoResizeBgDrawable == null && this.mVivoResizingBgSupport && -1 != vivoResizeBgForceColor) {
            this.mVivoResizeBgDrawable = new ColorDrawable(Color.argb(255, Color.red(vivoResizeBgForceColor), Color.green(vivoResizeBgForceColor), Color.blue(vivoResizeBgForceColor)));
        }
    }
    ...
    private void redrawLocked(Rect newBounds, boolean fullscreen, Rect systemInsets, Rect stableInsets) {
        ...
        if ((this.mLastCaptionHeight != 0 || !this.mDecorView.isShowingCaption()) && this.mLastContentWidth != 0 && this.mLastContentHeight != 0) {
            ...
            if (this.mVivoResizingBgSupport) {
                Drawable background_new;
                if (this.mVivoResizeBgDrawable != null) {
                    background_new = this.mVivoResizeBgDrawable;
                } else {
                    background_new = this.mResizingBackgroundDrawable;
                }
                if (background_new != null) {
                    background_new.setBounds(0, this.mLastCaptionHeight, left + width, top + height);
                    background_new.draw(canvas);
                }
            } else if (this.mResizingBackgroundDrawable != null) {
