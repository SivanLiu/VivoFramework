...
public class ColorManager {
    ...
    private static ColorManager[] myInstance = new ColorManager[3];
    ...
    private static ServiceDead serviceDie = new ServiceDead();
    ...
    private static class DisplayConn implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            if (ColorManager.colorMgrListener != null) {
                ...
                if (!ColorManager.isConnecting) {
                    ...
                    ColorManager.isBound = true;
                    Log.v(ColorManager.TAG, "service connected, callback called");
                    ColorManager.colorMgrListener.onConnected();
                }
                ColorManager.serviceDie;
                ServiceDead.mToken = service;
                try {
                    service.linkToDeath(ColorManager.serviceDie, 0);
                } catch (Exception e) {
                }
                return;
            }
        ...
        public void onServiceDisconnected(ComponentName name) {
            Log.e(ColorManager.TAG, "service disconnected");
            ColorManager.isBound = false;
        }
    }
    ...
    private static class ServiceDead implements DeathRecipient {
        public static IBinder mToken;

        public void binderDied() {
            Log.e(ColorManager.TAG, "color service is dead");
            boolean serviceBound = false;
            while (!serviceBound) {
                try {
                    ColorManager.isConnecting = true;
                    try {
                        if (ColorManager.isBound) {
                            ColorManager.serviceContext.getApplicationContext().unbindService(ColorManager.conn);
                            ColorManager.isBound = false;
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    ...
                    serviceIntent.setClassName("com.qti.service.colorservice", ColorManager.SERVICE_INTENT_NAME);
                    serviceBound = ColorManager.serviceContext.getApplicationContext().bindService(serviceIntent, ColorManager.conn, 1);
                    if (serviceBound) {
                        Log.e(ColorManager.TAG, "service restarted success");
                    } else {
                        Log.e(ColorManager.TAG, "Failed to connect to remote service");
                        ColorManager.isConnecting = false;
                    }
    ...
    public boolean isFeatureSupported(DCM_FEATURE feature) throws IllegalArgumentException {
        if (!isBound || this.myApplication == null) {
            return false;
        }
        if (feature != null) {
            try {
                PackageInfo packageInfo = this.myApplication.getPackageManager().getPackageInfo("com.qti.service.colorservice", 128);
                try {
    ...
    public int setUserColorBalance(double red, double green, double blue) {
        if (myInstance[this.displayId] == null) {
            Log.e(TAG, "Instance for the display type " + this.displayId + " doesnt exist");
            return -999;
        }
        try {
            if (!isFeatureSupported(DCM_FEATURE.FEATURE_USER_COLOR_BALANCE)) {
                return RET_NOT_SUPPORTED;
            }
            try {
                Log.v(TAG, "Calling setUserColorBalance for display " + this.displayId);
                int retVal = service.setUserColorBalance(this.displayId, 0, red, green, blue);
                if (retVal != 0) {
                    Log.e(TAG, "Service setUserColorBalance failed with return value " + retVal);
                    return -999;
                }
                Log.v(TAG, "setUserColorBalance() worked");
                return retVal;
            } catch (RemoteException e) {
                Log.e(TAG, "Service set color balance failed");
                return -999;
    ...
    public double[] getUserColorBalance() {
        double[] retArray = new double[]{-999.0d, 0.0d, 0.0d, 0.0d};
        if (myInstance[this.displayId] == null) {
            Log.e(TAG, "Instance for the display type " + this.displayId + " doesnt exist");
            retArray[0] = -999.0d;
            return retArray;
        ...
        try {
            if (isFeatureSupported(DCM_FEATURE.FEATURE_USER_COLOR_BALANCE)) {
                try {
                    ...
                    double[] retVal = new double[]{-999.0d, 0.0d, 0.0d, 0.0d};
                    retVal = service.getUserColorBalance(this.displayId);
                    if (retVal != null) {
                        if (retVal.length > 0) {
                            if (retVal[0] < 0.0d) {
                                Log.e(TAG, "Service getUserColorBalance failed with return value " + retVal[0]);
                                retArray[0] = -999.0d;
                                return retArray;
                            }
                            retArray[0] = retVal[0];
    ...
    public int setSunlightVisibilityEnabled(boolean enable) {
        if (myInstance[this.displayId] == null) {
            ...
            try {
                ...
                int retVal = service.setActiveFeatureControl(this.displayId, ACTIVE_FEATURE_TYPE.FEATURE_SUNLIGHT_VISBILITY_IMPROVEMENT.getValue(), (!enable ? CONTROL_REQUEST_TYPE.OFF : CONTROL_REQUEST_TYPE.ON).getValue());
                if (retVal != 0) {
    ...
    public int setAdaptiveBacklightEnabled(boolean enable) {
        if (myInstance[this.displayId] == null) {
            ...
            try {
                Log.v(TAG, "Calling setActiveFeatureControl for display " + this.displayId);
                int retVal = service.setActiveFeatureControl(this.displayId, ACTIVE_FEATURE_TYPE.FEATURE_ADAPTIVE_BACKLIGHT.getValue(), (!enable ? CONTROL_REQUEST_TYPE.OFF : CONTROL_REQUEST_TYPE.ON).getValue());
                if (retVal != 0) {
                    Log.e(TAG, "Service setActiveFeatureControl failed with return value " + retVal);
                    return -999;
                }
                ...
                if (retVal2 != null) {
                    if (retVal2.length >= 10) {
                        ...
                        if (retVal != null && retVal.length >= 6) {
                            ...
                            PictureAdjustmentConfig paConfig = new PictureAdjustmentConfig(params, retVal[1], retVal[2], retVal[3], retVal[4], retVal[5]);
    ...
    public void release() {
        ...
        if (serviceDie != null) {
            try {
                Log.e(TAG, "de-register death notify");
                ServiceDead serviceDead = serviceDie;
                ServiceDead.mToken.unlinkToDeath(serviceDie, 0);
            } catch (Exception e) {
            }
        }
        serviceContext = null;
        colorMgrListener = null;
    }
    ...
    private static boolean isServiceRunning() {
        ...
        try {
            inputStream = new ProcessBuilder(new String[]{"/system/bin/sh", "-c", "ps"}).start().getInputStream();
