...
public class PduParser {
    ...
    static final /* synthetic */ boolean f24-assertionsDisabled = (PduParser.class.desiredAssertionStatus() ^ 1);
    ...
    protected static int parseUnsignedInt(ByteArrayInputStream pduDataStream) {
        if (f24-assertionsDisabled || pduDataStream != null) {
            int result = 0;
    ...
    protected static int parseValueLength(ByteArrayInputStream pduDataStream) {
        if (f24-assertionsDisabled || pduDataStream != null) {
            int temp = pduDataStream.read();
            if (f24-assertionsDisabled || -1 != temp) {
                int first = temp & 255;
                if (first <= 30) {
                    return first;
                }
    ...
    protected static void parseContentTypeParams(ByteArrayInputStream pduDataStream, HashMap<Integer, Object> map, Integer length) {
        if (!f24-assertionsDisabled && pduDataStream == null) {
            throw new AssertionError();
        } else if (f24-assertionsDisabled || length.intValue() > 0) {
            int startPos = pduDataStream.available();
            int lastLen = length.intValue();
            while (lastLen > 0) {
                int param = pduDataStream.read();
                if (f24-assertionsDisabled || -1 != param) {
                    lastLen--;
                    switch (param) {
                        case 129:
                            pduDataStream.mark(1);
    ...
    protected boolean parsePartHeaders(ByteArrayInputStream pduDataStream, PduPart part, int length) {
        if (!f24-assertionsDisabled && pduDataStream == null) {
            throw new AssertionError();
        } else if (!f24-assertionsDisabled && part == null) {
            throw new AssertionError();
        } else if (f24-assertionsDisabled || length > 0) {
            int startPos = pduDataStream.available();
            int lastLen = length;
            while (lastLen > 0) {
                int header = pduDataStream.read();
                if (f24-assertionsDisabled || -1 != header) {
                    lastLen--;
                    if (header > 127) {
                        switch (header) {
                            case 142:
    ...
    private static int checkPartPosition(PduPart part) {
        if (!f24-assertionsDisabled && part == null) {
            throw new AssertionError();
