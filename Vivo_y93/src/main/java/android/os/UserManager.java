...
public class UserManager {
    ...
    public static final String DOUBLE_APP_PROFILE_NAME = "DOBULE_APP";
    ...
    public List<UserHandle> getUserProfiles() {
        ...
        for (int userId : userIds) {
            if (this.mContext != null && isDoubleAppUserExist() && userId == getDoubleAppUserId()) {
                String callingPkg = this.mContext.getPackageManager().getNameForUid(Binder.getCallingUid());
                if (!"com.mobileiron".equals(callingPkg)) {
                    if (!"com.mobileiron.anyware.android".equals(callingPkg)) {
                        if ("com.google.android.googlequicksearchbox".equals(callingPkg)) {
                        }
                    }
                }
            }
    ...
    public static int getMaxSupportedUsers() {
        ...
        return SystemProperties.getInt("fw.max_users", Resources.getSystem().getInteger(17694814));
    }
    ...
    public boolean isDoubleAppUserExist() {
        boolean exist = false;
        try {
            return this.mService.isDoubleAppUserExist();
        } catch (RemoteException re) {
            Log.w(TAG, "Could not get double app user exist state", re);
            re.printStackTrace();
            return exist;
        }
    }
    ...
    public int getDoubleAppUserId() {
        int doubleAppUserId = -10000;
        try {
            return this.mService.getDoubleAppUserId();
        } catch (RemoteException re) {
            Log.w(TAG, "Could not get double app user exist state", re);
            re.printStackTrace();
