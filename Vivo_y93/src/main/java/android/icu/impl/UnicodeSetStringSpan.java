...
public class UnicodeSetStringSpan {
    ...
    private static final class OffsetList {
        ...
        static final /* synthetic */ boolean f73-assertionsDisabled = (OffsetList.class.desiredAssertionStatus() ^ 1);
        private int length;
        ...
        public void addOffset(int offset) {
            ...
            if (f73-assertionsDisabled || this.list[i] == 0) {
                this.list[i] = 1;
                this.length++;
                return;
            }
        ...
        public void addOffsetAndCount(int offset, int count) {
            if (f73-assertionsDisabled || count > 0) {
                int i = this.start + offset;
                if (i >= this.list.length) {
                    i -= this.list.length;
                }
