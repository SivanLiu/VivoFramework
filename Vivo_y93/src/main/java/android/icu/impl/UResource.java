...
public final class UResource {
    ...
    public static final class Key implements CharSequence, Cloneable, Comparable<Key> {
        ...
        static final /* synthetic */ boolean f3-assertionsDisabled = (Key.class.desiredAssertionStatus() ^ 1);
        private byte[] bytes;
        private int length;
        private int offset;
        private String s;
        ...
        public char charAt(int i) {
            if (f3-assertionsDisabled || (i >= 0 && i < this.length)) {
        ...
        public Key subSequence(int start, int end) {
            if (!f3-assertionsDisabled && (start < 0 || start >= this.length)) {
                throw new AssertionError();
            } else if (f3-assertionsDisabled || (start <= end && end <= this.length)) {
