...
public final class CollationData {
    ...
    static final /* synthetic */ boolean f84-assertionsDisabled = (CollationData.class.desiredAssertionStatus() ^ 1);
    ...
    int getIndirectCE32(int ce32) {
        if (f84-assertionsDisabled || Collation.isSpecialCE32(ce32)) {
    ...
    private void makeReorderRanges(int[] reorder, boolean latinMustMove, UVector32 ranges) {
        ...
        if (length != 0 && (length != 1 || reorder[0] != 103)) {
            ...
            if (!f84-assertionsDisabled && this.scriptStarts.length < 2) {
                throw new AssertionError();
            } else if (f84-assertionsDisabled || this.scriptStarts[0] == 0) {
                int lowStart = this.scriptStarts[1];
                if (f84-assertionsDisabled || lowStart == 768) {
                    int highLimit = this.scriptStarts[this.scriptStarts.length - 1];
                    if (f84-assertionsDisabled || highLimit == 65280) {
                        int start;
                        int specials = 0;
                        for (int i : reorder) {
                        ...
                        if (specials == 0 && reorder[0] == 25 && (latinMustMove ^ 1) != 0) {
                            ...
                            if (f84-assertionsDisabled || index != 0) {
                                ...
                                if (f84-assertionsDisabled || lowStart <= start) {
