...
public final class FCDIterCollationIterator extends IterCollationIterator {
    ...
    static final /* synthetic */ boolean f95-assertionsDisabled = (FCDIterCollationIterator.class.desiredAssertionStatus() ^ 1);
    private int limit;
    ...
    protected char handleGetTrailSurrogate() {
        if (this.state.compareTo(State.ITER_IN_FCD_SEGMENT) <= 0) {
            ...
            return (char) trail;
        } else if (f95-assertionsDisabled || this.pos < this.normalized.length()) {
    ...
    private void switchToForward() {
        if (!f95-assertionsDisabled && this.state != State.ITER_CHECK_BWD && ((this.state != State.ITER_IN_FCD_SEGMENT || this.pos != this.limit) && (this.state.compareTo(State.IN_NORM_ITER_AT_LIMIT) < 0 || this.pos != this.normalized.length()))) {
            throw new AssertionError();
    ...
    private boolean nextSegment() {
        if (f95-assertionsDisabled || this.state == State.ITER_CHECK_FWD) {
            ...
            this.limit = this.pos + this.s.length();
            if (f95-assertionsDisabled || this.pos != this.limit) {
    ...
    private void switchToBackward() {
        if (!f95-assertionsDisabled && this.state != State.ITER_CHECK_FWD && ((this.state != State.ITER_IN_FCD_SEGMENT || this.pos != this.start) && (this.state.compareTo(State.IN_NORM_ITER_AT_LIMIT) < 0 || this.pos != 0))) {
            throw new AssertionError();
    ...
    private boolean previousSegment() {
        if (f95-assertionsDisabled || this.state == State.ITER_CHECK_BWD) {
            this.pos = this.iter.getIndex();
            if (this.s == null) {
                this.s = new StringBuilder();
            } else {
            ...
            this.start = this.pos - this.s.length();
            if (f95-assertionsDisabled || this.pos != this.start) {
                this.iter.moveIndex(this.s.length());
                this.state = State.ITER_IN_FCD_SEGMENT;
                return true;
            }
