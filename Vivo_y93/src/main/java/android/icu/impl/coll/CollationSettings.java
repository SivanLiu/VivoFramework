...
public final class CollationSettings extends SharedObject {
    ...
    static final /* synthetic */ boolean f77-assertionsDisabled = (CollationSettings.class.desiredAssertionStatus() ^ 1);
    ...
    void aliasReordering(CollationData data, int[] codesAndRanges, int codesLength, byte[] table) {
        ...
        if (firstSplitByteRangeIndex == rangesLimit) {
            if (f77-assertionsDisabled || !reorderTableHasSplitBytes(table)) {
                this.minHighNoReorder = 0;
                this.reorderRanges = null;
            } else {
                throw new AssertionError();
            }
        } else if (f77-assertionsDisabled || table[codesAndRanges[firstSplitByteRangeIndex] >>> 24] == (byte) 0) {
    ...
    public void setReordering(CollationData data, int[] codes) {
        ...
        if (!f77-assertionsDisabled && rangesLength < 2) {
            throw new AssertionError();
        } else if (f77-assertionsDisabled || ((ranges[0] & DateTimePatternGenerator.MATCH_ALL_FIELDS_LENGTH) == 0 && (ranges[rangesLength - 1] & DateTimePatternGenerator.MATCH_ALL_FIELDS_LENGTH) != 0)) {
            int rangesStart;
    ...
    private static boolean reorderTableHasSplitBytes(byte[] table) {
        if (f77-assertionsDisabled || table[0] == (byte) 0) {
            for (int i = 1; i < 256; i++) {
                if (table[i] == (byte) 0) {
                    return true;
                }
    ...
    private long reorderEx(long p) {
        if (!f77-assertionsDisabled && this.minHighNoReorder <= 0) {
            throw new AssertionError();
    ...
    public void setCaseFirst(int value) {
        if (f77-assertionsDisabled || value == 0 || value == 512 || value == 768) {
