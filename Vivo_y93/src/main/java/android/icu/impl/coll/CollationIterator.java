...
public abstract class CollationIterator {
    ...
    static final /* synthetic */ boolean f81-assertionsDisabled = (CollationIterator.class.desiredAssertionStatus() ^ 1);
    ...
    private static final class SkippedState {
        /* renamed from: -assertionsDisabled */
        static final /* synthetic */ boolean f82-assertionsDisabled = (SkippedState.class.desiredAssertionStatus() ^ 1);
        ...
        void incBeyond() {
            if (f82-assertionsDisabled || !hasNext()) {
                this.pos++;
                return;
            }
    ...
    public final long nextCE() {
        ...
        } else if (f81-assertionsDisabled || this.cesIndex == this.ceBuffer.length) {
    ...
    final void setCurrentCE(long ce) {
        if (f81-assertionsDisabled || this.cesIndex > 0) {
        ...
        while (Collation.isSpecialCE32(ce32)) {
            ...
            switch (Collation.tagFromCE32(ce32)) {
                ...
                case 12:
                    ...
                    return;
                case 13:
                    if (!f81-assertionsDisabled && !forward) {
                        throw new AssertionError();
                    } else if (f81-assertionsDisabled || isLeadSurrogate(c)) {
    ...
    private final int nextCE32FromDiscontiguousContraction(CollationData d, CharsTrie suffixes, int ce32, int lookAhead, int c) {
        ...
        if (f81-assertionsDisabled || fcd16 > 255) {
            int nextCp = nextSkippedCodePoint();
            if (nextCp < 0) {
                backwardNumSkipped(1);
                return ce32;
    ...
    private final long previousCEUnsafe(int c, UVector32 offsets) {
        ...
        this.cesIndex = 0;
        if (f81-assertionsDisabled || this.ceBuffer.length == 0) {
            int offset = getOffset();
            while (this.numCpFwd > 0) {
                this.numCpFwd--;
                nextCE();
                if (f81-assertionsDisabled || this.ceBuffer.get(this.ceBuffer.length - 1) != Collation.NO_CE) {
                    this.cesIndex = this.ceBuffer.length;
                    if (f81-assertionsDisabled || offsets.size() < this.ceBuffer.length) {
    ...
    private final void appendNumericSegmentCEs(CharSequence digits) {
        ...
        if (!f81-assertionsDisabled && (1 > length || length > 254)) {
            throw new AssertionError();
        } else if (f81-assertionsDisabled || length == 1 || digits.charAt(0) != 0) {
            ...
            if (f81-assertionsDisabled || length >= 7) {
                int pair;
                int pos;
