...
final class CollationFastLatinBuilder {
    ...
    static final /* synthetic */ boolean f90-assertionsDisabled = (CollationFastLatinBuilder.class.desiredAssertionStatus() ^ 1);
    ...
    private boolean inSameGroup(long p, long q) {
        if (p >= this.firstShortPrimary) {
            ...
            if (p > lastVariablePrimary) {
                ...
                if (f90-assertionsDisabled || !(p == 0 || q == 0)) {
                    int i = 0;
                    while (true) {
    ...
    private void getCEs(CollationData data) {
        ...
        while (true) {
            ...
            if (c == 0 && (isContractionCharCE(this.ce0) ^ 1) != 0) {
                if (f90-assertionsDisabled || this.contractionCEs.isEmpty()) {
    ...
    private boolean getCEsFromContractionCE32(CollationData data, int ce32) {
        ...
        if (f90-assertionsDisabled || !Collation.isContractionCE32(ce32)) {
    ...
    private int getMiniCE(long ce) {
        ...
        if (f90-assertionsDisabled || index >= 0) {
    ...
    private void encodeUniqueCEs() {
        ...
        if (f90-assertionsDisabled || (((int) this.uniqueCEs.elementAti(0)) >>> 16) != 0) {
            long prevPrimary = 0;
            int prevSecondary = 0;
            int pri = 0;
            int sec = 0;
            ...
            while (i < this.uniqueCEs.size()) {
                ...
                if (p != prevPrimary) {
                    while (p > lastGroupPrimary) {
                        if (f90-assertionsDisabled || pri <= 4088) {
                ...
                if (f90-assertionsDisabled || (Collation.CASE_MASK & lower32) == 0) {
                    ...
                    if (3072 > pri || pri > 4088) {
                        ...
                        i++;
                    } else if (f90-assertionsDisabled || sec == 160) {
    ...
    private int encodeTwoCEs(long first, long second) {
        ...
        if (f90-assertionsDisabled || (first >>> 32) != 1) {
