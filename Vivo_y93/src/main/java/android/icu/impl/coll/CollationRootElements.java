...
public final class CollationRootElements {
    ...
    static final /* synthetic */ boolean f75-assertionsDisabled = (CollationRootElements.class.desiredAssertionStatus() ^ 1);
    ...
    long lastCEWithPrimaryBefore(long p) {
        ...
        if (f75-assertionsDisabled || p > this.elements[(int) this.elements[2]]) {
            long secTer;
            int index = findP(p);
            ...
            if (p != (PRIMARY_SENTINEL & q)) {
                ...
                if (!(f75-assertionsDisabled || (127 & q) == 0)) {
                    throw new AssertionError();
                }
            } else if (f75-assertionsDisabled || (127 & q) == 0) {
    ...
    long firstCEWithPrimaryAtLeast(long p) {
        ...
        if (p != (this.elements[index] & PRIMARY_SENTINEL)) {
            ...
            if (!(f75-assertionsDisabled || (127 & p) == 0)) {
                throw new AssertionError();
            }
        }
    ...
    long getPrimaryBefore(long p, boolean isCompressible) {
        ...
        if (f75-assertionsDisabled || isEndOfPrimaryRange(nextElement)) {
    ...
    int getSecondaryBefore(long p, int s) {
        ...
        if (f75-assertionsDisabled || s >= sec) {
            while (true) {
                int index2 = index;
                if (s > sec) {
                    previousSec = sec;
                    if (f75-assertionsDisabled || (this.elements[index2] & 128) != 0) {
                        index = index2 + 1;
                        sec = (int) (this.elements[index2] >> 16);
                    } else {
                        throw new AssertionError();
                    }
                } else if (f75-assertionsDisabled || sec == s) {
                    return previousSec;
                } else {
                    throw new AssertionError();
                }
    ...
    int getTertiaryBefore(long p, int s, int t) {
        if (f75-assertionsDisabled || (t & -16192) == 0) {
            int index;
            int previousTer;
            long secTer;
            if (p == 0) {
            ...
            while (true) {
                ...
                } else if (f75-assertionsDisabled || secTer == st) {
    ...
    int findPrimary(long p) {
        Object obj = 1;
        if (f75-assertionsDisabled || (255 & p) == 0) {
            int index = findP(p);
    ...
    int getSecondaryAfter(int index, int s) {
        ...
        } else if (f75-assertionsDisabled || index >= ((int) this.elements[2])) {
    ...
    int getTertiaryAfter(int index, int s, int t) {
        ...
        if (index == 0) {
            ...
            } else if (f75-assertionsDisabled || t != 0) {
                index = (int) this.elements[0];
                terLimit = 16384;
            } else {
                throw new AssertionError();
        ...
        if (f75-assertionsDisabled || (secTer >> 16) == ((long) s)) {
    ...
    private int findP(long p) {
        if (f75-assertionsDisabled || (p >> 24) != 254) {
            int start = (int) this.elements[2];
            if (f75-assertionsDisabled || p >= this.elements[start]) {
                int limit = this.elements.length - 1;
                if (!f75-assertionsDisabled && this.elements[limit] < PRIMARY_SENTINEL) {
                    throw new AssertionError();
                } else if (f75-assertionsDisabled || p < this.elements[limit]) {
                    while (start + 1 < limit) {
