...
public final class CollationWeights {
    ...
    static final /* synthetic */ boolean f92-assertionsDisabled = (CollationWeights.class.desiredAssertionStatus() ^ 1);
    ...
    public long nextWeight() {
        ...
        if (i == 0) {
            ...
            if (!f92-assertionsDisabled && range.start > range.end) {
    ...
    private long incWeight(long weight, int length) {
        while (true) {
            ...
            length--;
            if (!f92-assertionsDisabled && length <= 0) {
                throw new AssertionError();
            }
        }
    }
    ...
    private boolean getWeightRanges(long lowerLimit, long upperLimit) {
        if (!f92-assertionsDisabled && lowerLimit == 0) {
            throw new AssertionError();
        } else if (f92-assertionsDisabled || upperLimit != 0) {
            int lowerLength = lengthOfWeight(lowerLimit);
            int upperLength = lengthOfWeight(upperLimit);
            if (!f92-assertionsDisabled && lowerLength < this.middleLength) {
                throw new AssertionError();
                    ...
                    while (length > this.middleLength) {
                        if (lower[length] != null && upper[length] != null && lower[length].count > 0 && upper[length].count > 0) {
                            ...
                            if (lowerEnd > upperStart) {
                                if (f92-assertionsDisabled || truncateWeight(lowerEnd, length - 1) == truncateWeight(upperStart, length - 1)) {
                                ...
                                if (!f92-assertionsDisabled && this.minBytes[length] >= this.maxBytes[length]) {
                                    throw new AssertionError();
                                }
    ...
    private boolean allocWeightsInMinLengthRanges(int n, int minLength) {
        ...
        if (count2 == 0 || (count2 * nextCountBytes) + count1 < n) {
            count2++;
            count1--;
            if (!f92-assertionsDisabled && (count2 * nextCountBytes) + count1 < n) {
                throw new AssertionError();
            }
        }
