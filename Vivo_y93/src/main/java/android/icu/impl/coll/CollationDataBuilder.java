...
final class CollationDataBuilder {
    ...
    static final /* synthetic */ boolean f86-assertionsDisabled = (CollationDataBuilder.class.desiredAssertionStatus() ^ 1);
    ...
    private static final class CopyHelper {
        /* renamed from: -assertionsDisabled */
        static final /* synthetic */ boolean f87-assertionsDisabled = (CopyHelper.class.desiredAssertionStatus() ^ 1);
        ...
        int copyCE32(int ce32) {
            ...
            if (Collation.isSpecialCE32(ce32)) {
                ...
                if (tag == 5) {
                    ...
                    if (f87-assertionsDisabled || !cond.hasContext()) {
                ...
                } else if (f87-assertionsDisabled || tag == 1 || tag == 2 || tag == 4 || tag == 12) {
                    return ce32;
                } else {
                    throw new AssertionError();
                }
    ...
    private static final class DataBuilderCollationIterator extends CollationIterator {
        /* renamed from: -assertionsDisabled */
        static final /* synthetic */ boolean f88-assertionsDisabled = (DataBuilderCollationIterator.class.desiredAssertionStatus() ^ 1);
        ...
        protected int getCE32FromBuilderData(int ce32) {
            if (!f88-assertionsDisabled && !Collation.hasCE32Tag(ce32, 7)) {
                throw new AssertionError();
            } else if ((ce32 & 256) != 0) {
    ...
    protected int addConditionalCE32(String context, int ce32) {
        if (f86-assertionsDisabled || context.length() != 0) {
    ...
    protected static int encodeOneCEAsCE32(long ce) {
        ...
        if (!f86-assertionsDisabled && (t & Collation.CASE_MASK) == Collation.CASE_MASK) {
            throw new AssertionError();
    ...
    protected int copyContractionsFromBaseCE32(StringBuilder context, int c, int ce32, ConditionalCE32 cond) {
        ...
        if ((ce32 & 256) == 0) {
            ce32 = this.base.getCE32FromContexts(trieIndex);
            if (f86-assertionsDisabled || !Collation.isContractionCE32(ce32)) {
                index = addConditionalCE32(context.toString(), copyFromBaseCE32(c, ce32, true));
                cond.next = index;
                cond = getConditionalCE32(index);
            } else {
        ...
        } else if (f86-assertionsDisabled || context.length() > 1) {
            index = -1;
        } else {
            throw new AssertionError();
        }
        ...
        if (f86-assertionsDisabled || index >= 0) {
            return index;
        }
        throw new AssertionError();
    }
            ...
            if (Collation.isSpecialCE32(ce32)) {
                switch (Collation.tagFromCE32(ce32)) {
                    ...
                    case 14:
                        ...
                        break;
                    case 15:
                        if (f86-assertionsDisabled || fromBase) {
                            ce32 = 192;
                            needToCopyFromBase = true;
                            break;
                        }
    ...
    protected void setDigitTags() {
        ...
        while (iter.next()) {
            if (f86-assertionsDisabled || iter.codepoint != UnicodeSetIterator.IS_STRING) {
                int c = iter.codepoint;
                int ce32 = this.trie.get(c);
                if (!(ce32 == 192 || ce32 == -1)) {
        ...
        if (isMutable()) {
            ...
            if (getJamoCE32s(jamoCE32s)) {
                ...
                while (c < Hangul.HANGUL_LIMIT) {
                    ...
                    if (f86-assertionsDisabled || Collation.hasCE32Tag(ce32, 12)) {
                        limit = c + Hangul.JAMO_VT_COUNT;
                        this.trie.setRange(c, limit - 1, ce32, true);
                        c = limit;
                    } else {
    ...
    protected void clearContexts() {
        ...
        while (iter.next()) {
            if (f86-assertionsDisabled || iter.codepoint != UnicodeSetIterator.IS_STRING) {
                ...
                if (f86-assertionsDisabled || isBuilderContextCE32(ce32)) {
    ...
    protected int buildContext(ConditionalCE32 head) {
        if (!f86-assertionsDisabled && head.hasContext()) {
            throw new AssertionError();
        } else if (f86-assertionsDisabled || head.next >= 0) {
            ...
            while (true) {
                ...
                } else if (f86-assertionsDisabled || firstCond == lastCond) {
                    ce32 = lastCond.ce32;
                    cond = lastCond;
                } else {
                    throw new AssertionError();
                }
                if (f86-assertionsDisabled || cond == lastCond) {
                    ...
                    if (prefixLength != 0) {
                        ...
                        if (cond.next < 0) {
                            if (f86-assertionsDisabled || head.defaultCE32 != 1) {
