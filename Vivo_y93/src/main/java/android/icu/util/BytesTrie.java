...
public final class BytesTrie implements Cloneable, Iterable<Entry> {
    ...
    static final /* synthetic */ boolean f54-assertionsDisabled = (BytesTrie.class.desiredAssertionStatus() ^ 1);
            ...
            if (length < 0) {
                ...
                while (true) {
                    ...
                    if (node < 16) {
                        ...
                        if (!f54-assertionsDisabled && (this.bytes_[pos] & 255) >= 32) {
    ...
    public int getValue() {
        ...
        if (f54-assertionsDisabled || leadByte >= 32) {
    ...
    public int getNextBytes(Appendable out) {
        ...
        if (node >= 32) {
            ...
            pos2 = pos + 1;
            node = this.bytes_[pos] & 255;
            if (!f54-assertionsDisabled && node >= 32) {
                throw new AssertionError();
            }
        }
        if (node < 16) {
