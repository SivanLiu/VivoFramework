...
public class MessageFormat extends UFormat {
    ...
    static final /* synthetic */ boolean f46-assertionsDisabled = (MessageFormat.class.desiredAssertionStatus() ^ 1);
    ...
    private static final class PluralSelectorProvider implements PluralSelector {
        /* renamed from: -assertionsDisabled */
        static final /* synthetic */ boolean f47-assertionsDisabled = (PluralSelectorProvider.class.desiredAssertionStatus() ^ 1);
        ...
        if (source != null) {
            ...
            while (true) {
                ...
                if (len == 0 || msgString.regionMatches(prevIndex, source, sourceOffset, len)) {
                    ...
                    } else if (f46-assertionsDisabled || type == Type.ARG_START) {
                        Object argId;
    ...
    private String getLiteralStringUntilNextArgument(int from) {
        ...
        while (true) {
            ...
            if (type != Type.ARG_START && type != Type.MSG_LIMIT) {
                if (f46-assertionsDisabled || type == Type.SKIP_SYNTAX || type == Type.INSERT_CHAR) {
                    prevIndex = part.getLimit();
                    i++;
                } else {
                    throw new AssertionError("Unexpected Part " + part + " in parsed message.");
    ...
    private static int findChoiceSubMessage(MessagePattern pattern, int partIndex, double number) {
        ...
        do {
            ...
            if (type == Type.ARG_LIMIT) {
                ...
                break;
            } else if (f46-assertionsDisabled || type.hasNumericValue()) {
    ...
    private int findOtherSubMessage(int partIndex) {
        ...
        while (true) {
            ...
            if (type == Type.ARG_LIMIT) {
                partIndex = partIndex2;
                break;
            } else if (f46-assertionsDisabled || type == Type.ARG_SELECTOR) {
