...
public abstract class AsyncQueryHandler extends Handler {
            ...
            if (resolver != null) {
                ...
                switch (msg.arg1) {
                    case 1:
                        ...
                        break;
                    case 2:
                        if (!AsyncQueryHandler.this.checkIsNeedTryCatch(args.uri)) {
                        ...
                        try {
                            ...
                            args.result = args.uri;
                            Log.w(AsyncQueryHandler.TAG, "#Exception thrown during handling EVENT_ARG_INSERT; To avoid system carsh, catch it.", e2);
                            break;
                        }
                    case 3:
                        if (!AsyncQueryHandler.this.checkIsNeedTryCatch(args.uri)) {
                        ...
                        try {
                            ...
                            args.result = Integer.valueOf(-1);
                            Log.w(AsyncQueryHandler.TAG, "#Exception thrown during handling EVENT_ARG_UPDATE; To avoid process carsh, catch it.", e22);
                            break;
                        }
                    case 4:
    ...
    private boolean checkIsNeedTryCatch(Uri uri) {
        boolean shouldCatch = false;
        if (uri != null) {
            try {
                String uriPath = uri.getPath();
                String uriAuthority = uri.getAuthority();
                if (uriPath == null || uriAuthority == null) {
                    return false;
                }
                int callingUid = Binder.getCallingUid();
                Log.i(TAG, "check path:" + uriPath + " authority:" + uriAuthority + " callingUid:" + callingUid);
                if (1000 == callingUid) {
                    shouldCatch = true;
                }
            } catch (Exception e) {
