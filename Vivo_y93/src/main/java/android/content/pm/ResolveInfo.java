...
public class ResolveInfo implements Parcelable {
    ...
    public CharSequence loadLabel(PackageManager pm) {
        ...
        if (DoubleAppSwitcher.sEnabled) {
            if (this.mUm == null) {
                this.mUm = Stub.asInterface(ServiceManager.getService(Context.USER_SERVICE));
                try {
                    this.mDoubleAppUseId = this.mUm.getDoubleAppUserId();
                } catch (RemoteException e) {
                }
            }
            label = loadLabelImpl(pm);
            if (label == null || this.targetUserId != this.mDoubleAppUseId) {
                return label;
            }
            return pm.getUserBadgedLabel(label, new UserHandle(this.targetUserId));
        } else if (this.nonLocalizedLabel != null) {
    ...
    private CharSequence loadLabelImpl(PackageManager pm) {
    ...
    public Drawable loadIcon(PackageManager pm) {
        if (DoubleAppSwitcher.sEnabled && this.mUm == null) {
            this.mUm = Stub.asInterface(ServiceManager.getService(Context.USER_SERVICE));
            try {
                this.mDoubleAppUseId = this.mUm.getDoubleAppUserId();
            } catch (RemoteException e) {
        ...
        if (!DoubleAppSwitcher.sEnabled || this.targetUserId == -2) {
            return pm.getUserBadgedIcon(dr, new UserHandle(UserHandle.myUserId()));
        }
        return pm.getUserBadgedIcon(dr, new UserHandle(this.targetUserId));
    }
