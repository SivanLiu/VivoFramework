...
public abstract class ContentProvider implements ComponentCallbacks2 {
    ...
    private static HashSet<String> sInternalPkgs = new HashSet();
    ...
    class Transport extends ContentProviderNative {
        ...
        private int enforceReadPermission(String callingPkg, Uri uri, IBinder callerToken) throws SecurityException {
            ...
            if (this.mReadOp != -1) {
                int result = this.mAppOpsManager.noteProxyOp(this.mReadOp, callingPkg);
                if (result != 0) {
                    return result;
                }
            }
            ...
            if (!componentPerm.equals(permission.READ_SMS)) {
                return !VivoPermissionManager.checkCallingVivoPermission(componentPerm) ? 1 : 0;
            } else {
                String authority = uri.getAuthority();
                if ("sms".equals(authority)) {
                    if (!VivoPermissionManager.checkCallingVivoPermission(permission.READ_SMS)) {
                        return 1;
                    }
                ...
                } else if ("mms-sms".equals(authority)) {
                    String uriStr = uri.toString();
                    if ((uriStr == null || !uriStr.contains("content://mms-sms/threadID")) && !VivoPermissionManager.checkCallingVivoPermission(permission.READ_SMS)) {
                        return 1;
                    }
                }
                return 0;
        ...
        private int enforceWritePermission(String callingPkg, Uri uri, IBinder callerToken) throws SecurityException {
            ...
            boolean isWriteSmsIgnored = false;
            if (this.mWriteOp != -1) {
                if (this.mWriteOp == 15 && ContentProvider.isVivoInternalPackage(callingPkg)) {
                    return 0;
                }
                int result = this.mAppOpsManager.noteProxyOp(this.mWriteOp, callingPkg);
                if (result != 0) {
                    if (this.mWriteOp != 15) {
                        return result;
                    }
                    isWriteSmsIgnored = true;
                }
            }
            String componentPerm = ContentProvider.this.getWritePermission();
            if (componentPerm == null) {
                return isWriteSmsIgnored ? 1 : 0;
            } else {
                if (!componentPerm.equals(permission.WRITE_SMS)) {
                    return !VivoPermissionManager.checkCallingVivoPermission(componentPerm) ? 1 : 0;
                } else {
                    String authority = uri.getAuthority();
                    if ("sms".equals(authority)) {
                        if (!VivoPermissionManager.checkCallingVivoPermission(permission.WRITE_SMS)) {
                            return 1;
                        }
                    } else if ("mms".equals(authority)) {
                        if (!VivoPermissionManager.checkCallingVivoPermission(permission.WRITE_SMS)) {
                        ...
                        String uriStr = uri.toString();
                        if (uriStr != null && uriStr.contains("content://mms-sms/threadID")) {
                            return 0;
                        }
                        if (!VivoPermissionManager.checkCallingVivoPermission(permission.WRITE_SMS)) {
                            return 1;
    ...
    static {
        sInternalPkgs.add("com.android.mms");
        sInternalPkgs.add("com.android.contacts");
        sInternalPkgs.add("com.android.mms.service");
        sInternalPkgs.add("com.android.phone");
        sInternalPkgs.add("com.iqoo.secure");
        sInternalPkgs.add("com.bbk.PCTools");
        sInternalPkgs.add("com.vivo.PCTools");
        sInternalPkgs.add("com.bbk.cloud");
        sInternalPkgs.add("com.mediatek.backuprestore");
        sInternalPkgs.add("com.cutemms.cuteimageanim");
        sInternalPkgs.add("com.vivo.daemonService");
        sInternalPkgs.add("com.vivo.motionrecognition");
        sInternalPkgs.add("com.vivo.ewarranty");
        sInternalPkgs.add("com.vivo.floatingwindow");
        sInternalPkgs.add("com.android.bluetooth");
        sInternalPkgs.add("com.vivo.lan");
        sInternalPkgs.add("com.vivo.easyshare");
        sInternalPkgs.add("com.android.providers.contacts");
    }

    @VivoHook(hookType = VivoHookType.NEW_METHOD)
    private static boolean isVivoInternalPackage(String pkg) {
        return sInternalPkgs.contains(pkg);
    }
