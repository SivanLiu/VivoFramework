...
public class Linkify {
    ...
    public static final boolean addLinks(Spannable text, int mask, boolean isVigour) {
        if (isVigour) {
            return addVigourLinks(text, mask);
        }
        return addLinks(text, mask);
    }

    public static final boolean addVigourLinks(Spannable text, int mask) {
        if (mask == 0) {
            return false;
        }
        Spannable spannable;
        ...
        if ((mask & 1) != 0) {
            spannable = text;
            gatherLinks(links, spannable, Patterns.VIGOUR_WEB_URL, new String[]{"http://", "https://", "rtsp://"}, sUrlMatchFilter, null);
        }
        if ((mask & 2) != 0) {
            gatherLinks(links, text, Patterns.EMAIL_ADDRESS, new String[]{WebView.SCHEME_MAILTO}, null, null);
        }
        if ((mask & 4) != 0) {
            int cur_size = links.size();
            spannable = text;
            gatherLinks(links, spannable, Patterns.VIGOUR_PHONE_CN, new String[]{WebView.SCHEME_TEL}, sPhoneNumberMatchFilter, sPhoneNumberTransformFilter);
            spannable = text;
            gatherLinks(links, spannable, Patterns.VIGOUR_PHONE, new String[]{WebView.SCHEME_TEL}, sPhoneNumberMatchFilter, sPhoneNumberTransformFilter);
        }
        if ((mask & 8) != 0) {
            gatherMapLinks(links, text);
        }
