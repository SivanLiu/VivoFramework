...
class TextLine {
    ...
    float[] measureAllOffsets(boolean[] trailing, FontMetricsInt fmi) {
        ...
        if (!this.mHasTabs) {
            if (this.mDirections == Layout.DIRS_ALL_LEFT_TO_RIGHT) {
                for (offset = 0; offset <= this.mLen; offset++) {
                    measurement[offset] = measureRun(0, offset, this.mLen, false, fmi);
                }
                return measurement;
            } else if (this.mDirections == Layout.DIRS_ALL_RIGHT_TO_LEFT) {
                for (offset = 0; offset <= this.mLen; offset++) {
                    measurement[offset] = measureRun(0, offset, this.mLen, true, fmi);
                }
                return measurement;
            }
        }
        ...
        for (int i2 = 0; i2 < runs.length; i2 += 2) {
            ...
            int runLimit = runStart + (runs[i2 + 1] & 67108863);
            if (runLimit > this.mLen) {
                runLimit = this.mLen;
            }
            ...
            while (j <= runLimit) {
                ...
                if (j == runLimit || codept == 9) {
                    ...
                    offset = segstart;
                    while (offset <= j && offset <= this.mLen) {
                        if (target[offset] >= segstart && target[offset] < j) {
                            measurement[offset] = measureRun(segstart, offset, j, runIsRtl, crtfmi) + baseh;
                        }
                        offset++;
                    }
                    if (codept == 9) {
