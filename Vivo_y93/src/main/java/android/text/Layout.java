...
public abstract class Layout {
    ...
    private class HorizontalMeasurementProvider {
        ...
        float get(int offset, int isStart) {
            if (this.mHorizontals == null) {
                return Layout.this.getHorizontal(offset, this.mPrimary, isStart);
            }
    ...
    private static /* synthetic */ int[] m30-getandroid-text-Layout$AlignmentSwitchesValues() {
    ...
    private boolean[] primaryIsTrailingPreviousAllLineOffsets(int line) {
        ...
        for (i = 0; i < runs.length; i += 2) {
            int limit = (lineStart + runs[i]) + (runs[i + 1] & RUN_LENGTH_MASK);
            if (limit > lineEnd) {
                limit = lineEnd;
            }
    ...
    public float getPrimaryHorizontal(int offset, int start) {
        return getPrimaryHorizontal(offset, false, start);
    }

    public float getPrimaryHorizontal(int offset, boolean clamped, int start) {
        return getHorizontal(offset, primaryIsTrailingPrevious(offset), clamped, start == 1);
    }

    public float getPrimaryHorizontal(int offset, boolean clamped) {
        return getPrimaryHorizontal(offset, clamped, 1);
    }
    ...
    private float getHorizontal(int offset, boolean primary) {
        return getHorizontal(offset, primary, 1);
    }

    private float getHorizontal(int offset, boolean primary, int start) {
        return primary ? getPrimaryHorizontal(offset, start) : getSecondaryHorizontal(offset);
    }

    private float getHorizontal(int offset, boolean trailing, boolean clamped) {
        return getHorizontal(offset, trailing, clamped, true);
    }

    private float getHorizontal(int offset, boolean trailing, boolean clamped, boolean isStart) {
        return getHorizontal(offset, trailing, getLineForOffset(offset, isStart), clamped);
    }
    ...
    private float[] getLineHorizontals(int line, boolean clamped, boolean primary) {
        ...
        if (!primary) {
            for (offset = 0; offset < trailings.length; offset++) {
        ...
        TextLine.recycle(tl);
        if (clamped) {
            for (offset = 0; offset <= wid.length; offset++) {
    ...
    public int getLineForOffset(int offset, boolean isStart) {
        int high = getLineCount();
        int low = -1;
        while (high - low > 1) {
            int guess = (high + low) / 2;
            ...
            } else if (getLineStart(guess) >= offset) {
                high = guess;
            } else {
                low = guess;
            }
    ...
    public int getOffsetForHorizontal(int line, float horiz, int start) {
        return getOffsetForHorizontal(line, horiz, true, start);
    }

    public int getOffsetForHorizontal(int line, float horiz, boolean primary) {
        return getOffsetForHorizontal(line, horiz, primary, 1);
    }

    public int getOffsetForHorizontal(int line, float horiz, boolean primary, int isStart) {
        int max;
        float dist;
        ...
        HorizontalMeasurementProvider horizontalMeasurementProvider = new HorizontalMeasurementProvider(line, primary);
        if (line == getLineCount() - 1 || isStart == 0) {
            max = lineEndOffset;
        } else {
            max = tl.getOffsetToLeftRightOf(lineEndOffset - lineStartOffset, isRtlCharAt(lineEndOffset - 1) ^ 1) + lineStartOffset;
        }
        int best = lineStartOffset;
        float bestdist = Math.abs(horizontalMeasurementProvider.get(lineStartOffset) - horiz);
        for (int i = 0; i < dirs.mDirections.length; i += 2) {
            ...
            while (high - low > 1) {
                int guess = (high + low) / 2;
                if (horizontalMeasurementProvider.get(getOffsetAtStartOf(guess)) * ((float) swap) >= ((float) swap) * horiz) {
                    high = guess;
                } else {
                    low = guess;
                }
            ...
            if (low < there) {
                ...
                if (low >= here && low < there) {
                    dist = Math.abs(horizontalMeasurementProvider.get(low) - horiz);
                    if (aft < there) {
                        float other = Math.abs(horizontalMeasurementProvider.get(aft) - horiz);
                        if (other < dist) {
                            dist = other;
                            low = aft;
                        }
            ...
            dist = Math.abs(horizontalMeasurementProvider.get(here) - horiz);
            if (dist < bestdist) {
                bestdist = dist;
                best = here;
            }
        }
        dist = Math.abs(horizontalMeasurementProvider.get(max, isStart) - horiz);
        if (dist <= bestdist) {
            bestdist = dist;
            best = max;
        }
    ...
    public boolean shouldClampCursor(int line) {
        boolean z = true;
        switch (m30-getandroid-text-Layout$AlignmentSwitchesValues()[getParagraphAlignment(line).ordinal()]) {
            case 1:
                return true;
            case 2:
