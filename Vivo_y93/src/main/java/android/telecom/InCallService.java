...
public abstract class InCallService extends Service {
    ...
    private String mCallIdOfMsgQueue = null;
    ...
    private final class InCallServiceBinder extends Stub {
        ...
        public void updateCall(ParcelableCall call) {
            if (call != null) {
                if (Process.myUid() == 1001) {
                    String newId = call.getId();
                    if (!InCallService.this.mHandler.hasMessages(3) && (call.isVideoCallProviderChanged() ^ 1) != 0) {
                        InCallService.this.mCallIdOfMsgQueue = newId;
                    } else if (TextUtils.equals(InCallService.this.mCallIdOfMsgQueue, newId)) {
                        InCallService.this.mHandler.removeMessages(3);
                        Log.d("InCallService", "remove duplicate msgs");
                        if (call.isVideoCallProviderChanged()) {
                            InCallService.this.mCallIdOfMsgQueue = null;
                        }
                    } else {
                        InCallService.this.mCallIdOfMsgQueue = null;
                    }
        ...
        public void bringToForeground(boolean showDialpad) {
            int i;
            Handler -get1 = InCallService.this.mHandler;
            if (showDialpad) {
                i = 1;
            } else {
                i = 0;
            }
            -get1.obtainMessage(6, i, 0).sendToTarget();
        }
        ...
        public void onCanAddCallChanged(boolean canAddCall) {
            ...
            -get1.obtainMessage(7, i, 0).sendToTarget();
        }

        @VivoHook(hookType = VivoHookType.PUBLIC_API_METHOD)
        public VivoTelephonyApiParams vivoTelephonyApi(VivoTelephonyApiParams v) {
            return InCallService.this.vivoTelephonyApi(v);
        }

        @VivoHook(hookType = VivoHookType.PUBLIC_API_METHOD)
        public VivoTelephonyApiParams registerVivoApiCallback(VivoTelephonyApiParams v, IVivoTelephonyApiCallback callback) {
            return InCallService.this.registerVivoApiCallback(v, callback);
        }

        @VivoHook(hookType = VivoHookType.PUBLIC_API_METHOD)
        public VivoTelephonyApiParams unregisterVivoApiCallback(VivoTelephonyApiParams v, IVivoTelephonyApiCallback callback) {
            return InCallService.this.unregisterVivoApiCallback(v, callback);
        }
