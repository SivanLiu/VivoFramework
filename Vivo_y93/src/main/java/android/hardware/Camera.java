...
public class Camera {
    ...
    private static final int CAMERA_MSG_PPDATA_FRAME = 16384;
    ...
    private boolean finalizeCamera = false;
    ...
    private boolean mFilterShotRunning = false;
    private boolean mFilterVideoRunning = false;
    ...
    private PPDataCallback mPPDataCallback;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 1:
                    ...
                    if ((msg.arg1 == 100 || msg.arg1 == 1 || msg.arg1 == 2) && Camera.this.mCameraId < 2) {
                        Camera.this.sendReleaseCameraBroadCast();
                    }
                    return;
                case 2:
                ...
                case 8192:
                    ...
                    return;
                case 16384:
                    PPDataCallback ppCb = Camera.this.mPPDataCallback;
                    if (ppCb != null) {
                        ppCb.OnPPDataFrame((FileDescriptor) msg.obj, this.mCamera);
                    }
                    return;
                default:
    ...
    @Deprecated
    public interface PPDataCallback {
        void OnPPDataFrame(FileDescriptor fileDescriptor, Camera camera);
    }
    ...
    private final native void _startFilterShot();

    private final native void _startFilterVideo();

    private final native void _startPreview();

    private final native void _stopFaceDetection();

    private final native void _stopFilterShot();

    private final native void _stopFilterVideo();
    ...
    public static int getNumberOfCameras() {
        ...
        String packageList = SystemProperties.get("vendor.camera.aux.packagelist") + ",com.android.camera,com.vivo.bsptest,com.iqoo.engineermode";
        ...
        if (this.mEventHandler != null) {
            this.mEventHandler.post(new Runnable() {
                public void run() {
                    ...
                    Log.d(Camera.TAG, "showToast " + msg);
    ...
    private int cameraInitVersion(int cameraId, int halVersion) {
        if (VivoPermissionManager.checkCallingVivoPermission(permission.CAMERA)) {
        ...
        int CheckResult = JudgeSettingAndShowMessage();
        if (CheckResult != 0) {
            return CheckResult;
        }
        String packageName = ActivityThread.currentOpPackageName();
    ...
    private void sendReleaseCameraBroadCast() {
        String ACTION_CLOSE_CAMERA = "com.android.camera.ACTION_CLOSE_CAMERA";
        ActivityThread caller = ActivityThread.currentActivityThread();
        Context context = caller != null ? caller.getApplication() : null;
        Intent intent = new Intent(ACTION_CLOSE_CAMERA);
        String packageName = ActivityThread.currentPackageName();
        Log.v(TAG, "sendReleaseCameraBroadCast isNormalCamera = " + this.mIsNormalCamera);
        intent.putExtra("isNormalCamera", this.mIsNormalCamera);
        intent.putExtra(NetworkScoreManager.EXTRA_PACKAGE_NAME, packageName);
        if (context != null) {
            context.sendBroadcast(intent);
        }
        String closeact = "com.android.camera.WHITELIST_CLOSE";
        if (context != null) {
            String sendLists = Global.getString(context.getContentResolver(), "flt_white_pkg_list");
            if (sendLists != null && packageName != null && sendLists.contains(packageName)) {
                Intent intent2 = new Intent(closeact);
                intent2.putExtra("pkg", packageName);
                intent2.putExtra(Instrumentation.REPORT_KEY_IDENTIFIER, this.mCameraId);
                Log.v(TAG, "send WHITELIST_CLOSE packageName = " + packageName + " id=" + this.mCameraId);
    ...
    private void sendStartPreviewBroadCast() {
        ...
        String packageName = ActivityThread.currentPackageName();
        String openact = "com.android.camera.WHITELIST_STARTPREVIEW";
        if (context != null) {
            String sendLists = Global.getString(context.getContentResolver(), "flt_white_pkg_list");
            if (sendLists != null && packageName != null && sendLists.contains(packageName)) {
                Size preSize = getParameters().getPreviewSize();
                Log.v(TAG, "sendStartPreviewBroadCast w: " + preSize.width + ",h: " + preSize.height);
                Intent intent2 = new Intent(openact);
                intent2.putExtra("pkg", packageName);
                intent2.putExtra(Instrumentation.REPORT_KEY_IDENTIFIER, this.mCameraId);
                intent2.putExtra("prewidth", preSize.width);
                intent2.putExtra("preheight", preSize.height);
                Log.v(TAG, "sendStartPreviewBroadCast packageName = " + packageName + " id=" + this.mCameraId);
                context.sendBroadcast(intent2);
            }
        }
    }
    ...
    protected void finalize() {
        Log.v(TAG, "finalize camera");
        this.finalizeCamera = true;
        release();
    }

    public final void release() {
        if (this.finalizeCamera) {
            this.finalizeCamera = false;
        } else if (this.mCameraId < 2) {
            sendReleaseCameraBroadCast();
        }
        native_release();
        this.mFaceDetectionRunning = false;
        this.mFilterShotRunning = false;
        this.mFilterVideoRunning = false;
    }
    ...
    public final void setPPDataCallback(PPDataCallback cb) {
        Log.v(TAG, "setPPDataCallback");
        this.mPPDataCallback = cb;
    }
    ...
    private static void PostPPEventFromNative(Object camera_ref, int what, int arg1, int arg2, Object obj) {
        Log.v(TAG, "PostPPEventFromNative E " + what);
        Camera c = (Camera) ((WeakReference) camera_ref).get();
        if (c == null) {
            Log.e(TAG, "PostPPEventFromNative c == null");
            return;
        }
        if (c.mEventHandler != null) {
            c.mEventHandler.sendMessage(c.mEventHandler.obtainMessage(what, arg1, arg2, obj));
        } else {
            Log.e(TAG, "PostPPEventFromNative c.mEventHandler == null");
        }
        Log.v(TAG, "PostPPEventFromNative X " + what);
    }
    ...
    public final void startFilterShot() {
        if (this.mFilterShotRunning) {
            throw new RuntimeException("HAL filter shot is already running");
        }
        _startFilterShot();
        this.mFilterShotRunning = true;
    }

    public final void stopFilterShot() {
        _stopFilterShot();
        this.mFilterShotRunning = false;
    }

    public final void startFilterVideo() {
        if (this.mFilterVideoRunning) {
            throw new RuntimeException("HAL filter video is already running");
        }
        _startFilterVideo();
        this.mFilterVideoRunning = true;
    }

    public final void stopFilterVideo() {
        _stopFilterVideo();
        this.mFilterVideoRunning = false;
    }
