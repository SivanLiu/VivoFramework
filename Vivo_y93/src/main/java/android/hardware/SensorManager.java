...
public abstract class SensorManager {
    ...
    public Sensor getDefaultSensor(int type) {
        ...
        if (type == Sensor.SENSOR_TYPE_AMBIENT_LIGHT_SCENE || type == 8 || type == 17 || type == 22 || type == 23 || type == 24 || type == 25 || type == 26 || type == 32 || type == Sensor.TYPE_RAISEUP_DETECT || type == Sensor.TYPE_VIVOMOTION_DETECT || type == Sensor.TYPE_AMD || type == Sensor.TYPE_DROP_DET || type == Sensor.TYPE_SMART_SCRN) {
            wakeUpSensor = true;
        }
        for (Sensor sensor : l) {
    ...
    public void notifyScreenBrightness(SensorAdditionalInfo parameter) {
        setOperationParameterImpl(parameter);
    }
