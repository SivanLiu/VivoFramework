...
public class SystemSensorManager extends SensorManager {
    ...
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    private static boolean debug = (IS_LOG_CTRL_OPEN | IS_ENG);
    ...
    private static abstract class BaseEventQueue {
        ...
        private int enableSensor(Sensor sensor, int rateUs, int maxBatchReportLatencyUs) {
            if (this.nSensorEventQueue == 0) {
                ...
                if (sensor.getType() == 1 && rateUs > 20000) {
                    boolean isQQShakeFlag;
                    boolean isMicroMsgFlag;
                    if (this.mContextImpl != null) {
                        isQQShakeFlag = isQQShake();
                        isMicroMsgFlag = isMicroMsg();
                    } else {
                        isQQShakeFlag = false;
                        isMicroMsgFlag = false;
                        Log.d("SensorManager", "isShakeFlag judge mContextImpl is null.");
                    }
                    if (isQQShakeFlag || isMicroMsgFlag) {
                        Log.d("SensorManager", "rateUs: " + rateUs + " isShakeFlag: true");
                        rateUs = 20000;
                    }
                }
        ...
        public Context getContext(Context mContext) {
            this.mContextImpl = mContext;
            return this.mContextImpl;
        }

        private boolean isQQShake() {
            int callerPid = Binder.getCallingPid();
            String callerName = this.mContextImpl.getPackageName();
            Log.d("SensorManager", "isQQShake: package is = " + callerName);
            if (callerName == null || !callerName.equals("com.tencent.mobileqq")) {
                return false;
            }
            return true;
        }

        private boolean isMicroMsg() {
            String callerName = this.mContextImpl.getPackageName();
            Log.d("SensorManager", "isMicroMsg: package is = " + callerName);
            if (callerName == null || !callerName.equals("com.tencent.mm")) {
                return false;
            }
            return true;
        }
    ...
    static final class SensorEventQueue extends BaseEventQueue {
        ...
        @VivoHook(hookType = VivoHookType.NEW_FIELD)
        private float mTempProximityActive = -1.0f;
            ...
            if (sensor != null) {
                ...
                if (t != null) {
                    ...
                    switch (t.sensor.getType()) {
                        case 8:
                            if (this.mTempProximityActive != t.values[0]) {
                                if (SystemSensorManager.debug) {
                                    Log.d("SensorManager", "framework alsps raw_data dataX=" + t.values[0] + " dataY=" + t.values[1] + " dataZ=" + t.values[2] + " to " + this.mListener.toString());
                                } else if (this.mTempProximityActive != t.values[0]) {
                                    Log.d("SensorManager", "framework alsps raw_data dataX=" + t.values[0] + " dataY=" + t.values[1] + " dataZ=" + t.values[2]);
                                }
                                this.mTempProximityActive = t.values[0];
                                break;
                            }
                            return;
                        case 19:
                            Log.d("SensorManager", "framework step counter " + t.values[0]);
                            break;
                    }
    ...
    protected boolean registerListenerImpl(SensorEventListener listener, Sensor sensor, int delayUs, Handler handler, int maxBatchReportLatencyUs, int reservedFlags) {
        ...
        if (sensor.getType() == 8 && packageName == null) {
            try {
        ...
        if (debug) {
            Log.d("SensorManager", "registerListenerImpl type:" + sensor.getType() + " strType=" + sensor.getStringType() + " packageName=" + packageName);
        }
        if (sensor.getReportingMode() == 2) {
            ...
            synchronized (this.mSensorListeners) {
                ...
                if (queue == null) {
                    ...
                    queue.getContext(this.mContext);
                    if (queue.addSensor(sensor, delayUs, maxBatchReportLatencyUs)) {
                        this.mSensorListeners.put(listener, queue);
                        if (debug) {
                            Log.d("SensorManager", "registerListener by " + packageName + " [" + listener.toString() + "]");
                        }
                        return true;
                    }
                    queue.dispose();
    ...
    protected void unregisterListenerImpl(SensorEventListener listener, Sensor sensor) {
        ...
        if (sensor == null || sensor.getReportingMode() != 2) {
            if (debug) {
                Log.d("SensorManager", "unregisterListenerImpl mMemsSensorCollection=NULL listener=" + (listener == null ? WifiEnterpriseConfig.EMPTY_VALUE : listener.toString()) + (sensor == null ? "sensor=NULL" : "sensor=" + sensor.getStringType()));
            }
            synchronized (this.mSensorListeners) {
                SensorEventQueue queue = (SensorEventQueue) this.mSensorListeners.get(listener);
                if (queue != null) {
