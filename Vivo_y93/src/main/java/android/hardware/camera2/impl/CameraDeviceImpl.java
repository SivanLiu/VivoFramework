...
public class CameraDeviceImpl extends CameraDevice implements DeathRecipient {
    ...
    public Builder createReprocessCaptureRequest(int templateType) throws CameraAccessException {
        Builder builder;
        synchronized (this.mInterfaceLock) {
            checkIfCameraClosedOrInError();
            CameraMetadataNative templatedRequest = this.mRemoteDevice.createDefaultRequest(templateType);
            ...
            builder = new Builder(templatedRequest, true, this.mNextSessionId - 1);
        }
        return builder;
    }
    ...
    private void sendReleaseCameraBroadCast() {
        ...
        Log.v(this.TAG, "sendReleaseCameraBroadCast isNormalCamera = " + this.mIsNormalCamera);
        ...
        if (context != null) {
            ...
            if (sendLists != null && packageName != null && sendLists.contains(packageName)) {
                ...
                Log.v(this.TAG, "send WHITELIST_CLOSE packageName = " + packageName + " id=" + this.mCameraId);
    ...
    private boolean isNormalCamera(String packageName) {
        Log.v(this.TAG, "isNormalCamera packageName = " + packageName);
        if (isTopActivity()) {
            Log.v(this.TAG, "isNormalCamera isTopActivity true");
            return true;
        }
        Log.v(this.TAG, "current package isTopActivity false ");
        return false;
    }

    public final boolean isBackgroundProcess() {
        Log.v(this.TAG, "isBackgroundProcess");
        ActivityThread caller = ActivityThread.currentActivityThread();
        Context context = caller != null ? caller.getApplication() : null;
        if (context != null) {
            for (RunningAppProcessInfo appProcess : ((ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE)).getRunningAppProcesses()) {
                if (appProcess.processName.equals(context.getPackageName())) {
                    Log.v(this.TAG, "isBackgroundProcess importance " + appProcess.importance);
                    if (appProcess.importance == 400 || appProcess.importance == 300) {
                        Log.v(this.TAG, "appProcess.processName " + appProcess.processName + " true");
                        return true;
                    }
                    Log.v(this.TAG, "appProcess.processName " + appProcess.processName + " false");
                    return false;
                }
            }
        }
        ...
        if (context != null) {
            ...
            if (activityManager != null) {
                ...
                if (runningTaskInfos != null && runningTaskInfos.size() > 0) {
                    ...
                    Log.i(this.TAG, "isTopActivity: pkgname: " + cmpNameTemp);
    ...
    private boolean checkPrivilegedAppList() {
        ...
        String packageList = SystemProperties.get("vendor.camera.privapp.list");
    ...
    public boolean setIsNormalCamera(String isNormalCamera) {
        this.mIsNormalCamera = isNormalCamera;
        Log.v(this.TAG, "mIsNormalCamera " + this.mIsNormalCamera + " isNormalCamera " + isNormalCamera);
        return true;
    }
