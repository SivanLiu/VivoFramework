...
public class MtpDatabase implements AutoCloseable {
    ...
    private Cursor createObjectQuery(int storageID, int format, int parent) throws RemoteException {
        ...
        if (where == null) {
            where = "_data != '/storage/emulated/0/bbklog' AND _data != '/storage/sdcard1/bbklog'";
        } else {
            where = where + " AND " + "_data != '/storage/emulated/0/bbklog'" + " AND " + "_data != '/storage/sdcard1/bbklog'";
        }
    ...
    private int renameFile(int handle, String newName) {
        ...
        try {
            ...
            if (oldFile.renameTo(newFile)) {
                ...
                int format = isDirectory ? 12289 : 12288;
                Log.d(TAG, "renameFile from " + path + " to " + newPath + ", handle = " + handle + ", isDirectory = " + isDirectory);
                this.mMediaScanner.scanMtpFile(newPath, handle, format);
                if (isDirectory) {
                    this.mContext.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(newFile)));
                }
    ...
    private int getDeviceProperty(int property, long[] outIntValue, char[] outStringValue) {
        ...
        boolean mMtpDisabled = (keyguardManager.isKeyguardLocked() && keyguardManager.isKeyguardSecure()) ? true : (UserHandle.myUserId() == ActivityManager.getCurrentUser()) ^ 1;
        String path = this.mContext.getResources().getString(51249537);
        switch (property) {
    ...
    private int[] getObjectReferences(int handle) {
        Cursor cursor = null;
        try {
            cursor = this.mMediaProvider.query(Files.getMtpReferencesUri(this.mVolumeName, (long) handle), ID_PROJECTION, null, null, null, null);
            if (cursor == null) {
                if (cursor != null) {
                    cursor.close();
                }
