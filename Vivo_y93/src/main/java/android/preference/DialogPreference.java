...
public abstract class DialogPreference extends Preference implements OnClickListener, OnDismissListener, OnActivityDestroyListener {
    ...
    protected void showDialog(Bundle state) {
        ...
        boolean isVigourTheme = VivoThemeUtil.isVigourTheme(getContext());
        boolean isSlideStyle = (this instanceof ListPreference) && FtBuild.getRomVersion() >= 3.0f;
        boolean isEditTextPreferenceStyle = (this instanceof EditTextPreference) && FtBuild.getRomVersion() >= 4.0f;
        if (isVigourTheme && isEditTextPreferenceStyle) {
            this.mBuilder = new Builder(context, 51315051);
        } else if (isVigourTheme && isSlideStyle) {
            this.mBuilder = new Builder(context, VivoThemeUtil.getSystemThemeStyle(ThemeType.DIALOG_SLIDE));
        } else {
            this.mBuilder = new Builder(context);
        }
        this.mBuilder.setTitle(this.mDialogTitle).setIcon(this.mDialogIcon).setPositiveButton(this.mPositiveButtonText, (OnClickListener) this).setNegativeButton(this.mNegativeButtonText, (OnClickListener) this);
