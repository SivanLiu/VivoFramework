...
public class Spinner extends AbsSpinner implements OnClickListener {
    ...
    private class DialogPopup implements SpinnerPopup, OnClickListener {
        ...
        public void show(int textDirection, int textAlignment) {
            if (this.mListAdapter != null) {
                Builder builder;
                boolean isSlideStyle = VivoThemeUtil.isVigourTheme(Spinner.this.getContext()) ? FtBuild.getRomVersion() >= 3.0f : false;
                if (isSlideStyle) {
                    builder = new Builder(Spinner.this.getPopupContext(), VivoThemeUtil.getSystemThemeStyle(ThemeType.DIALOG_SLIDE));
                    builder.setCancelable(true);
                } else {
                    builder = new Builder(Spinner.this.getPopupContext());
                }
                ...
                if (isSlideStyle) {
                    LayoutParams lp = this.mPopup.getWindow().getAttributes();
                    lp.gravity = 80;
                    lp.width = -1;
                    lp.height = -2;
                    this.mPopup.getWindow().setAttributes(lp);
                    this.mPopup.setCanceledOnTouchOutside(true);
                }
                ListView listView = this.mPopup.getListView();
                listView.setTextDirection(textDirection);
                listView.setTextAlignment(textAlignment);
                if (VivoThemeUtil.isVigourTheme(Spinner.this.getContext())) {
                    this.mPopup.setIconAttribute(R.attr.alertDialogIcon);
                }
                this.mPopup.show();
            }
        }
    ...
    public void setAdapter(SpinnerAdapter adapter) {
        ...
        if (this.mContext.getApplicationInfo().targetSdkVersion < 21 || adapter == null || adapter.getViewTypeCount() == 1) {
            ...
            if (!VivoThemeUtil.isVigourTheme(getContext()) || FtBuild.getRomVersion() < 3.0f) {
                this.mPopup.setAdapter(new DropDownAdapter(adapter, popupContext.getTheme()));
            } else {
                this.mPopup.setAdapter(new DropDownAdapter(adapter, new ContextThemeWrapper(popupContext, VivoThemeUtil.getSystemThemeStyle(ThemeType.DIALOG_SLIDE)).getTheme()));
            }
            return;
        }
