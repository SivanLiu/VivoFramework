...
public class HorizontalScrollView extends FrameLayout {
    ...
    public HorizontalScrollView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (VivoThemeUtil.isVigourTheme(getContext())) {
            Log.d("horizontalscroll", "vivo style!!!");
                ...
                if (activePointerIndex != -1) {
                    ...
                    if (this.mIsBeingDragged) {
                        ...
                        if (this.mEnableSpringEffect) {
                            reviseOverScrollByTouch = reviseOverScrollByTouch(deltaX, oldX, range);
                        } else {
                            reviseOverScrollByTouch = deltaX;
                        }
                        if (overScrollBy(reviseOverScrollByTouch, 0, this.mScrollX, 0, range, 0, this.mOverscrollDistance, 0, true)) {
                            this.mVelocityTracker.clear();
                        }
                        if (!(!canOverscroll || (this.mEnableSpringEffect ^ 1) == 0 || this.mEdgeGlowLeft == null)) {
    ...
    public void computeScroll() {
        if (this.mScroller.computeScrollOffset()) {
            ...
            if (!(oldX == x && oldY == y)) {
                ...
                int overscrollMode = getOverScrollMode();
                boolean canOverscroll = overscrollMode != 0 ? overscrollMode == 1 && range > 0 : true;
                overScrollBy(x - oldX, y - oldY, oldX, oldY, range, 0, Math.abs(oldX) > this.mOverflingDistance ? this.mOverscrollDistance : this.mOverflingDistance, 0, false);
                onScrollChanged(this.mScrollX, this.mScrollY, oldX, oldY);
                if (!(!canOverscroll || (this.mEnableSpringEffect ^ 1) == 0 || this.mEdgeGlowLeft == null)) {
                    if (x < 0 && oldX >= 0) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (this.mEnableSpringEffect) {
            this.mOverscrollDistance = (r - l) / 3;
    ...
    public void setOverScrollMode(int mode) {
        ...
        } else if (this.mEdgeGlowLeft == null && (this.mEnableSpringEffect ^ 1) != 0) {
    ...
    public void draw(Canvas canvas) {
        super.draw(canvas);
        if (this.mEdgeGlowLeft != null && (this.mEnableSpringEffect ^ 1) != 0) {
            int restoreCount;
            int height;
            int scrollX = this.mScrollX;
            if (!this.mEdgeGlowLeft.isFinished()) {
    ...
    private int reviseOverScrollByTouch(int srcs, int old, int range) {
        int total = srcs + old;
        if (srcs > 0 && total > range) {
            return (((getWidth() - Math.abs(this.mScrollX - range)) * srcs) / getWidth()) / 2;
        }
        if (srcs > 0 || total >= 0) {
            return srcs;
        }
        return (((getWidth() - Math.abs(this.mScrollX)) * srcs) / getWidth()) / 2;
    }
}
