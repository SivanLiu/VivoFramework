...
public abstract class AbsListView extends AdapterView<ListAdapter> implements TextWatcher, OnGlobalLayoutListener, FilterListener, OnTouchModeChangeListener, RemoteAdapterConnectionCallback {
    ...
    private static int END_HOLDMODE_TIMEOUT = 1000;
    private static int HOLD_MAX_POSITION_Y = 800;
    private static int HOLD_MIN_HEIGHT = 800;
    private static int HOLD_THRESHOLD = 0;
    private static int HOLD_TOLERANCE = 10;
    private static final int INVALID_POINTER = -1;
    private static final String KEY_LISTVIEW_OVERSCROLL = "listview_oversroll";
    ...
    private static final String MINSCREEN_ACTION = "android.action.multifloatingtask.showsmallwindow";
    ...
    private static int OVERSCROLLER_MODE = 3;
    static final int OVERSCROLL_LIMIT_DIVISOR = 3;
    private static final int OVERSCROLL_LISTVIEW_BOTTOM = 1;
    ...
    private static final int UNOVERSCROLL_LISTVIEW_BOTTOM = 0;
    static final Interpolator sLinearInterpolator = new LinearInterpolator();
    private static final String sRomVersion = SystemProperties.get("ro.vivo.rom.version");
    private String HOLDED_TEXT;
    private String HOLDING_TEXT;
    private ListItemAccessibilityDelegate mAccessibilityDelegate;
    private int mActivePointerId;
    ListAdapter mAdapter;
    boolean mAdapterHasStableIds;
    private boolean mAllowNewTask;
    ...
    private boolean mEnableEdgeEffect;
    private boolean mEnableSpringEffect;
    private final Runnable mEndFlingRunnable;
    ...
    private Paint mHoldPaint;
    private String mHoldedString;
    private boolean mHoldingMode;
    private boolean mHoldingModeClientSetEnabled;
    private boolean mHoldingModeEnabled;
    private String mHoldingString;
    ...
    BroadcastReceiver mMinScreenReceiver;
    ...
    private boolean mOnListViewBottom;
    private OnScrollListener mOnScrollListener;
    private int mOriginalOverscrollDistance;
    ...
    private BoostFramework mPerfTouch;
    ...
    private final Runnable mSpringBackRunnable;
    ...
    private UpdateBottomFlagTask mUpdateBottomFlagTask;
    ...
    private class FlingRunnable implements Runnable {
        ...
        private final Runnable mCheckFlywheel = new Runnable() {
            public void run() {
                ...
                OverScrollerProxy scroller = FlingRunnable.this.mScroller;
                if (vt != null && activeId != -1) {
                    ...
                    if (AbsListView.this.mEnableSpringEffect || Math.abs(yvel) < ((float) AbsListView.this.mMinimumVelocity) || !scroller.isScrollingInDirection(0.0f, yvel)) {
        ...
        private boolean mEadgeReached = false;
        private int mLastFlingY;
        private final OverScrollerProxy mScroller;
        private boolean mSuppressIdleStateChangeCall;

        FlingRunnable() {
            this.mScroller = new OverScrollerProxy(AbsListView.this.mEnableSpringEffect ? AbsListView.OVERSCROLLER_MODE : 1, AbsListView.this.getContext());
        }
            ...
            switch (AbsListView.this.mTouchMode) {
                ...
                case 6:
                    ...
                    if (!scroller.computeScrollOffset()) {
                        this.mEadgeReached = false;
                        if (AbsListView.this.mScrollY != 0 && AbsListView.this.mEnableSpringEffect) {
                            startSpringback();
                            break;
                        } else {
                            endFling();
                    ...
                    if (!AbsListView.this.overScrollBy(0, currY - scrollY, 0, scrollY, 0, 0, 0, Math.abs(scrollY) > AbsListView.this.mOverflingDistance ? AbsListView.this.mOverscrollDistance : AbsListView.this.mOverflingDistance, false)) {
                        AbsListView.this.invalidate();
                        AbsListView.this.postOnAnimation(this);
                        break;
                    }
    ...
    class UpdateBottomFlagTask extends AsyncTask {
        UpdateBottomFlagTask() {
        }

        protected Object doInBackground(Object... params) {
            if (!isSuperFloatViewServiceRunning() || params == null || params.length <= 0 || isCancelled()) {
                return null;
            }
            boolean isBottom = ((Boolean) params[0]).booleanValue();
            try {
                int i;
                ContentResolver contentResolver = AbsListView.this.getContext().getContentResolver();
                String str = AbsListView.KEY_LISTVIEW_OVERSCROLL;
                if (isBottom) {
                    i = 1;
                } else {
                    i = 0;
                ...
                Log.d(AbsListView.TAG, " Exception " + e.toString());
            }
            return null;
        }

        private boolean isSuperFloatViewServiceRunning() {
            List<RunningServiceInfo> serviceList = ((ActivityManager) AbsListView.this.getContext().getSystemService("activity")).getRunningServices(Integer.MAX_VALUE);
            if (isCancelled()) {
                AbsListView.this.mAllowNewTask = true;
                return false;
            }
            boolean isRunning = false;
            for (int i = 0; i < serviceList.size(); i++) {
                RunningServiceInfo serviceInfo = (RunningServiceInfo) serviceList.get(i);
                if (serviceInfo != null && serviceInfo.service != null && serviceInfo.service.getClassName() != null && serviceInfo.service.getClassName().contains("SuperShotFloatViewService")) {
                    isRunning = true;
                    break;
                }
            }
            AbsListView.this.mAllowNewTask = isRunning;
            return isRunning;
        }
    }
    ...
    public AbsListView(Context context) {
        ...
        this.mHoldingString = null;
        this.mHoldedString = null;
        this.mSideMargin = 0;
        this.mEnableSpringEffect = false;
        this.mEnableEdgeEffect = true;
        this.mUpdateBottomFlagTask = null;
        this.mAllowNewTask = true;
        this.mHoldingMode = false;
        this.mHoldingModeEnabled = true;
        this.mHoldingModeClientSetEnabled = true;
        this.mReceiverRegistered = false;
        this.mIsSystemApp = false;
        this.mEndFlingRunnable = new Runnable() {
            public void run() {
                if (AbsListView.this.mFlingRunnable != null) {
                    AbsListView.this.mFlingRunnable.endFling();
                }
                if (AbsListView.this.mScrollY != 0) {
                    AbsListView.this.mHoldingMode = false;
                    AbsListView.this.mScrollY = 0;
                    AbsListView.this.invalidateParentCaches();
                    AbsListView.this.finishGlows();
                    AbsListView.this.invalidate();
                    AbsListView.this.mTouchMode = -1;
                    AbsListView.this.reportScrollStateChange(-1);
                }
        ...
        this.mSpringBackRunnable = new Runnable() {
            public void run() {
                AbsListView.this.mHoldingMode = false;
                if (AbsListView.this.mFlingRunnable != null) {
                    AbsListView.this.removeCallbacks(AbsListView.this.mFlingRunnable);
                    AbsListView.this.mFlingRunnable.endFling();
                }
                if (AbsListView.this.mPositionScroller != null) {
                    AbsListView.this.mPositionScroller.stop();
                ...
                AbsListView.this.reportScrollStateChange(2);
                if (AbsListView.this.mFlingRunnable == null) {
                    AbsListView.this.mFlingRunnable = new FlingRunnable();
                }
                AbsListView.this.mFlingRunnable.startSpringback();
            }
        };
        this.mMinScreenReceiver = new BroadcastReceiver() {
            public void onReceive(Context context, Intent intent) {
                if (AbsListView.MINSCREEN_ACTION.equals(intent.getAction()) && AbsListView.this.mHoldingMode) {
                    AbsListView.this.post(AbsListView.this.mSpringBackRunnable);
                }
            }
        };
        initAbsListView();
    ...
    public AbsListView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mReceiverRegistered = false;
        this.mIsSystemApp = false;
        this.mEndFlingRunnable = /* anonymous class already generated */;
        this.mSpringBackRunnable = /* anonymous class already generated */;
        this.mMinScreenReceiver = /* anonymous class already generated */;
        initAbsListView();
        this.mOwnerThread = Thread.currentThread();
        ...
        a.recycle();
        TypedArray vivoAttr = context.obtainStyledAttributes(attrs, vivo.R.styleable.AbsListView, defStyleAttr, defStyleRes);
        this.mHoldingModeClientSetEnabled = vivoAttr.getBoolean(0, this.mHoldingModeClientSetEnabled);
        boolean vivoSpringEffect = vivoAttr.getBoolean(1, false);
        vivoAttr.recycle();
        if (VivoThemeUtil.isVigourTheme(getContext()) || vivoSpringEffect) {
            setSpringEffect(true);
            setEdgeEffect(false);
        }
        HOLD_THRESHOLD = (int) (this.mDensityScale * 170.0f);
        int i = (int) (this.mDensityScale * 267.0f);
        HOLD_MAX_POSITION_Y = i;
        HOLD_MIN_HEIGHT = i;
        initializeHoldText();
        if ("rom_2.0".equals(sRomVersion)) {
            this.mHoldingModeEnabled = false;
        }
    ...
    private void initializeHoldText() {
        this.mHoldPaint = new Paint(1);
        this.mHoldPaint.setTextSize(getResources().getDimension(com.vivo.internal.R.dimen.abslistview_hold_text_size));
        this.mSideMargin = getResources().getDimensionPixelOffset(com.vivo.internal.R.dimen.abslistview_hold_side_margin);
        this.mHoldingString = getResources().getString(com.vivo.internal.R.string.vivo_list_pull_to_hold);
        this.mHoldedString = getResources().getString(com.vivo.internal.R.string.vivo_list_holded);
    }
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        clearChoices();
        if (this.mHoldingMode && adapter == null) {
            post(this.mEndFlingRunnable);
        }
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (this.mEnableSpringEffect) {
            int i2 = (b - t) / 2;
            this.mOverscrollDistance = i2;
            this.mOriginalOverscrollDistance = i2;
            this.mOverflingDistance = this.mOverscrollDistance / 5;
        }
        refreshHoldModeEnableState();
    }
    ...
    protected void onAttachedToWindow() {
        ...
        if (!this.mReceiverRegistered) {
            getContext().registerReceiver(this.mMinScreenReceiver, new IntentFilter(MINSCREEN_ACTION));
    ...
    protected void onDetachedFromWindow() {
        ...
        if (this.mReceiverRegistered) {
            try {
                getContext().unregisterReceiver(this.mMinScreenReceiver);
            } catch (IllegalArgumentException e) {
                Log.d(TAG, "mMinScreenReceiver not registered!");
            } catch (Throwable th) {
                this.mReceiverRegistered = false;
            }
            this.mReceiverRegistered = false;
    ...
    public void onWindowFocusChanged(boolean hasWindowFocus) {
        ...
        this.mAllowNewTask = true;
        if (hasWindowFocus && this.mOnListViewBottom) {
            setListiewBottom(false, true);
        }
        if (this.mHoldingMode) {
            postDelayed(this.mSpringBackRunnable, (long) END_HOLDMODE_TIMEOUT);
        }
        if (hasWindowFocus) {
            ...
            if (this.mFlingRunnable != null) {
                ...
                if (!(this.mScrollY == 0 || (this.mHoldingMode ^ 1) == 0)) {
                    this.mScrollY = 0;
    ...
    private void scrollIfNeeded(int x, int y, MotionEvent vtev) {
        ...
        if (this.mTouchMode == 3) {
                ...
                if (motionView != null) {
                    ...
                    if (atEdge) {
                        ...
                        if (dispatchNestedScroll(0, overscroll - incrementalDeltaY, 0, overscroll, this.mScrollOffset)) {
                            ...
                            if (this.mEnableSpringEffect) {
                                reviseOverScrollByTouch = reviseOverScrollByTouch(overscroll);
                            } else {
                                reviseOverScrollByTouch = overscroll;
                            }
                            boolean atOverscrollEdge = overScrollBy(0, reviseOverScrollByTouch, 0, this.mScrollY, 0, 0, 0, this.mOverscrollDistance, true);
            ...
            if (overScrollDistance != 0) {
                if (this.mEnableSpringEffect) {
                    reviseOverScrollByTouch = reviseOverScrollByTouch(overScrollDistance);
                } else {
                    reviseOverScrollByTouch = overScrollDistance;
                }
                overScrollBy(0, reviseOverScrollByTouch, 0, this.mScrollY, 0, 0, 0, this.mOverscrollDistance, true);
                overscrollMode = getOverScrollMode();
                if (overscrollMode == 0 || (overscrollMode == 1 && (contentFits() ^ 1) != 0)) {
                    if (rawDeltaY > 0) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (isEnabled()) {
            ...
            switch (actionMasked) {
                case 0:
                    try {
                        if ("com.tencent.mm".equals(getContext().getPackageName())) {
                            if (this.mPerfTouch == null) {
                                this.mPerfTouch = new BoostFramework();
                            }
                            if (this.mPerfTouch != null) {
                                this.mPerfTouch.perfHint(BoostFramework.VENDOR_HINT_TOUCH_BOOST, "", -1, 3);
                            }
                        }
                    } catch (Exception e) {
                        Log.d(TAG, "getPackageName fail");
                    }
                    onTouchDown(ev);
                    if (OPTS_INPUT) {
                        this.mNumTouchMoveEvent = 0;
    ...
    private void onTouchDown(MotionEvent ev) {
        ...
        if (this.mTouchMode == 6) {
            ...
            if (imm != null && this.mHoldingModeEnabled) {
            ...
            if (!this.mDataChanged) {
                ...
                } else if ((motionPosition >= 0 && ((ListAdapter) getAdapter()).isEnabled(motionPosition)) || this.mHoldingModeEnabled) {
                    this.mTouchMode = 0;
                    this.mMotionPosition = motionPosition;
                    if (this.mPendingCheckForTap == null) {
    ...
    private void onTouchMove(MotionEvent ev, MotionEvent vtev) {
        if (!this.mHasPerformedLongPress) {
            ...
            if (this.mHoldingMode && this.mScrollY >= HOLD_TOLERANCE - HOLD_THRESHOLD) {
                this.mHoldingMode = false;
            }
            switch (this.mTouchMode) {
                case 0:
    ...
    private void onTouchUp(MotionEvent ev) {
        ...
        switch (this.mTouchMode) {
            ...
            case 2:
                            ...
                            if (this.mDataChanged || !this.mAdapter.isEnabled(motionPosition)) {
                                ...
                                this.mTouchModeReset = new Runnable() {
                                    public void run() {
                                        ...
                                        child.setPressed(false);
                                        AbsListView.this.setPressed(false);
                                        if (!AbsListView.this.mDataChanged && (AbsListView.this.mIsDetaching ^ 1) != 0 && AbsListView.this.isAttachedToWindow() && !AbsListView.this.post(performClick2)) {
                                            performClick2.run();
                                        }
                                    }
                                };
                ...
                } else if (this.mHoldingMode && motionPosition == -1 && ev.getY() < ((float) HOLD_THRESHOLD)) {
                    post(this.mSpringBackRunnable);
                }
                this.mTouchMode = -1;
                updateSelectorState();
                break;
            ...
            case 5:
                ...
                reportScrollStateChange(2);
                if (this.mScrollY < HOLD_TOLERANCE - HOLD_THRESHOLD && (this.mHoldingMode ^ 1) != 0 && this.mHoldingModeEnabled) {
                    this.mHoldingMode = true;
                    this.mTouchMode = -1;
                    reportScrollStateChange(0);
                    ViewParent parent = getParent();
                    if (parent != null) {
                ...
                if (!this.mHoldingMode) {
                    if (Math.abs(initialVelocity) > this.mMinimumVelocity && (this.mEnableSpringEffect ^ 1) != 0) {
                        this.mFlingRunnable.startOverfling(-initialVelocity);
                        if (this.mScrollY >= HOLD_TOLERANCE - HOLD_THRESHOLD && this.mHoldingModeEnabled) {
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.mEdgeGlowTop != null) {
            ...
            if (!this.mEdgeGlowTop.isFinished() && this.mEnableEdgeEffect) {
            ...
            if (!(this.mEdgeGlowBottom.isFinished() || !this.mEnableEdgeEffect || (skipDrawChild() ^ 1) == 0)) {
                restoreCount = canvas.save();
    ...
    boolean trackMotionScroll(int deltaY, int incrementalDeltaY) {
        ...
        if (cannotScrollDown || cannotScrollUp) {
            boolean z;
            if (incrementalDeltaY != 0 && deltaY <= 0) {
                setListiewBottom(true);
            }
            if (incrementalDeltaY != 0) {
                z = true;
            } else {
    ...
    public void setSpringEffect(boolean value) {
        this.mEnableSpringEffect = value;
        setOverScrollMode(value ? 0 : 2);
        if (OVERSCROLLER_MODE == 3) {
            this.mMinimumVelocity = getResources().getInteger(com.vivo.internal.R.integer.vigour_config_springMinSpeed);
        }
    }

    public void setEdgeEffect(boolean value) {
        this.mEnableEdgeEffect = value;
        invalidate();
    }

    private int reviseOverScrollByTouch(int srcs) {
        if (this.mScrollY * srcs < 0) {
            return srcs;
        }
        int res = (((getHeight() - Math.abs(this.mScrollY)) * srcs) / getHeight()) / 2;
        if (this.mHoldingModeEnabled) {
            res = (res * 7) / 4;
        }
        return res;
    ...
    private void setListiewBottom(boolean value) {
        setListiewBottom(value, false);
    }

    private void setListiewBottom(boolean value, boolean force) {
        if (force || this.mOnListViewBottom != value) {
            this.mOnListViewBottom = value;
            if (this.mAllowNewTask) {
                if (!(this.mUpdateBottomFlagTask == null || this.mUpdateBottomFlagTask.getStatus() == Status.FINISHED)) {
                    this.mUpdateBottomFlagTask.cancel(true);
                    this.mUpdateBottomFlagTask = null;
                }
                this.mUpdateBottomFlagTask = new UpdateBottomFlagTask();
                this.mUpdateBottomFlagTask.execute(new Object[]{Boolean.valueOf(this.mOnListViewBottom)});
            }
        }
    }

    public void setHoldingModeEnabled(boolean value) {
        this.mHoldingModeEnabled = value;
        this.mHoldingModeClientSetEnabled = value;
    }

    public boolean getHoldingModeEnabled() {
        return this.mHoldingModeEnabled ? this.mHoldingModeClientSetEnabled : false;
    }

    public boolean getIsHoldingMode() {
        return this.mHoldingMode;
    }
     ...
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    private void drawHoldText(Canvas canvas) {
        CharSequence holdString = null;
        if (!((this.mHoldingMode && this.mScrollY >= HOLD_TOLERANCE - HOLD_THRESHOLD && this.mDirection == 1) || this.mTouchMode == 6 || this.mScrollY * 2 > (-HOLD_THRESHOLD))) {
            int alpha = (int) ((((((float) Math.abs(this.mScrollY)) * 2.0f) / ((float) HOLD_THRESHOLD)) - 1.0f) * 255.0f);
            int colorTemp = VivoThemeUtil.getColor(getContext(), R.attr.textColorSecondary);
            this.mHoldPaint.setColor(Color.argb(alpha, Color.red(colorTemp), Color.green(colorTemp), Color.blue(colorTemp)));
            if (this.mScrollY < HOLD_TOLERANCE - HOLD_THRESHOLD) {
                holdString = this.mHoldedString;
            } else if (this.mDirection >= 0) {
                holdString = this.mHoldingString;
            }
            if (holdString != null) {
                StaticLayout layout = new StaticLayout(holdString, 0, holdString.length(), new TextPaint(this.mHoldPaint), getWidth() - (this.mSideMargin * 2), Alignment.ALIGN_CENTER, getTextDirectionHeuristic(), 1.0f, 0.0f, false, TruncateAt.END, getWidth() - (this.mSideMargin * 2), 2);
                int saveCount = canvas.save();
                canvas.translate((float) this.mSideMargin, (float) (this.mScrollY + (HOLD_THRESHOLD / 3)));
                layout.draw(canvas);
                canvas.restoreToCount(saveCount);
            }
        }
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        ...
        Log.d("yuan", "onVisibilityChanged----" + this);
        if (this.mHoldingMode) {
            post(this.mEndFlingRunnable);
        }
    }
    ...
    private boolean isSystemApp() {
        try {
            boolean isSystemPackageName;
            String packageName = getContext().getPackageName();
            PackageInfo pInfo = getContext().getPackageManager().getPackageInfo(packageName, 0);
            if (packageName.contains("com.android.") || packageName.contains("com.bbk.") || packageName.contains("com.vivo.") || packageName.contains("com.iqoo.") || packageName.contains("com.mediatek.") || packageName.contains("com.qualcomm.") || packageName.equals(ZenModeConfig.SYSTEM_AUTHORITY) || packageName.contains("com.quicinc.")) {
                isSystemPackageName = true;
            } else {
                isSystemPackageName = packageName.contains("com.baidu.input_bbk.");
            }
            if ((pInfo.applicationInfo.flags & 1) == 0 && (pInfo.applicationInfo.flags & 128) == 0) {
                isSystemPackageName = false;
            }
    ...
    private boolean isMinScreen() {
        if (System.getInt(getContext().getContentResolver(), "vivo_mini_screen_gesture_enable", 1) == 1) {
            String serviceClassName = "com.vivo.minscreen.service.MinScreenService";
            List<RunningServiceInfo> services = ((ActivityManager) getContext().getSystemService("activity")).getRunningServices(Integer.MAX_VALUE);
            if (services == null) {
                return false;
            }
    ...
    public void springBack() {
        if (this.mHoldingMode) {
            removeCallbacks(this.mEndFlingRunnable);
            post(this.mEndFlingRunnable);
        }
    }

    private void refreshHoldModeEnableState() {
        int[] pos = new int[2];
        getLocationOnScreen(pos);
        if (!"rom_2.0".equals(sRomVersion)) {
            if (this.mHoldingModeClientSetEnabled && this.mIsSystemApp && (isMinScreen() ^ 1) != 0) {
                ...
                if (this.mHoldingModeEnabled) {
                    if (pos[1] > HOLD_MAX_POSITION_Y || getHeight() < HOLD_MIN_HEIGHT || getChildCount() <= 0) {
                        this.mHoldingModeEnabled = false;
                    } else {
                        this.mHoldingModeEnabled = true;
                    }
        ...
        if (this.mHoldingModeEnabled) {
            this.mOverscrollDistance = HOLD_THRESHOLD;
        } else {
            this.mOverscrollDistance = this.mOriginalOverscrollDistance;
        }
        if (this.mEnableSpringEffect) {
