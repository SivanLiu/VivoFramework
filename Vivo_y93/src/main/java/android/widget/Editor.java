...
public class Editor {
    ...
    private static final int MENU_ITEM_ORDER_INPUT_METHOD = 14;
    private static final int MENU_ITEM_ORDER_PASTE = 8;
    private static final int MENU_ITEM_ORDER_PASTE_AS_PLAIN_TEXT = 12;
    ...
    private static final int MENU_ITEM_ORDER_SELECT_TEXT = 4;
    private static final int MENU_ITEM_ORDER_SHARE = 9;
    private static final int MENU_ITEM_ORDER_TEXT_SEARCH = 13;
    ...
    public abstract class HandleView extends View implements TextViewPositionListener {
        ...
        public float getHorizontal(Layout layout, int offset) {
            if (Editor.this.mTextView.mEndCursorSupport && (Editor.this.needPositionEnd(offset, Editor.this.mLastDownPositionX) ^ 1) == 0 && (Editor.this.mTextView.mEndCursorSupportFlag ^ 1) == 0) {
                return layout.getPrimaryHorizontal(offset, 0);
            }
        ...
        protected void positionAtCursorOffset(int offset, boolean forceUpdatePosition, boolean fromTouchScreen) {
            ...
            if (offsetChanged || forceUpdatePosition) {
                ...
                if (Editor.this.mTextView.mEndCursorSupport && (Editor.this.needPositionEnd(offset, Editor.this.mLastDownPositionX) ^ 1) == 0) {
                    line = layout.getLineForOffset(offset, false);
                } else {
        ...
        public boolean onTouchEvent(MotionEvent ev) {
            ...
            switch (ev.getActionMasked()) {
                ...
                case 2:
                    ...
                    float newPosX = ((xInWindow - this.mTouchToWindowOffsetX) + ((float) this.mHotspotX)) + ((float) getHorizontalOffset());
                    float newPosY = (yInWindow - this.mTouchToWindowOffsetY) + this.mTouchOffsetY;
                    Editor.this.hideFloatingToolbar(-1);
    ...
    private class InsertionHandleView extends HandleView {
        ...
        protected void updatePosition(float x, float y, boolean fromTouchScreen) {
            int offset;
            Layout layout = Editor.this.mTextView.getLayout();
            Editor.this.mLastDownPositionX = x;
            Editor.this.mLastDownPositionY = y;
            if (layout != null) {
            ...
            SelectionModifierCursorController control = Editor.this.getSelectionController();
            if (control != null) {
                control.resetTouchOffsets();
            }
    ...
    private class SelectionHandleView extends HandleView {
        ...
        /* JADX WARNING: Missing block: B:81:0x021d, code:
            if (android.widget.Editor.-get12(r26.this$0).canScrollHorizontally(r6 ? -1 : 1) == false) goto L_0x021f;
     */
        ...
        private float getHorizontal(Layout layout, int offset, boolean startHandle) {
            ...
            if (Editor.this.mTextView.mEndCursorSupport && (Editor.this.needPositionEnd(offset, Editor.this.mLastDownPositionX) ^ 1) == 0 && (Editor.this.mTextView.mEndCursorSupportFlag ^ 1) == 0) {
                return isRtlChar == isRtlParagraph ? layout.getPrimaryHorizontal(offset, 0) : layout.getSecondaryHorizontal(offset);
            }
    ...
    private class TextActionModeCallback extends Callback2 {
        ...
        private void populateMenuWithItemsExt(Menu menu) {
            updateSelectAllItemExt(menu);
            ...
            boolean isInSuperPowerSaveMode = SystemProperties.getBoolean("sys.super_power_save", false) ^ 1;
            boolean isRootUser = UserHandle.myUserId() == 0;
            if (Editor.this.mTextView.canSearch() && isInSuperPowerSaveMode && isRootUser) {
                menu.add(0, (int) com.vivo.internal.R.id.search, 13, (int) R.string.websearch).setShowAsAction(2);
            }
            boolean isPasswordInputType = isPasswordInputType(Editor.this.mTextView.getInputType());
            int imeSecure;
            if ("1".equals(Secure.getString(Editor.this.mTextView.getContext().getContentResolver(), InputMethodUtils.VIVO_SECURE_INPUTMETHOD_DATABASE))) {
                imeSecure = isPasswordInputType;
            } else {
                imeSecure = 0;
            }
            if (!(!Editor.this.mTextView.isInputMethodTarget() || (Editor.this.mTextView.hasSelection() ^ 1) == 0 || (imeSecure ^ 1) == 0)) {
                menu.add(0, (int) R.id.switchInputMethod, 14, (CharSequence) "").setIcon(Editor.this.mTextView.getContext().getDrawable(com.vivo.internal.R.drawable.vigour_text_toolbar_inputmethod)).setShowAsAction(2);
            }
        ...
        private void updateSelectAllItemExt(Menu menu) {
            boolean canSelectAll = Editor.this.mTextView.canSelectAllText();
            boolean selectAllItemExists = menu.findItem(R.id.selectAll) != null;
            boolean canCopy = Editor.this.mTextView.canCopy();
            if (canSelectAll && (selectAllItemExists ^ 1) != 0) {
                menu.add(0, (int) R.id.selectAll, 5, (int) R.string.selectAll).setShowAsAction(1);
            }
            if (!canCopy && canSelectAll && (selectAllItemExists ^ 1) != 0) {
                menu.add(0, (int) R.id.startSelectingText, 4, (int) com.vivo.internal.R.string.bbk_edit_selectText).setShowAsAction(2);
            } else if (!canSelectAll && selectAllItemExists) {
                menu.removeItem(R.id.startSelectingText);
        ...
        private void populateMenuWithItems(Menu menu) {
            ...
            if (Editor.this.mTextView.canShare()) {
                menu.add(0, (int) R.id.shareText, 9, (int) R.string.share).setShowAsAction(1);
            }
            if (Editor.this.mTextView.canRequestAutofill()) {
                String selected = Editor.this.mTextView.getSelectedText();
                if (selected == null || selected.isEmpty()) {
            ...
            if (Editor.this.mTextView.canPasteAsPlainText()) {
                menu.add(0, (int) R.id.pasteAsPlainText, 12, (int) R.string.paste_as_plain_text).setShowAsAction(1);
            }
        ...
        private void updateAssistMenuItem(Menu menu) {
            ...
            boolean isInSuperPowerSaveMode = SystemProperties.getBoolean("sys.super_power_save", false);
            if (canAssist() && (isInSuperPowerSaveMode ^ 1) != 0) {
        ...
        public void onGetContentRect(ActionMode mode, View view, Rect outRect) {
            ...
            if (Editor.this.mTextView.getSelectionStart() != Editor.this.mTextView.getSelectionEnd()) {
                ...
                Layout layout = Editor.this.mTextView.getLayout();
                if (Editor.this.mTextView.mEndCursorSupport && (Editor.this.mTextView.mEndCursorSupportFlag ^ 1) == 0 && (Editor.this.needPositionEnd(Editor.this.mTextView.getSelectionStart(), Editor.this.mLastDownPositionX) ^ 1) == 0) {
                    line = layout.getLineForOffset(Editor.this.mTextView.getSelectionStart(), false);
                    primaryHorizontal = (float) Editor.this.clampHorizontalPosition(null, layout.getPrimaryHorizontal(Editor.this.mTextView.getSelectionStart(), 0));
                } else {
                    line = layout.getLineForOffset(Editor.this.mTextView.getSelectionStart());
                    primaryHorizontal = (float) Editor.this.clampHorizontalPosition(null, layout.getPrimaryHorizontal(Editor.this.mTextView.getSelectionStart()));
                }
    ...
    private int getErrorX() {
        int i = 0;
        if (VivoThemeUtil.isVigourTheme(this.mTextView.getContext())) {
            return getCustomErrorX();
        }
        int errorX;
        float scale = this.mTextView.getResources().getDisplayMetrics().density;
        Drawables dr = this.mTextView.mDrawables;
        switch (this.mTextView.getLayoutDirection()) {
            case 1:
                ...
                errorX = this.mTextView.getPaddingLeft() + ((i / 2) - ((int) ((25.0f * scale) + LINE_SLOP_MULTIPLIER_FOR_HANDLEVIEWS)));
                break;
            default:
                if (dr != null) {
                    i = dr.mDrawableSizeRight;
                }
                errorX = ((this.mTextView.getWidth() - this.mErrorPopup.getWidth()) - this.mTextView.getPaddingRight()) + (((-i) / 2) + ((int) ((25.0f * scale) + LINE_SLOP_MULTIPLIER_FOR_HANDLEVIEWS)));
                break;
        }
        return errorX;
    }
    ...
    private int getCustomErrorX() {
        int i = 0;
        Drawables dr = this.mTextView.mDrawables;
        int arrowDistance = (int) this.mTextView.getResources().getDimension(com.vivo.internal.R.dimen.textview_error_box_arrow_distance);
        switch (this.mTextView.getLayoutDirection()) {
            case 1:
                if (dr != null) {
                    i = dr.mDrawableSizeLeft;
                }
                return -(((this.mTextView.getWidth() - this.mErrorPopup.getWidth()) - this.mTextView.getPaddingLeft()) + (((-i) / 2) + arrowDistance));
            default:
                if (dr != null) {
                    i = dr.mDrawableSizeRight;
                }
                return ((this.mTextView.getWidth() - this.mErrorPopup.getWidth()) - this.mTextView.getPaddingRight()) + (((-i) / 2) + arrowDistance);
        }
    }
    ...
    boolean isCursorVisible() {
        if (this.mTextView.mCanShowCursorWhenInputTypeNull) {
    ...
    boolean selectCurrentWord() {
        ...
        if (!(this.mTextView.mEndCursorSupport && !needPositionEnd(minOffset, this.mLastDownPositionX) && (this.mTextView.mEndCursorSupportFlag ^ 1) == 0)) {
            Layout layout = this.mTextView.getLayout();
            if (minOffset == layout.getLineEnd(layout.getLineForOffset(minOffset, false))) {
                minOffset--;
            }
        }
    ...
    private boolean needPositionEnd(int offset, float x) {
        char last = 0;
        if (offset > 0) {
            last = this.mTextView.getText().charAt(offset - 1);
        }
        if (x <= ((float) ((this.mTextView.getLeft() + this.mTextView.getRight()) / 2)) || last == 10) {
            return false;
        }
        return true;
    }
    ...
    public boolean performLongClick(boolean handled) {
        ...
        if (!(handled || this.mTextActionMode == null)) {
            boolean isNote = this.mTextView.getContext().getPackageName().equals("com.android.notes");
            if (FtBuild.getRomVersion() < 4.5f || (isNote ^ 1) == 0 || !touchPositionIsInSelection()) {
                stopTextActionMode();
    ...
    private void showFloatingToolbar() {
        if (this.mTextActionMode != null) {
            ...
            this.mTextView.post(this.mShowFloatingToolbar);
    ...
    void updateCursorsPositions() {
        ...
        if (this.mTextView.mEndCursorSupport && (needPositionEnd(offset, this.mLastDownPositionX) ^ 1) == 0 && (this.mTextView.mEndCursorSupportFlag ^ 1) == 0) {
            line = layout.getLineForOffset(offset, false);
        } else {
            line = layout.getLineForOffset(offset);
        }
        ...
        if (this.mTextView.mEndCursorSupport && (needPositionEnd(offset, this.mLastDownPositionX) ^ 1) == 0 && (this.mTextView.mEndCursorSupportFlag ^ 1) == 0) {
            horizon = layout.getPrimaryHorizontal(offset, clamped, 0);
        } else {
            horizon = layout.getPrimaryHorizontal(offset, clamped);
        }
        updateCursorPosition(0, top, middle, horizon);
        if (this.mCursorCount == 2) {
    ...
    void startInsertionActionMode() {
        ...
        if (!extractedTextModeWillBeStarted()) {
            stopTextActionMode();
            this.mTextView.mShowToobar = false;
    ...
    boolean startSelectionActionModeInternal() {
        ...
        if (this.mTextActionMode != null) {
            invalidateActionMode();
            return false;
        } else if ((!checkField() || (this.mTextView.hasSelection() ^ 1) != 0) && !selectCurrentWord()) {
            return false;
        } else {
    ...
    void onTouchUpEvent(MotionEvent event) {
        if (!getSelectionActionModeHelper().resetSelection(getTextView().getOffsetForPosition(event.getX(), event.getY()))) {
            ...
            if (!selectAllGotFocus && text.length() > 0) {
                ...
                if (this.mTextView.mEndCursorSupport && this.mTextView.mEndCursorSupportFlag) {
                    this.mTextView.mHighlightPathBogus = true;
                }
                if (this.mSpellChecker != null) {
    ...
    void onCreateContextMenu(ContextMenu menu) {
        if (!this.mIsBeingLongClicked && !Float.isNaN(this.mContextMenuAnchorX) && !Float.isNaN(this.mContextMenuAnchorY)) {
            ...
            if (offset != -1) {
                ...
                menu.add(0, (int) R.id.pasteAsPlainText, 12, (int) R.string.paste_as_plain_text).setEnabled(this.mTextView.canPasteAsPlainText()).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener);
                menu.add(0, (int) R.id.shareText, 9, (int) R.string.share).setEnabled(this.mTextView.canShare()).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener);
                menu.add(0, (int) R.id.selectAll, 5, (int) R.string.selectAll).setAlphabeticShortcut(DateFormat.AM_PM).setEnabled(this.mTextView.canSelectAllText()).setOnMenuItemClickListener(this.mOnContextMenuItemClickListener);
    ...
    void checkMinOffset() {
        ...
        if (minOffset < 0 || maxOffset < 0) {
            selectionController.mMinTouchOffset = selectionController.mMaxTouchOffset = this.mTextView.getOffsetForPosition(this.mLastDownPositionX, this.mLastDownPositionY);
        }
    }
}
