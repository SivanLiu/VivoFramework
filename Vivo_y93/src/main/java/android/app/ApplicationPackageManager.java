...
public class ApplicationPackageManager extends PackageManager {
    public static final int[] CORP_BADGE_LABEL_RES_ID = new int[]{17040246, 17040247, 17040248};
    ...
    public Drawable getDrawable(String packageName, int resId, ApplicationInfo appInfo) {
        ...
        if (appInfo == null) {
            try {
                if (DoubleAppSwitcher.sEnabled && UserHandle.myUserId() == getUserManager().getDoubleAppUserId()) {
                    appInfo = getApplicationInfoAsUser(packageName, 1024, 0);
                } else {
    ...
    public Drawable getUserBadgedIcon(Drawable icon, UserHandle user) {
        if (DoubleAppSwitcher.sEnabled && isDoubleAppUser(user.getIdentifier())) {
            return new BitmapDrawable(getImageUtil().getCloneBitmap(icon, this.mContext));
        }
    ...
    private Drawable getManagedProfileIconForDensity(UserHandle user, int drawableId, int density) {
        if (DoubleAppSwitcher.sEnabled && isDoubleAppUser(user.getIdentifier())) {
            String theme_id = System.getString(this.mContext.getContentResolver(), "theme_id");
            if (theme_id == null || "2".equals(theme_id)) {
                return getDrawableForDensity(50463081, density);
            }
            return getDrawableForDensity(50463082, density);
        } else if (isManagedProfile(user.getIdentifier())) {
    ...
    public CharSequence getUserBadgedLabel(CharSequence label, UserHandle user) {
        if (DoubleAppSwitcher.sEnabled && isDoubleAppUser(user.getIdentifier())) {
            return Resources.getSystem().getString(51249556, label);
        } else if (!isManagedProfile(user.getIdentifier())) {
            return label;
        } else {
    ...
    public Resources getResourcesForApplication(ApplicationInfo app) throws NameNotFoundException {
        ...
        if (r != null) {
            r.initResource(app.packageName);
            return r;
        }
    ...
    @VivoHook(hookType = VivoHookType.NEW_METHOD)
    private boolean isDoubleAppUser(int userId) {
        if (!DoubleAppSwitcher.sEnabled) {
            return false;
        }
        if (getUserManager().getDoubleAppUserId() == userId) {
            return true;
        }
        return false;
    }

    public void grantTheRestPermissions(Boolean onlySystem) {
        try {
            this.mPM.grantTheRestPermissions(onlySystem.booleanValue());
        } catch (RemoteException e) {
        }
    }

    public Map getUninstallSysAppMap() {
        try {
            Map uninstallSysAppMap = this.mPM.getUninstallSysAppMap();
            if (uninstallSysAppMap == null) {
                return new HashMap();
            }
            return uninstallSysAppMap;
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public void insertHybridPackageInfo(String packageName, PackageInfo packageInfo) {
        try {
            this.mPM.insertHybridPackageInfo(packageName, packageInfo);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public void deleteHybridPackageInfo(String packageName) {
        try {
            this.mPM.deleteHybridPackageInfo(packageName);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public void insertHybridRule(String callingPkgname, String xcxPkgname, String platfromPkgname, String realActitityname) {
        try {
            this.mPM.insertHybridRule(callingPkgname, xcxPkgname, platfromPkgname, realActitityname);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public void deleteHybridRule(String xcxPkgname) {
        try {
            this.mPM.deleteHybridRule(xcxPkgname);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public void insertNewHybridRule(String callingPkgname, String xcxPkgname, String platfromPkgname, String realActitityname, String action, Bundle extras) {
        try {
            this.mPM.insertNewHybridRule(callingPkgname, xcxPkgname, platfromPkgname, realActitityname, action, extras);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
    ...
    public String getHybridCallingPackage(int callinguid, String action) {
        try {
            return this.mPM.getHybridCallingPackage(callinguid, action);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
