...
public class Dialog implements DialogInterface, Callback, KeyEvent.Callback, OnCreateContextMenuListener, OnWindowDismissedCallback {
    ...
    private final String hasNavigationGesture;
    ...
    Dialog(Context context, int themeResId, boolean createContextThemeWrapper) {
        ...
        this.hasNavigationGesture = WifiEnterpriseConfig.ENGINE_ENABLE;
        ...
        if (FtBuild.getRomVersion() < 4.0f || !VivoThemeUtil.isVigourTheme(this.mContext) || ((this instanceof ProgressDialog) ^ 1) == 0 || ((this instanceof SoftInputWindow) ^ 1) == 0) {
            w.setGravity(17);
        } else {
            TypedValue typedValue = new TypedValue();
            this.mContext.getResources().getValue(51118356, typedValue, true);
            w.setDimAmount(typedValue.getFloat());
            w.setGravity(80);
            if (WifiEnterpriseConfig.ENGINE_ENABLE.equals(SystemProperties.get("qemu.hw.mainkeys"))) {
                int bottomMargin = this.mContext.getResources().getDimensionPixelSize(51118352);
                w.getAttributes().y = bottomMargin - FtDeviceInfo.getGestureBarHeight(this.mContext);
            } else {
                w.getAttributes().y = this.mContext.getResources().getDimensionPixelSize(51118351);
            }
        }
        this.mListenersHandler = new ListenersHandler(this);
    }

    /* renamed from: lambda$-android_app_Dialog_7053 */
    /* synthetic */ void m6lambda$-android_app_Dialog_7053() {
        if (this.mCancelable) {
            cancel();
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (!this.mCancelable || !this.mShowing || !this.mWindow.shouldCloseOnTouch4Vivo(this.mContext, event)) {
            return false;
        }
        cancel();
        return true;
