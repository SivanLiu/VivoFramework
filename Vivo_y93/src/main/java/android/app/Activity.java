...
import android.app.mobileagent.ext.ActivityExt;
...
public class Activity extends ContextThemeWrapper implements Factory2, Callback, KeyEvent.Callback, OnCreateContextMenuListener, ComponentCallbacks2, OnWindowDismissedCallback, WindowControllerCallback, AutofillClient {
    ...
    private static int mLastAction = -1;
    private static BoostFramework mLaunchPerf = null;
    private static int mMoveActiontimes = 0;
    private static BoostFramework mPerfAnim = null;
    private static BoostFramework mPerfTouch = null;
    ActionBar mActionBar = null;
    private int mActionModeTypeStarting = 0;
    ActivityExt mActivityExt = new ActivityExt();
    ...
    private float mFreeformAlpha = 0.8f;
    private final boolean mFreeformEnableAlpha = false;
    private boolean mFreeformMode = false;
    ...
    protected void onResume() {
        ...
        AbsVivoUserProfilingManager userProfilingManager = getApplication().getUserProfilingManager();
        if (userProfilingManager != null && pkgName != null && pageName != null && userProfilingManager.isCaptureOn()) {
            boolean isPageContains = userProfilingManager.isPropertiesContainPage(pkgName, pageName);
            this.mExecutorService = getApplication().getExecutorService();
            Slog.d(TAG, "Contains:" + isPageContains);
            if (isPageContains && this.mActivityExt != null) {
                Slog.d(TAG, "handleOnResume name:" + pageName);
                this.mActivityExt.handleOnResume(this, this.mExecutorService);
            }
        }
    }
        ...
        if (keyCode == 4) {
            if (getApplicationInfo().targetSdkVersion >= 5) {
                ...
                if (InputEventReceiver.DEBUG_VIVO_CTRL) {
                    Log.d(TAG, "vLog onKeyDown startTracking");
                }
            } else {
                if (InputEventReceiver.DEBUG_VIVO_CTRL) {
                    Log.d(TAG, "vLog onKeyDown onBackPressed.");
                }
                onBackPressed();
            }
            return true;
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        if (getApplicationInfo().targetSdkVersion >= 5) {
            if (keyCode == 4 && event.isTracking() && (event.isCanceled() ^ 1) != 0) {
                if (InputEventReceiver.DEBUG_VIVO_CTRL) {
                    Log.d(TAG, "vLog onKeyUp onBackPressed.");
                }
                onBackPressed();
                return true;
            } else if (keyCode == 4 && InputEventReceiver.DEBUG_VIVO_CTRL) {
                Log.d(TAG, "vLog onKeyUp do.");
            }
        }
        return false;
    }
    ...
    public void onBackPressed() {
        ...
        if (InputEventReceiver.DEBUG_VIVO_CTRL) {
            Log.d(TAG, "vLog onBackPressed return.");
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (!this.mWindow.shouldCloseOnTouch4Vivo(this, event)) {
            return false;
        }
        finish();
        return true;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if ("com.bbk.launcher2".equals(getPackageName()) && (ev.getAction() == 0 || ev.getAction() == 1)) {
            if (mLaunchPerf == null) {
                mLaunchPerf = new BoostFramework();
            }
            if (mLaunchPerf != null) {
                mLaunchPerf.perfHint(4229, "", -1, 2);
            }
        }
        if (this.mComponent != null) {
            compClassName = this.mComponent.getClassName();
        }
        if (("com.netease.nr.phone.main.MainActivity".equals(compClassName) || "com.cmplay.tile2.ui.AppActivity".equals(compClassName)) && ev.getAction() == 0) {
            if (mPerfTouch == null) {
                mPerfTouch = new BoostFramework();
            }
            if (mPerfTouch != null) {
                mPerfTouch.perfHint(4229, "", -1, 3);
            }
        }
        if ("com.tencent.mm.ui.LauncherUI".equals(compClassName) || "cooperation.qzone.QzoneFeedsPluginProxyActivity".equals(compClassName) || "com.tencent.mobileqq.activity.SplashActivity".equals(compClassName) || "com.whatsapp.HomeActivity".equals(compClassName) || "com.facebook.katana.LoginActivity".equals(compClassName) || "com.facebook.katana.activity.FbMainTabActivity".equals(compClassName) || "com.facebook.katana.activity.ImmersiveActivity".equals(compClassName) || "com.facebook.messenger.neue.MainActivity".equals(compClassName)) {
            if (ev.getAction() == 1 && mMoveActiontimes <= 2) {
                if (mPerfTouch == null) {
                    mPerfTouch = new BoostFramework();
                }
                if (mPerfTouch != null) {
                    mPerfTouch.perfHint(4229, "", -1, 1);
                }
                if (mPerfAnim == null) {
                    mPerfAnim = new BoostFramework();
                }
                if (mPerfAnim != null) {
                    mPerfAnim.perfHint(4227, "", -1, 3);
                }
            }
            if (mLastAction != 2) {
                mMoveActiontimes = 0;
    ...
    final void attach(Context context, ActivityThread aThread, Instrumentation instr, IBinder token, int ident, Application application, Intent intent, ActivityInfo info, CharSequence title, Activity parent, String id, NonConfigurationInstances lastNonConfigurationInstances, Configuration config, String referrer, IVoiceInteractor voiceInteractor, Window window, ActivityConfigCallback activityConfigCallback) {
        ...
        this.mWindow = new PhoneWindow(this, window, activityConfigCallback, intent.getComponent());
        ...
        if (getPackageName().equals("com.tencent.mobileqq") || getPackageName().equals("com.tencent.mm")) {
            ((PhoneWindow) this.mWindow).updateActivityName(getClass().getName());
        }
    }
    ...
    public void miniMizeWindowFreeformMode(boolean mini) throws RemoteException {
        ActivityManagerNative.getDefault().miniMizeWindowFreeformMode(this.mToken, mini);
    }

    public void enterResizeMode(boolean enter) throws RemoteException {
        ActivityManagerNative.getDefault().enterResizeMode(this.mToken, enter);
    }
}
