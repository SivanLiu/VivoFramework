        ...
        for (int i = 0; i < input.bucketCount; i++) {
            ...
            if (bucketStart >= start && bucketEnd <= end) {
                ...
                if (entry.rxBytes < 0 || entry.rxPackets < 0 || entry.txBytes < 0 || entry.txPackets < 0 || entry.operations < 0) {
                    Log.v("NetworkStatsHistory", "recordHistory with negative data rxBytes = " + entry.rxBytes + " rxPackets = " + entry.rxPackets + " txBytes = " + entry.txBytes + " txPackets = " + entry.txPackets + " operations = " + entry.operations);
