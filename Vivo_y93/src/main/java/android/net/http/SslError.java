...
public class SslError {
    ...
    static final /* synthetic */ boolean f92-assertionsDisabled = (SslError.class.desiredAssertionStatus() ^ 1);
    ...
    public SslError(int error, SslCertificate certificate, String url) {
        if (!f92-assertionsDisabled && certificate == null) {
            throw new AssertionError();
        } else if (f92-assertionsDisabled || url != null) {
            addError(error);
    ...
    public static SslError SslErrorFromChromiumErrorCode(int error, SslCertificate cert, String url) {
        if (!f92-assertionsDisabled && (error < -299 || error > PackageManager.INSTALL_FAILED_CANCELLED_BY_USER)) {
            throw new AssertionError();
        } else if (error == PackageManager.INSTALL_FAILED_CANCELLED_BY_USER) {
