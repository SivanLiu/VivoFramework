...
public class NetworkStats implements Parcelable {
    ...
    public static class Entry {
        public boolean containExtendData;
        public String iface;
        public int metered;
        ...
        public String toString() {
            ...
            if (this.containExtendData) {
                builder.append(" packInfo=").append(this.packInfo);
            }
        ...
        public boolean equals(Object o) {
            ...
            Entry e = (Entry) o;
            if (this.uid == e.uid && this.set == e.set && this.tag == e.tag && this.metered == e.metered && this.roaming == e.roaming && this.rxBytes == e.rxBytes && this.rxPackets == e.rxPackets && this.txBytes == e.txBytes && this.txPackets == e.txPackets && this.operations == e.operations && this.iface.equals(e.iface)) {
                z = this.containExtendData ? this.packInfo.equals(e.packInfo) : true;
            }
            return z;
        }
    ...
    public NetworkStats(long elapsedRealtime, int initialSize) {
        ...
        if (initialSize >= 0) {
            ...
            this.packInfo = new String[initialSize];
            return;
        }
        this.capacity = 0;
        ...
        this.packInfo = EmptyArray.STRING;
    }
    ...
    public NetworkStats(Parcel parcel) {
        ...
        this.containExtendData = z;
        if (this.containExtendData) {
            this.packInfo = parcel.createStringArray();
        }
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeInt(this.containExtendData ? 1 : 0);
        if (this.containExtendData) {
            dest.writeStringArray(this.packInfo);
        }
    }
    ...
    public NetworkStats addValues(Entry entry) {
        if (this.containExtendData != entry.containExtendData) {
            return this;
        }
        if (this.size >= this.capacity) {
            int newLength = (Math.max(this.size, 10) * 3) / 2;
            this.iface = (String[]) Arrays.copyOf(this.iface, newLength);
            this.packInfo = (String[]) Arrays.copyOf(this.packInfo, newLength);
        ...
        if (this.containExtendData && entry.containExtendData) {
            this.packInfo[this.size] = entry.packInfo;
        }
        this.size++;
        return this;
    }
    ...
    public Entry getValues(int i, Entry recycle) {
        ...
        if (this.containExtendData) {
            ...
            entry.packInfo = this.packInfo[i];
        }
        return entry;
    }
    ...
    public NetworkStats combineValues(Entry entry) {
        int i;
        if (this.containExtendData && entry.containExtendData) {
            i = findIndexWithPackInfo(entry.iface, entry.packInfo, entry.uid, entry.set, entry.tag, entry.metered, entry.roaming);
        } else {
    ...
    public static <C> NetworkStats subtract(NetworkStats left, NetworkStats right, NonMonotonicObserver<C> observer, C cookie, NetworkStats recycle) {
        ...
        boolean mLeftContain = left.isContainExtendData();
        boolean mRightContain = right.isContainExtendData();
        result.setContainExtendDataFlag(mLeftContain);
        if (DEBUG_SYSTEM_NETWORKSTATS) {
            Slog.d(TAG, "[NetworkStatsPack] NetworkStats subtract LeftContain= " + mLeftContain + " RightContain = " + mRightContain);
        }
        if (mLeftContain != mRightContain) {
            if (DEBUG_SYSTEM_NETWORKSTATS) {
                Slog.d(TAG, "[NetworkStatsPack] NetworkStats subtract contain not same. left.containExtendData = " + mLeftContain);
            }
            return result;
        }
        int i = 0;
        while (i < left.size) {
            ...
            if (result.isContainExtendData() && left.packInfo != null && left.packInfo.length > i) {
                entry.containExtendData = true;
                entry.packInfo = left.packInfo[i];
            }
            if (result.isContainExtendData()) {
                j = right.findIndexHintedWithPackInfo(entry.iface, entry.packInfo, entry.uid, entry.set, entry.tag, entry.metered, entry.roaming, i);
            } else {
                j = right.findIndexHinted(entry.iface, entry.uid, entry.set, entry.tag, entry.metered, entry.roaming, i);
            }
            if (result.isContainExtendData() && DEBUG_SYSTEM_NETWORKSTATS) {
                Slog.d(TAG, "[NetworkStatsPack] NetworkStats subtract result entry found index = " + j);
            }
            if (j != -1) {
            ...
            if (result.isContainExtendData() && DEBUG_SYSTEM_NETWORKSTATS) {
                Slog.d(TAG, "[NetworkStatsPack] NetworkStats subtract result entry = " + entry);
            }
        ...
        if (result.isContainExtendData() && DEBUG_SYSTEM_NETWORKSTATS) {
            ...
            Slog.d(TAG, "[NetworkStatsPack] NetworkStats subtract result NetworkStats = " + result.getElapsedRealtime());
            for (i = 0; i < result.size(); i++) {
                Slog.d(TAG, "[NetworkStatsPack] NetworkStats subtract result NetworkStats = [" + i + "] " + result.getValues(i, printRecycle));
            }
        }
        return result;
    }
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("NetworkStats: elapsedRealtime= " + this.elapsedRealtime + "\n");
        for (int i = 0; i < this.size; i++) {
            builder.append("  [");
            builder.append(i);
            builder.append("]");
            builder.append(" iface=");
            builder.append(this.iface[i]);
            builder.append(" uid=");
            builder.append(this.uid[i]);
            builder.append(" set=");
            builder.append(setToString(this.set[i]));
            builder.append(" tag=");
            builder.append(tagToString(this.tag[i]));
            builder.append(" metered=");
            builder.append(meteredToString(this.metered[i]));
            builder.append(" roaming=");
            builder.append(roamingToString(this.roaming[i]));
            ...
            if (this.containExtendData) {
                builder.append(" packInfo=");
                builder.append(this.packInfo[i]);
            }
            builder.append("\n");
        }
        return builder.toString();
    ...
    public boolean migrateTun(int tunUid, String tunIface, String underlyingIface) {
        ...
        if (isContainExtendData() && DEBUG_SYSTEM_NETWORKSTATS) {
            Slog.d(TAG, "[NetworkStatsPack] migrateTun tunIfaceTotal=" + tunIfaceTotal + " underlyingIfaceTotal=" + underlyingIfaceTotal + " poll=" + pool);
        }
    ...
    private Entry addTrafficToApplications(int tunUid, String tunIface, String underlyingIface, Entry tunIfaceTotal, Entry pool) {
        ...
        while (i < this.size) {
            if (Objects.equals(this.iface[i], tunIface) && this.uid[i] != tunUid) {
                ...
                if (this.containExtendData) {
                    tmpEntry.containExtendData = true;
                    tmpEntry.packInfo = this.packInfo[i];
                    if (DEBUG_SYSTEM_NETWORKSTATS) {
                        Slog.d(TAG, "[NetworkStatsPack] addTrafficToApplications entry =" + tmpEntry);
                    }
                }
    ...
    private void deductTrafficFromVpnApp(int tunUid, String underlyingIface, Entry moved) {
        ...
        moved.roaming = -1;
        if (isContainExtendData() && DEBUG_SYSTEM_NETWORKSTATS) {
            Slog.d(TAG, "[NetworkStatsPack] deductTrafficFromVpnApp entry =" + moved);
        }
        combineValues(moved);
    ...
    public int findIndexHintedWithPackInfo(String iface, String packInfo, int uid, int set, int tag, int metered, int roaming, int hintIndex) {
        for (int offset = 0; offset < this.size; offset++) {
            int i;
            int halfOffset = offset / 2;
            if (offset % 2 == 0) {
            ...
            if (uid == this.uid[i] && set == this.set[i] && tag == this.tag[i] && metered == this.metered[i] && roaming == this.roaming[i] && Objects.equals(iface, this.iface[i]) && Objects.equals(packInfo, this.packInfo[i])) {
                return i;
            }
        }
        return -1;
    ...
    public int findIndexWithPackInfo(String iface, String packInfo, int uid, int set, int tag, int metered, int roaming) {
        int i = 0;
        while (i < this.size) {
            if (uid == this.uid[i] && set == this.set[i] && tag == this.tag[i] && metered == this.metered[i] && roaming == this.roaming[i] && Objects.equals(iface, this.iface[i]) && Objects.equals(packInfo, this.packInfo[i])) {
                return i;
    ...
    public void setContainExtendDataFlag(boolean flag) {
        this.containExtendData = flag;
    }
