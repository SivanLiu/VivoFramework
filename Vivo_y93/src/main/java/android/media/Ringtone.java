...
public class Ringtone {
    ...
    public static String getTitle(Context context, Uri uri, boolean followSettingsUri, boolean allowRemote) {
        ...
        if (uri != null) {
            ...
            if (!"settings".equals(authority)) {
                ...
                title = context.getString(17041212, getTitle(context, actualUri, false, allowRemote));
            }
        } else {
            title = context.getString(17041216);
        }
        if (title == null) {
            title = context.getString(17041217);
            if (title == null) {
                title = "";
            }
        }
    ...
    public void play() {
        if (this.mLocalPlayer != null) {
            if (this.mAudioManager.getStreamVolume(AudioAttributes.toLegacyStreamType(this.mAudioAttributes)) != 0 || AudioAttributes.toLegacyStreamType(this.mAudioAttributes) == 5) {
                startLocalPlayer();
            }
        } else if (this.mAllowRemote && this.mRemotePlayer != null) {
            boolean looping;
