...
public abstract class Animation implements Cloneable {
    ...
    private boolean mInitedPreDraw = false;
    ...
    public boolean getTransformation(long currentTime, Transformation outTransformation) {
        ...
        if ((normalizedTime >= 0.0f || this.mFillBefore) && (normalizedTime <= 1.0f || this.mFillAfter)) {
            if (!this.mStarted) {
                if (this.mPerf != null) {
                    this.mPerfDuration = (int) this.mDuration;
                    if (this.mPerfDuration > 1000) {
                        this.mPerfDuration = 1000;
                    }
                    this.mPerf.perfHint(BoostFramework.VENDOR_HINT_ANIM_BOOST, "", this.mPerfDuration, 2);
                }
                fireAnimationStart();
                this.mStarted = true;
    ...
    public void setInitedPreDraw() {
        this.mInitedPreDraw = true;
        this.mInitialized = false;
    }

    public boolean isInitedPreDraw() {
        return this.mInitedPreDraw;
    }

    public void getTransformationSpecify(Transformation outTransformation, boolean isLastFrame) {
        if (isLastFrame) {
            applyTransformation(1.0f, outTransformation);
        } else {
            applyTransformation(0.0f, outTransformation);
        }
    }

    public void getTransformationNodraw(long currentTime, Transformation outTransformation) {
        boolean tmpOneMoreTime = this.mOneMoreTime;
        getTransformation(currentTime, outTransformation);
        this.mOneMoreTime = tmpOneMoreTime;
    }
}
