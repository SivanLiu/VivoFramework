...
import vivo.contentcatcher.IApplicationWatcher;
...
public class View implements Callback, KeyEvent.Callback, AccessibilityEventSource {
    ...
    public static final String DEBUG_TOUCH_AREA_PROPERTY = "debug.touch.area";
    ...
    public static final int SYSTEM_UI_FLAG_BLACK_BACKGROUND = 32;
    ...
    int mPrivateFlags2;
    int mPrivateFlags3;
    private Integer mRecordedColor;
    ...
    private boolean mSkipDrawChild;
    ...
    static final class AttachInfo {
        ...
        boolean mDebugTouchArea = SystemProperties.getBoolean(View.DEBUG_TOUCH_AREA_PROPERTY, false);
    ...
    public View(Context context) {
        ...
        this.mKeepTop = false;
        this.mKeepTopValue = 0;
        this.mKeepVisible = true;
        ...
        this.mRecordedColor = null;
        this.mSkipDrawChild = false;
    ...
    public String toString() {
        ...
        if ((this.mPrivateFlags & 33554432) != 0) {
            out.append(PhoneNumberUtils.BBK_PAUSE);
        } else {
    ...
    public WindowInsets dispatchApplyWindowInsets(WindowInsets insets) {
        try {
            ...
            if (this.mDelegate != null) {
                this.mDelegate.onApplyWindowInsets(insets);
            }
    ...
    public void setVisibility(int visibility) {
        if (this.mKeepVisible || visibility != 0) {
    ...
    /* renamed from: lambda$-android_view_View_420958 */
    static /* synthetic */ boolean m2lambda$-android_view_View_420958(int id, View t) {
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (li == null || li.mOnKeyListener == null || (this.mViewFlags & 32) != 0 || !li.mOnKeyListener.onKey(this, event.getKeyCode(), event)) {
            ...
            if (event.dispatch(this, dispatcherState, this)) {
                debugVivoInputEvent("handle Key event by onKeyXXX", event);
                return true;
            }
            debugVivoInputEvent("Do not handle key event", event);
            if (this.mInputEventConsistencyVerifier != null) {
                this.mInputEventConsistencyVerifier.onUnhandledEvent(event, 0);
            }
            return false;
        }
        debugVivoInputEvent("handle Key event by listerner, listener = " + li, event);
        return true;
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        if (InputEventReceiver.DEBUG_VIVO_INPUT) {
            debugVivoInputEvent("dispatchTouchEvent", event);
        }
        ...
        if (onFilterTouchEventForSecurity(event)) {
            ...
            if (li != null && li.mOnTouchListener != null && (this.mViewFlags & 32) == 0 && li.mOnTouchListener.onTouch(this, event)) {
                debugVivoInputEvent("handle Touch event by listerner, listener = " + li, event);
                result = true;
            }
            if (!result && onTouchEvent(event)) {
                debugVivoInputEvent("handle Touch event by onTouchEvent", event);
                result = true;
            }
        }
        if (!result) {
            debugVivoInputEvent("Do not handle Touch event", event);
        }
    ...
    private final void dispatchTouchEventToContentCatcher(MotionEvent paramMotionEvent) {
        try {
            IApplicationWatcher watcher = ContentCatcherManagerFactory.getApplicationWatcher();
            if (watcher != null) {
                watcher.onTouchEvent(this, paramMotionEvent);
            }
        } catch (Exception e) {
            Log.w("ContentCatcher", "dispatchTouchEventToContentCatcher get Exception" + e.toString());
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        int viewFlags = this.mViewFlags;
        if (InputEventReceiver.DEBUG_VIVO_INPUT) {
            debugVivoInputEvent("onTouchEvent mTouchDelegate = " + this.mTouchDelegate + ",enable = " + isEnabled() + ",clickable = " + isClickable() + ",isLongClickable = " + isLongClickable(), event);
        }
        int action = event.getAction();
        ...
        if ((this.mPrivateFlags & 32768) != 0 && (renderNode.isValid() ^ 1) == 0 && !this.mRecreateDisplayList) {
            ...
            if (layerType == 1) {
                ...
                if ((this.mPrivateFlags & 128) == 128) {
                    dispatchDraw(canvas);
                    if (!(this.mDelegate == null || (this.mSkipDrawChild ^ 1) == 0)) {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (background != null) {
            setBackgroundBounds();
            if (!(this.mSkipDrawChild || !canvas.isHardwareAccelerated() || this.mAttachInfo == null || this.mAttachInfo.mThreadedRenderer == null)) {
            ...
            int scrollX = this.mScrollX;
            int scrollY = this.mScrollY;
            if (this.mSkipDrawChild || (scrollX | scrollY) == 0) {
    ...
    /* renamed from: -android_view_View-mthref-0 */
    /* synthetic */ void m3-android_view_View-mthref-0() {
        showHoverTooltip();
    }
    ...
    private void debugVivoInputEvent(String tag, InputEvent event) {
        if (InputEventReceiver.DEBUG_VIVO_INPUT) {
            if (event == null) {
                Log.d(VIEW_LOG_TAG, tag);
            } else if ((event instanceof MotionEvent) && (event.getSource() & 2) != 0) {
                MotionEvent motionEvent = (MotionEvent) event;
                if (InputEventReceiver.DEBUG_VIVO_MOTION) {
                    Log.d(VIEW_LOG_TAG, tag + " ,vLog[" + this + "] ,motion action=" + motionEvent.getAction() + " ,count=" + motionEvent.getPointerCount());
                } else if (!(motionEvent.getAction() == 2 || motionEvent.getAction() == 7)) {
                    Log.d(VIEW_LOG_TAG, tag + " ,vLog[" + this + "] ,motion action=" + motionEvent.getAction() + " ,count=" + motionEvent.getPointerCount());
                }
            } else if (event instanceof KeyEvent) {
                KeyEvent keyEvent = (KeyEvent) event;
                Log.d(VIEW_LOG_TAG, tag + " ,vLog[" + this + "] ,keyCode=" + keyEvent.getKeyCode() + " ,action=" + keyEvent.getAction());
            }
        }
    }

    public void vivosetPadding(int left, int top, int right, int bottom) {
        this.mKeepTop = true;
        this.mKeepTopValue = top;
        setPadding(left, top, right, bottom);
        this.mKeepTop = false;
    }

    public void vivosetVisible(boolean visible) {
        this.mKeepVisible = visible;
    }

    public void setRecordedColor(Integer color) {
        this.mRecordedColor = color;
    }

    public Integer getRecordedColor() {
        return this.mRecordedColor;
    }

    protected boolean skipDrawChild() {
        return this.mSkipDrawChild;
    }

    public void drawNoChildren(Canvas canvas, boolean background) {
        if (!this.mSkipDrawChild) {
            this.mSkipDrawChild = true;
            if (background) {
                drawBackground(canvas);
            } else {
                draw(canvas);
    ...
    public boolean getGlobalVisibleRect(Rect r, Point globalOffset, boolean forceCheckParent) {
        int width = this.mRight - this.mLeft;
        int height = this.mBottom - this.mTop;
        if (width <= 0 || height <= 0) {
            return false;
        ...
        if (forceCheckParent && this.mParent != null && (this.mParent instanceof ViewGroup)) {
            return ((ViewGroup) this.mParent).getChildVisibleRect(this, r, globalOffset, true);
        }
        return this.mParent != null ? this.mParent.getChildVisibleRect(this, r, globalOffset) : true;
    }

    private void updateSystemUiVisibility() {
        if ((this instanceof TextureView) || (this instanceof VideoView)) {
            boolean blackBackground = false;
            boolean visible = getVisibility() != 8;
            Rect viewBounds = new Rect();
            View rootView = getRootView();
            Rect windowBounds = new Rect();
            if (visible && rootView != null) {
                windowBounds.set(0, 0, rootView.getWidth(), rootView.getHeight());
                if (this.mAttachInfo != null) {
                    windowBounds.top += getRootWindowInsets().getSystemWindowInsetTop();
                }
                int[] location = new int[2];
                getLocationInWindow(location);
                viewBounds.set(location[0], location[1], location[0] + getWidth(), location[1] + getHeight());
                int topInset = viewBounds.top - windowBounds.top;
                int bottomInset = windowBounds.bottom - viewBounds.bottom;
                if (Math.abs((viewBounds.left - windowBounds.left) - (windowBounds.right - viewBounds.right)) <= 2 && Math.abs(topInset - bottomInset) <= 2) {
                    blackBackground = true;
                }
            }
            Slog.i(VIEW_LOG_TAG, "updateSystemUiVisibility, blackBackground = " + blackBackground + ", getVisibility() = " + getVisibility() + ", getWindowVisibility() = " + getWindowVisibility() + ", viewBounds = " + viewBounds + ", windowBounds = " + windowBounds + ", view = " + (this instanceof TextureView ? "TextureView:" : "VideoView:") + this);
            final boolean black = blackBackground;
            post(new Runnable() {
                public void run() {
                    if (black) {
                        View.this.setSystemUiVisibility(View.this.getSystemUiVisibility() | 32);
                    } else {
                        View.this.setSystemUiVisibility(View.this.getSystemUiVisibility() & -33);
                    }
                }
            });
        }
