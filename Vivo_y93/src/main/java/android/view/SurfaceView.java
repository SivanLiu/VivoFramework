...
public class SurfaceView extends View implements WindowStoppedCallback {
    ...
    public SurfaceView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        this.mSurfaceHolder = new SurfaceHolder() {
            ...
            /* renamed from: lambda$-android_view_SurfaceView$3_43709 */
            /* synthetic */ void m17lambda$-android_view_SurfaceView$3_43709(boolean screenOn) {
    ...
    private void init() {
        this.mSurfaceHolder.addCallback(new Callback2() {
            ...
            public void surfaceRedrawNeededAsync(SurfaceHolder holder, Runnable drawingFinished) {
                Handler handler = SurfaceView.this.getHandler();
                if (handler != null) {
                    handler.post(drawingFinished);
                } else {
                    drawingFinished.run();
                }
            }
    ...
    private void updateRequestedVisibility() {
        this.mRequestedVisible = this.mViewVisibility ? this.mWindowVisibility : false;
    }
    ...
    /* renamed from: -android_view_SurfaceView-mthref-0 */
    /* synthetic */ void m15-android_view_SurfaceView-mthref-0() {
        onDrawFinished();
    }
    ...
    private void updateSystemUiVisibility() {
        ...
        int viewVisibility = getVisibility();
        if (rootView != null) {
            appBounds.set(0, 0, rootView.getWidth(), rootView.getHeight());
            if (!(appBounds == null || viewVisibility == 8)) {
                int topInset = this.mScreenRect.top - appBounds.top;
                int bottomInset = appBounds.bottom - this.mScreenRect.bottom;
                if (Math.abs((this.mScreenRect.left - appBounds.left) - (appBounds.right - this.mScreenRect.right)) <= 2 && Math.abs(topInset - bottomInset) <= 2) {
                    blackBackground = true;
                }
            }
        }
        Slog.i(TAG, "updateSystemUiVisibility, blackBackground = " + blackBackground + ", viewVisibility = " + viewVisibility + ", appBounds = " + appBounds + ", mScreenRect = " + this.mScreenRect + ", surface = " + this);
        final boolean black = blackBackground;
        post(new Runnable() {
            public void run() {
                if (black) {
                    SurfaceView.this.setSystemUiVisibility(SurfaceView.this.getSystemUiVisibility() | 32);
                } else {
                    SurfaceView.this.setSystemUiVisibility(SurfaceView.this.getSystemUiVisibility() & -33);
                }
            }
        });
    }
    ...
    /* renamed from: lambda$-android_view_SurfaceView_35442 */
    /* synthetic */ void m16lambda$-android_view_SurfaceView_35442() {
        performDrawFinished();
    }
