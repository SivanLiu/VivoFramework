...
import com.android.internal.policy.NavigationBarWindow;
...
public final class ViewRootImpl implements ViewParent, Callbacks, DrawCallbacks {
    ...
    private static boolean DEBUG_CONFIGURATION = LOCAL_LOGV;
    private static boolean DEBUG_DIALOG = LOCAL_LOGV;
    private static boolean DEBUG_DISABLEHW = SystemProperties.getBoolean("debug.viewroot.disableHW", false);
    private static boolean DEBUG_DRAW = LOCAL_LOGV;
    private static boolean DEBUG_FPS = false;
    private static boolean DEBUG_IMF = LOCAL_LOGV;
    private static boolean DEBUG_INPUT_RESIZE = LOCAL_LOGV;
    private static final boolean DEBUG_INPUT_STAGES = LOCAL_LOGV;
    private static final boolean DEBUG_KEEP_SCREEN_ON = LOCAL_LOGV;
    private static boolean DEBUG_LAYOUT = LOCAL_LOGV;
    private static boolean DEBUG_ORIENTATION = LOCAL_LOGV;
    private static boolean DEBUG_TRACKBALL = LOCAL_LOGV;
    ...
    private NavigationBarWindow mNavigationBarWindow;
    ...
    abstract class InputStage {
        ...
        protected void finish(QueuedInputEvent q, boolean handled) {
            ...
            if (InputEventReceiver.DEBUG_VIVO_INPUT || ViewRootImpl.this.isKeyEent(q)) {
                ViewRootImpl.this.debugVivoInputEvent("finish evFlags = " + q.mFlags + " ,handled = " + handled + " ,with " + getClass().getSimpleName(), q);
            }
            forward(q);
        }
        ...
        protected void onDeliverToNext(QueuedInputEvent q) {
            ...
            if (InputEventReceiver.DEBUG_VIVO_INPUT || ViewRootImpl.this.isKeyEent(q)) {
                ViewRootImpl.this.debugVivoInputEvent("onDeliverToNext with " + getClass().getSimpleName(), q);
            }
            if (this.mNext != null) {
        ...
        protected boolean shouldDropInputEvent(QueuedInputEvent q) {
            if (ViewRootImpl.this.mView == null || (ViewRootImpl.this.mAdded ^ 1) != 0) {
                ...
                ViewRootImpl.this.debugVivoInputEvent("Dropping event focus: " + ViewRootImpl.this.mAttachInfo.mHasWindowFocus + " ,stop: " + ViewRootImpl.this.mStopped + " ,pausedForTransition=" + ViewRootImpl.this.mPausedForTransition, q);
        ...
        protected int onProcess(QueuedInputEvent q) {
            if (ViewRootImpl.this.mLastWasImTarget && (ViewRootImpl.this.isInLocalFocusMode() ^ 1) != 0) {
                ...
                if (imm != null) {
                    ...
                    } else if (InputEventReceiver.DEBUG_VIVO_INPUT || ViewRootImpl.this.isKeyEent(q)) {
                        ViewRootImpl.this.debugVivoInputEvent("Sending input event to IME", q);
                    }
    ...
    final class ViewPostImeInputStage extends InputStage {
        ...
        protected int onProcess(QueuedInputEvent q) {
            ...
            if (monitor != null) {
                try {
                    monitor.beginInputEvent();
                } catch (Throwable th) {
                    if (monitor != null) {
                        monitor.postInputEvent();
                    }
                }
            }
            int processKeyEvent;
            if (q.mEvent instanceof KeyEvent) {
                processKeyEvent = processKeyEvent(q);
                if (monitor != null) {
                    monitor.postInputEvent();
                }
            ...
            if ((source & 2) != 0) {
                processKeyEvent = processPointerEvent(q);
                if (monitor != null) {
                    monitor.postInputEvent();
                }
                return processKeyEvent;
            } else if ((source & 4) != 0) {
                processKeyEvent = processTrackballEvent(q);
                if (monitor != null) {
                    monitor.postInputEvent();
                }
                return processKeyEvent;
            } else {
                processKeyEvent = processGenericMotionEvent(q);
                if (monitor != null) {
                    monitor.postInputEvent();
                }
                return processKeyEvent;
        ...
        private int processKeyEvent(QueuedInputEvent q) {
            KeyEvent event = q.mEvent;
            ViewRootImpl.this.debugVivoInputEvent("processKeyEvent mView = " + ViewRootImpl.this.mView + ", mAdded = " + ViewRootImpl.this.mAdded, q);
            if (ViewRootImpl.this.mView.dispatchKeyEvent(event)) {
                if (InputEventReceiver.DEBUG_VIVO_CTRL) {
                    Log.d(ViewRootImpl.TAG, "App handle keyevent.");
                }
                return 1;
            }
            if (InputEventReceiver.DEBUG_VIVO_CTRL) {
                Log.d(ViewRootImpl.TAG, "No app handle.");
            }
    ...
    final class ViewRootHandler extends Handler {
        ...
        /* JADX WARNING: Missing block: B:31:0x00d9, code:
            if (r39.this$0.mDisplay.getDisplayId() == r14.argi3) goto L_0x0007;
     */
        /* Code decompiled incorrectly, please refer to instructions dump. */
        public void handleMessage(Message msg) {
            SomeArgs args;
            ...
            switch (msg.what) {
                ...
                case 11:
                    ...
                    } else if (InputEventReceiver.DEBUG_VIVO_INPUT) {
                        ViewRootImpl.this.debugVivoInputEvent("Dispatching key " + msg.obj + " from IME to " + ViewRootImpl.this.mView, null);
                    }
    ...
    public ViewRootImpl(Context context, Display display) {
        ...
        this.mPerf = null;
        this.mProfile = false;
        this.mNavigationBarWindow = null;
        ...
        this.mSurface.setHasSubSurface(true);
    ...
    public NavigationBarWindow getNavigationBarWindow() {
        return this.mNavigationBarWindow;
    }

    /* JADX WARNING: Missing block: B:131:0x05b3, code:
            return;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public void setView(View view, LayoutParams attrs, View panelParentView) {
        synchronized (this) {
            if (this.mView == null) {
                ...
                if (DEBUG_LAYOUT) {
                    Log.d(this.mTag, "WindowLayout in setView:" + attrs);
                }
                ...
                if (res < 0) {
                    ...
                    if (NavigationBarPolicy.hasNavigationBarWindow(null, attrs)) {
                        this.mNavigationBarWindow = new NavigationBarWindow(this.mView, this.mWindow, this.mWindowAttributes);
                    }
                }
            }
        }
    ...
    /* JADX WARNING: Missing block: B:45:0x00eb, code:
    ...
    void setLayoutParams(LayoutParams attrs, boolean newView) {
        synchronized (this) {
            ...
            if (this.mNavigationBarWindow != null) {
                this.mNavigationBarWindow.updateAttributes(this.mWindowAttributes);
            }
        }
    }
    ...
    void setDisableWindowStop(boolean disable) {
        this.mDisableStop = disable;
        if (this.mDisableStop && this.mStopped) {
            setWindowStopped(false);
        }
    }

    void setWindowStopped(boolean stopped) {
        if (!((this.mDisableStop && stopped) || this.mStopped == stopped)) {
    ...
    private boolean collectViewAttributes() {
        if (this.mAttachInfo.mRecomputeGlobalAttributes) {
            ...
            if (!(this.mAttachInfo.mKeepScreenOn == oldScreenOn && this.mAttachInfo.mSystemUiVisibility == params.subtreeSystemUiVisibility && this.mAttachInfo.mHasSystemUiListeners == params.hasSystemUiListeners)) {
                ...
                if (this.mNavigationBarWindow != null) {
                    this.mNavigationBarWindow.onSystemUIVisibilityChanged(this.mAttachInfo.mSystemUiVisibility);
                }
                return true;
            }
        }
    ...
    private boolean measureHierarchy(View host, LayoutParams lp, Resources res, int desiredWindowWidth, int desiredWindowHeight) {
        ...
        if (lp.width == -2) {
            ...
            if (baseSize != 0 && desiredWindowWidth > baseSize) {
                ...
                if (DEBUG_DIALOG) {
                    Log.v(this.mTag, "Window " + this.mView + ": measured (" + host.getMeasuredWidth() + "," + host.getMeasuredHeight() + ") from width spec: " + MeasureSpec.toString(childWidthMeasureSpec) + " and height spec: " + MeasureSpec.toString(childHeightMeasureSpec));
                }
                if ((host.getMeasuredWidthAndState() & 16777216) == 0) {
                    ...
                    if (DEBUG_DIALOG) {
                        Log.v(this.mTag, "Window " + this.mView + ": next baseSize=" + baseSize);
                    }
    ...
    /* JADX WARNING: Missing block: B:415:0x0c2e, code:
            if (r56 == false) goto L_0x077e;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    private void performTraversals() {
        ...
        if (host != null && (this.mAdded ^ 1) == 0) {
            ...
            if (this.mFirst) {
                ...
                if (!(desiredWindowWidth == this.mWidth && desiredWindowHeight == this.mHeight)) {
                    if (DEBUG_ORIENTATION) {
                        Log.v(this.mTag, "View " + host + " resized to: " + frame);
                    }
                    this.mFullRedrawNeeded = true;
                    this.mLayoutRequested = true;
            ...
            if (viewVisibilityChanged) {
                ...
                if (this.mNavigationBarWindow != null) {
                    this.mNavigationBarWindow.onWindowVisibilityChanged(viewVisibility == 0);
                }
            ...
            if (this.mFirst || windowShouldResize || insetsChanged || viewVisibilityChanged || params != null || this.mForceNextWindowRelayout) {
                ...
                try {
                    if (DEBUG_LAYOUT) {
                        Log.i(this.mTag, "host=w:" + host.getMeasuredWidth() + ", h:" + host.getMeasuredHeight() + ", params=" + params);
                    }
                    if (this.mAttachInfo.mThreadedRenderer != null) {
                    ...
                    if (DEBUG_LAYOUT) {
                        Log.v(this.mTag, "relayout: frame=" + frame.toShortString() + " overscan=" + this.mPendingOverscanInsets.toShortString() + " content=" + this.mPendingContentInsets.toShortString() + " visible=" + this.mPendingVisibleInsets.toShortString() + " visible=" + this.mPendingStableInsets.toShortString() + " outsets=" + this.mPendingOutsets.toShortString() + " surface=" + this.mSurface);
                    }
                    ...
                    if (contentInsetsChanged) {
                        ...
                        if (DEBUG_LAYOUT) {
                            Log.v(this.mTag, "Content insets changing to: " + this.mAttachInfo.mContentInsets);
                        }
                    }
                    ...
                    if ((this.mLastSystemUiVisibility & 32) != 0 && (this.mAttachInfo.mSystemUiVisibility & 32) == 0) {
                        this.mFullRedrawNeeded = true;
                        invalidate();
                    }
                    this.mLastSystemUiVisibility = this.mAttachInfo.mSystemUiVisibility;
                ...
                if (DEBUG_ORIENTATION) {
                    Log.v(TAG, "Relayout returned: frame=" + frame + ", surface=" + this.mSurface);
                }
                ...
                if (!this.mStopped || this.mReportNextDraw) {
                    if (ensureTouchModeLocally((relayoutResult & 1) != 0) || this.mWidth != host.getMeasuredWidth() || this.mHeight != host.getMeasuredHeight() || contentInsetsChanged || updatedConfiguration) {
                        ...
                        if (DEBUG_LAYOUT) {
                            Log.v(this.mTag, "Ooops, something changed!  mWidth=" + this.mWidth + " measuredWidth=" + host.getMeasuredWidth() + " mHeight=" + this.mHeight + " measuredHeight=" + host.getMeasuredHeight() + " coveredInsetsChanged=" + contentInsetsChanged);
                        }
                        performMeasure(childWidthMeasureSpec, childHeightMeasureSpec);
                        ...
                        if (measureAgain) {
                            if (DEBUG_LAYOUT) {
                                Log.v(this.mTag, "And hey let's measure once more: width=" + width + " height=" + height);
                            }
                            performMeasure(childWidthMeasureSpec, childHeightMeasureSpec);
                        }
                        layoutRequested = true;
    ...
    private void performDraw() {
        if ((this.mAttachInfo.mDisplayState != 1 || (this.mReportNextDraw ^ 1) == 0) && this.mView != null) {
            ...
            try {
                ...
                if (this.mReportNextDraw) {
                    ...
                    if (LOCAL_LOGV) {
                        Log.v(this.mTag, "FINISHED DRAWING: " + this.mWindowAttributes.getTitle());
                    }
    ...
    /* renamed from: -android_view_ViewRootImpl-mthref-0 */
    /* synthetic */ void m23-android_view_ViewRootImpl-mthref-0() {
        postDrawFinished();
    }
    ...
    private void draw(boolean fullRedrawNeeded) {
        ...
        if (surface.isValid()) {
            ...
            if (DEBUG_ORIENTATION || DEBUG_DRAW) {
                Log.v(this.mTag, "Draw " + this.mView + "/" + this.mWindowAttributes.getTitle() + ": dirty={" + dirty.left + "," + dirty.top + "," + dirty.right + "," + dirty.bottom + "} surface=" + surface + " surface.isValid()=" + surface.isValid() + ", appScale:" + appScale + ", width=" + this.mWidth + ", height=" + this.mHeight);
            }
            ...
            this.mTempRect.set(dirty);
            this.mTempRect.offset(0, this.mWinFrame.top);
            this.mAttachInfo.mTreeObserver.dispatchOnPostDraw(this.mTempRect);
            Surface extraSurface = this.mSurface.mSubSurface;
            if (this.mNavigationBarWindow != null) {
                this.mNavigationBarWindow.drawNavigationBar(extraSurface, fullRedrawNeeded);
            }
    ...
    /* JADX WARNING: Missing block: B:43:0x0134, code:
            if (r1 == r19.bottom) goto L_0x0027;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    ...
    boolean scrollToRectOrFocus(Rect rectangle, boolean immediate) {
                ...
                if (focus.getGlobalVisibleRect(this.mVisRect, null)) {
                    ...
                    if (this.mTempRect.intersect(this.mVisRect)) {
                        ...
                        if (this.mTempRect.height() > (this.mView.getHeight() - vi.top) - vi.bottom) {
                            if (DEBUG_INPUT_RESIZE) {
                                Log.v(this.mTag, "Too tall; leaving scrollY=" + scrollY);
                            }
        ...
        if (scrollY != this.mScrollY) {
            if (DEBUG_INPUT_RESIZE) {
                Log.v(this.mTag, "Pan scroll changed: old=" + this.mScrollY + " , new=" + scrollY);
            }
            if (!immediate) {
                if (this.mScroller == null) {
    ...
    private void performConfigurationChange(MergedConfiguration mergedConfiguration, boolean force, int newDisplayId) {
        ...
        if (DEBUG_CONFIGURATION) {
            Log.v(this.mTag, "Applying new config to window " + this.mWindowAttributes.getTitle() + ", globalConfig: " + globalConfig + ", overrideConfig: " + overrideConfig);
        }
    ...
    public void updateConfiguration(int newDisplayId) {
        if (this.mView != null) {
            ...
            if (this.mNavigationBarWindow != null) {
                this.mNavigationBarWindow.onConfigurationChanged();
            }
        }
    }
    ...
    public void loadSystemProperties() {
        this.mHandler.post(new Runnable() {
            public void run() {
                ...
                boolean touchTst = SystemProperties.getBoolean(View.DEBUG_TOUCH_AREA_PROPERTY, false);
                if (touchTst != ViewRootImpl.this.mAttachInfo.mDebugTouchArea) {
                    ViewRootImpl.this.mAttachInfo.mDebugTouchArea = touchTst;
                    if (!ViewRootImpl.this.mHandler.hasMessages(22)) {
                        ViewRootImpl.this.mHandler.sendEmptyMessageDelayed(22, 200);
                    }
                }
    ...
    void doProcessInputEvents() {
        while (this.mPendingInputEventHead != null) {
            ...
            this.mPendingInputEventCount--;
            Trace.traceCounter(4, this.mPendingInputEventQueueLengthCounterName, this.mPendingInputEventCount);
            if (InputEventReceiver.DEBUG_VIVO_INPUT || isKeyEent(q)) {
                debugVivoInputEvent("doProcessInputEvents", q);
            }
    ...
    private void finishInputEvent(QueuedInputEvent q) {
        ...
        debugVivoInputEvent("finishInputEvent: handled = " + handled, q);
        if (q.mReceiver != null) {
    ...
    private boolean isKeyEent(QueuedInputEvent q) {
        if (q == null || q.mEvent == null || !(q.mEvent instanceof KeyEvent)) {
            return false;
        }
        return true;
    }

    private void debugVivoInputEvent(QueuedInputEvent q) {
        debugVivoInputEvent("debugVivoInputEvent", q);
    }

    private void debugVivoInputEvent(String tag, QueuedInputEvent q) {
