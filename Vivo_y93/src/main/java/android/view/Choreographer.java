...
public final class Choreographer {
    ...
    void doFrame(long frameTimeNanos, int frame) {
        synchronized (this.mLock) {
            if (DEBUG) {
                Log.d(TAG, "doFrame: frameTimeNanos = " + frameTimeNanos + ",frame = " + frame + ",mFrameSheduled = " + this.mFrameScheduled + ",this = " + this);
            }
            if (this.mFrameScheduled) {
                ...
                try {
                    ...
                    doCallbacks(3, frameTimeNanos);
                    AnimationUtils.unlockAnimationClock();
                    Jank.getInstance().dropFrames((int) ((System.nanoTime() - startNanos) / this.mFrameIntervalNanos));
                    Trace.traceEnd(8);
                } catch (Throwable th) {
                    AnimationUtils.unlockAnimationClock();
                    Jank.getInstance().dropFrames((int) ((System.nanoTime() - startNanos) / this.mFrameIntervalNanos));
