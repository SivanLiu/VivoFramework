...
public class BaseInputConnection implements InputConnection {
    ...
    BaseInputConnection(InputMethodManager mgr, boolean fullEditor) {
        ...
        this.isExport = "yes".equals(SystemProperties.get("ro.vivo.product.overseas", "no"));
    }
    ...
    private void replaceText(CharSequence text, int newCursorPosition, boolean composing) {
        ...
        if (content != null) {
            ...
            LogPrinter lp;
            if (this.isExport) {
                text = rebuiltThaiInput(content, text);
            }
            beginBatchEdit();
    ...
    private CharSequence rebuiltThaiInput(Editable content, CharSequence text) {
        String temp = "";
        int start = getComposingSpanStart(content);
        int startSelect = Selection.getSelectionStart(content);
        ...
        String leftString = "";
        if (start == -1) {
            if (startSelect > conString.length()) {
                startSelect = conString.length();
            }
            if (startSelect < 0) {
                startSelect = 0;
            }
            leftString = conString.substring(0, startSelect);
        } else {
            if (start > conString.length()) {
                start = conString.length();
            }
            if (start < 0) {
                start = 0;
            }
            leftString = conString.substring(0, start);
        }
        boolean noFirstLetter = true;
        int numUpLetter = 0;
        int numDownLetter = 0;
        for (int i = 0; i < text.length(); i++) {
            char c = text.charAt(i);
            if (!noFirstLetter || (leftString.length() != 0 && leftString.charAt(leftString.length() - 1) != ' ')) {
                if (noFirstLetter) {
                    if (leftString.length() == 1) {
                        char ch = leftString.charAt(leftString.length() - 1);
                        if (isInUpLetter(ch)) {
                            numUpLetter++;
                        } else if (isInDownLetter(ch)) {
                            numDownLetter++;
                        }
                    } else if (leftString.length() >= 2) {
                        char ch1 = leftString.charAt(leftString.length() - 1);
                        char ch2 = leftString.charAt(leftString.length() - 2);
                        if (isInUpLetter(ch2)) {
                            numUpLetter++;
                        } else if (isInDownLetter(ch2)) {
                            numDownLetter++;
                        } else {
                            numUpLetter = 0;
                            numDownLetter = 0;
        ...
        return temp.subSequence(0, temp.length());
    }

    private boolean isInUpLetter(char c) {
        if (c == 3633 || ((c >= 3635 && c <= 3642) || c == 3655 || (c >= 3661 && c <= 3662))) {
            return true;
        }
        return false;
    }

    private boolean isInDownLetter(char c) {
        if (c < 3656 || c > 3660) {
            return false;
        }
        return true;
