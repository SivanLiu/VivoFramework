...
import android.appwidget.FtAppWidgetContext;
...
public abstract class LayoutInflater {
    ...
    private static final HashMap<String, ArrayList<String>> sUpgreadeConstructorMap = new HashMap();
    ...
    private void putUpgreadeConstructor(String name) {
        if (name != null && -1 != name.indexOf(46) && !name.isEmpty()) {
            String packageName = this.mContext.getPackageName();
            if (FtAppWidgetContext.isUpgradeApp(packageName)) {
                ArrayList<String> classes = (ArrayList) sUpgreadeConstructorMap.get(packageName);
                if (classes == null) {
                    classes = new ArrayList();
                    classes.add(name);
                    sUpgreadeConstructorMap.put(packageName, classes);
    ...
    public static final void updateUpgreadeConstructor(String packageName) {
        if (packageName != null) {
            ArrayList<String> classes = (ArrayList) sUpgreadeConstructorMap.get(packageName);
            if (classes != null) {
                for (String classTemp : classes) {
                    sConstructorMap.remove(classTemp);
                }
            }
            sUpgreadeConstructorMap.remove(packageName);
        }
    }
