...
public final class ViewTreeObserver {
    ...
    private ArrayList<OnPostDrawListener> mOnPostDrawListeners;
    ...
    public void addOnPostDrawListener(OnPostDrawListener listener) {
        checkIsAlive();
        if (this.mOnPostDrawListeners == null) {
            this.mOnPostDrawListeners = new ArrayList();
        }
        this.mOnPostDrawListeners.add(listener);
    }

    public void removeOnPostDrawListener(OnPostDrawListener victim) {
        checkIsAlive();
        if (this.mOnPostDrawListeners != null) {
            this.mOnPostDrawListeners.remove(victim);
        }
    }

    public final void dispatchOnPostDraw(Rect dirty) {
        if (this.mOnPostDrawListeners != null) {
            ArrayList<OnPostDrawListener> listeners = this.mOnPostDrawListeners;
            int numListeners = listeners.size();
            for (int i = 0; i < numListeners; i++) {
                ((OnPostDrawListener) listeners.get(i)).onPostDraw(dirty);
            }
        }
    }
}
