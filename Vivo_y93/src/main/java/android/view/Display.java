...
public final class Display {
    ...
    public void getRealSize(Point outSize) {
        synchronized (this) {
            updateDisplayInfoLocked();
            String currentStackTrace = getStringStackTrace();
            if ((((float) this.mDisplayInfo.logicalHeight) * 1.0f) / ((float) this.mDisplayInfo.logicalWidth) <= 2.0f || currentStackTrace == null || currentStackTrace.indexOf("com.instagram.boomerang.BoomerangActivity") <= 0) {
                outSize.x = this.mDisplayInfo.logicalWidth;
                outSize.y = this.mDisplayInfo.logicalHeight;
                return;
            }
            Log.d(TAG, "getRealSize vivo !!!");
            outSize.x = this.mDisplayInfo.logicalWidth;
            outSize.y = this.mDisplayInfo.logicalWidth * 2;
        }
    }
