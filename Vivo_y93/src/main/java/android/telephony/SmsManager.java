...
public final class SmsManager {
    ...
    public static final int RESULT_ERRORCODE_EMERGENCEY = 1001;
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    public static final int RESULT_ERRORCODE_MISSING_FEILD = 1000;
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    public static final int RESULT_ERRORCODE_NOT_SUPPORT = 1003;
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    public static final int RESULT_ERRORCODE_NULL_PACKAGE_NAME = 1004;
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    public static final int RESULT_ERRORCODE_PERMISSION_DENIED = 1005;
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    public static final int RESULT_ERRORCODE_PREMIUM_PERMISSION = 1002;
    ...
    public void sendTextMessage(Context context, String destinationAddress, String scAddress, String text, PendingIntent sentIntent, PendingIntent deliveryIntent) {
        ...
        if (TextUtils.isEmpty(destinationAddress)) {
            ...
            Log.d(TAG, "local calling send sms ,the callingPackage is:" + callingPackage);
            try {
                getISmsServiceOrThrow().sendTextForSubscriber(getSubscriptionId(), callingPackage, destinationAddress, scAddress, text, sentIntent, deliveryIntent, true);
    ...
    public void sendMultipartTextMessage(Context context, String destinationAddress, String scAddress, ArrayList<String> parts, ArrayList<PendingIntent> sentIntents, ArrayList<PendingIntent> deliveryIntents) {
        if (TextUtils.isEmpty(destinationAddress)) {
            throw new IllegalArgumentException("Invalid destinationAddress");
            ...
            Log.d(TAG, "sendMultipartTextMessage callingPackage: " + callingPackage);
            if (parts.size() > 1) {
                try {
                    getISmsServiceOrThrow().sendMultipartTextForSubscriber(getSubscriptionId(), callingPackage, destinationAddress, scAddress, parts, sentIntents, deliveryIntents, true);
                    return;
                } catch (RemoteException e) {
                    return;
                }
            ...
            sendTextMessage(context, destinationAddress, scAddress, (String) parts.get(0), pendingIntent, pendingIntent2);
        }
    }
    ...
    public static SmsManager getSmsManagerForSubscriber(long subId) {
        return getSmsManagerForSubscriptionId((int) subId);
    }
}
