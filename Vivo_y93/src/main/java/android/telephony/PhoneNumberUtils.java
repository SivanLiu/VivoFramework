...
public class PhoneNumberUtils {
    ...
    public static final char BBK_PAUSE = 'p';
    @VivoHook(hookType = VivoHookType.NEW_FIELD)
    public static final char BBK_WAIT = 'w';
    ...
    public static final boolean isNonSeparator(char c) {
        if ((c >= '0' && c <= '9') || c == '*' || c == '#' || c == PLUS_SIGN_CHAR || c == WILD || c == ';' || c == ',' || c == BBK_WAIT || c == BBK_PAUSE) {
            return true;
        }
        return false;
    }
    ...
    private static boolean isPause(char c) {
        return c == BBK_PAUSE || c == 'P';
    }

    private static boolean isToneWait(char c) {
        return c == BBK_WAIT || c == 'W';
    }
    ...
    public static String normalizeNumber(String phoneNumber) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (digit != -1) {
                ...
                sb.append(c);
            } else if (c == ';' || c == ',' || c == WILD || c == BBK_WAIT || c == BBK_PAUSE) {
                sb.append(c);
    ...
    private static boolean hasSimInsert() {
    ...
    @VivoHook(hookType = VivoHookType.NEW_METHOD)
    public static boolean isAllSimLockedCheck() {
        if (!TelephonyManager.getDefault().isMultiSimEnabled()) {
            return TelephonyManager.getDefault().getSimState() == 2;
        } else {
            int simState1 = TelephonyManager.getDefault().getSimState(0);
            int simState2 = TelephonyManager.getDefault().getSimState(1);
            boolean pin1Locked = simState1 == 2;
            boolean pin2Locked = simState2 == 2;
            boolean sim1Insert = (simState1 == 1 || simState1 == 0) ? false : true;
            boolean sim2Insert = (simState2 == 1 || simState2 == 0) ? false : true;
            if ((!pin1Locked || !pin2Locked) && (!pin1Locked || (sim2Insert ^ 1) == 0)) {
                return pin2Locked ? sim1Insert ^ 1 : false;
            } else {
                return true;
            }
        }
    ...
    private static boolean isEmergencyNumberInternal(int subId, String number, String defaultCountryIso, boolean useExactMatch) {
        ...
        if (TextUtils.isEmpty(emergencyNumbers)) {
            ...
            for (String emergencyNum : (slotId < 0 ? "112,911,000,08,110,118,119,999" : "112,911").split(",")) {
                if (useExactMatch) {
        ...
        String vivoEmergencyNumbers = SystemProperties.get(slotId <= 0 ? "ril.ecc.vivo.list" : "ril.ecc.vivo.list" + slotId, "");
        if (!TextUtils.isEmpty(vivoEmergencyNumbers)) {
            emergencyNumbers = emergencyNumbers + "," + vivoEmergencyNumbers;
        }
