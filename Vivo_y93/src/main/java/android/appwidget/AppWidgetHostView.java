...
public class AppWidgetHostView extends FrameLayout {
    ...
    public static Rect getDefaultPaddingForWidget(Context context, ComponentName component, Rect padding) {
        ...
        try {
            if (packageManager.getApplicationInfo(component.getPackageName(), 0).targetSdkVersion >= 14) {
                ...
                padding.left = r.getDimensionPixelSize(17105005);
                padding.right = r.getDimensionPixelSize(17105006);
                padding.top = r.getDimensionPixelSize(17105007);
                padding.bottom = r.getDimensionPixelSize(17105004);
            }
            return padding;
    ...
    void resetAppWidget(AppWidgetProviderInfo info) {
        this.mInfo = info;
        this.mViewMode = 0;
        if (!(info == null || info.provider == null || !FtAppWidgetContext.isUpgradeApp(info.provider.getPackageName()))) {
            LayoutInflater.updateUpgreadeConstructor(info.provider.getPackageName());
        }
            ...
            if (content == null) {
                try {
                    if (this.mInfo == null) {
                        ...
                        content = remoteViews.apply(FtAppWidgetContext.newWidgetContext(this.mContext, this.mInfo.providerInfo.applicationInfo), this, this.mOnClickHandler);
                    }
                } catch (Exception e2) {
                    exception = e2;
                }
    ...
    private void updateContentDescription(AppWidgetProviderInfo info) {
        if (info != null) {
            ...
            setContentDescription(Resources.getSystem().getString(17041383, info.label));
        }
    }
    ...
    protected View getDefaultView() {
        ...
        try {
            if (this.mInfo != null) {
                ...
                LayoutInflater inflater = ((LayoutInflater) theirContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE)).cloneInContext(FtAppWidgetContext.newWidgetContext(theirContext, this.mInfo.providerInfo.applicationInfo));
