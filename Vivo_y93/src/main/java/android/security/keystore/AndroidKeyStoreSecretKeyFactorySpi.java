...
public class AndroidKeyStoreSecretKeyFactorySpi extends SecretKeyFactorySpi {
        ...
        try {
            ...
            if (keySizeUnsigned == -1) {
                ...
                if (keymasterSwEnforcedUserAuthenticators == 2 || keymasterHwEnforcedUserAuthenticators == 2) {
                    BigInteger secureUserId = BigInteger.valueOf(0);
                    if (keyCharacteristics.getBoolean(KeymasterDefs.KM_TAG_SOTER_IS_FROM_SOTER)) {
                        secureUserId = getGateKeeperSecureUserIdForSoter();
                    } else {
                        secureUserId = getGateKeeperSecureUserId();
                    }
                    if (keymasterSecureUserIds == null || (keymasterSecureUserIds.isEmpty() ^ 1) == 0) {
                        invalidatedByBiometricEnrollment = false;
                    } else {
                        invalidatedByBiometricEnrollment = keymasterSecureUserIds.contains(secureUserId) ^ 1;
                    }
                }
    ...
    private static BigInteger getGateKeeperSecureUserIdForSoter() throws ProviderException {
        try {
            return BigInteger.valueOf(GateKeeper.getSecureUserIdForSoter());
        } catch (IllegalStateException e) {
            throw new ProviderException("Failed to get GateKeeper secure user ID for soter", e);
        }
    }
