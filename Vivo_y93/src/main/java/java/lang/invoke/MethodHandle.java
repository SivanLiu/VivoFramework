...
public abstract class MethodHandle {
    ...
    static final /* synthetic */ boolean f36-assertionsDisabled = (MethodHandle.class.desiredAssertionStatus() ^ 1);
    ...
    private MethodType asSpreaderChecks(Class<?> arrayType, int arrayLength) {
        ...
        boolean match = true;
        boolean fail = f36-assertionsDisabled;
        for (int i = nargs - arrayLength; i < nargs; i++) {
            Class<?> ptype = mtype.parameterType(i);
            if (ptype != arrayElement) {
                match = f36-assertionsDisabled;
    ...
    private void spreadArrayChecks(Class<?> arrayType, int arrayLength) {
        ...
        if (arrayElement == null) {
            ...
            } else if (!f36-assertionsDisabled && arrayLength < 128) {
    ...
    public MethodHandle asFixedArity() {
        ...
        if (f36-assertionsDisabled || !mh.isVarargsCollector()) {
            return mh;
        }
        throw new AssertionError();
    }
