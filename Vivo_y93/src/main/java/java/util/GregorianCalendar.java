...
public class GregorianCalendar extends Calendar {
    ...
    static final /* synthetic */ boolean f83-assertionsDisabled = (GregorianCalendar.class.desiredAssertionStatus() ^ 1);
    ...
    public boolean isLeapYear(int year) {
        ...
        if (year > this.gregorianCutoverYear) {
            if (year % 100 == 0 && year % HttpURLConnection.HTTP_BAD_REQUEST != 0) {
                z = f83-assertionsDisabled;
            }
            return z;
        } else if (year < this.gregorianCutoverYearJulian) {
            return true;
        } else {
            boolean gregorian = this.gregorianCutoverYear == this.gregorianCutoverYearJulian ? getCalendarDate(this.gregorianCutoverDate).getMonth() < 3 ? true : f83-assertionsDisabled : year == this.gregorianCutoverYear ? true : f83-assertionsDisabled;
        ...
        if (amount != 0) {
            ...
            switch (field) {
                ...
                case 5:
                    ...
                    if (f83-assertionsDisabled || d.getMonth() - 1 == internalGet(2)) {
                        set(5, d.getDayOfMonth());
                        return;
                    }
                ...
                case 11:
                    ...
                    if (internalGet(5) != d2.getDayOfMonth()) {
                        ...
                        if (field == 10) {
                            if (f83-assertionsDisabled || internalGet(9) == 1) {
                                d2.addHours(12);
                            } else {
                                throw new AssertionError();
                            }
        ...
        switch (field) {
            case 1:
                ...
                if (gc.internalGetEra() != 1) {
                    ...
                    value = d.getYear();
                    if (value <= 0) {
                        if (f83-assertionsDisabled || mincal == gcal) {
                            value = 1 - value;
                        } else {
                            throw new AssertionError();
                        }
    ...
    protected void computeFields() {
        ...
        if (isPartiallyNormalized()) {
            ...
            if (fieldMask != 0 || this.calsys == null) {
                ...
                if (!(f83-assertionsDisabled || mask == 131071)) {
                    throw new AssertionError();
                }
            }
        }
    ...
    private int computeFields(int fieldMask, int tzMask) {
        ...
        if (fixedDate >= this.gregorianCutoverDate) {
            ...
            if (f83-assertionsDisabled || this.cachedFixedDate == Long.MIN_VALUE || gcal.getFixedDate(this.gdate.getNormalizedYear(), this.gdate.getMonth(), this.gdate.getDayOfMonth(), this.gdate) == this.cachedFixedDate) {
                ...
                if (!f83-assertionsDisabled && this.cdate.getDayOfWeek() <= 0) {
    ...
    private int getWeekNumber(long fixedDay1, long fixedDate) {
        ...
        if (f83-assertionsDisabled || ndays <= 7) {
    ...
    private boolean isCutoverYear(int normalizedYear) {
        return normalizedYear == (this.calsys == gcal ? this.gregorianCutoverYear : this.gregorianCutoverYearJulian) ? true : f83-assertionsDisabled;
    }

    private long getFixedDateJan1(Date date, long fixedDate) {
        if (!f83-assertionsDisabled && date.getNormalizedYear() != this.gregorianCutoverYear && date.getNormalizedYear() != this.gregorianCutoverYearJulian) {
            throw new AssertionError();
    ...
    private static int getRolledValue(int value, int amount, int min, int max) {
        if (f83-assertionsDisabled || (value >= min && value <= max)) {
            int range = (max - min) + 1;
            ...
            if (f83-assertionsDisabled || (n >= min && n <= max)) {
                return n;
            }
            throw new AssertionError();
        }
