...
public final class UUID implements Serializable, Comparable<UUID> {
    ...
    static final /* synthetic */ boolean f153-assertionsDisabled = (UUID.class.desiredAssertionStatus() ^ 1);
    ...
    private UUID(byte[] data) {
        long msb = 0;
        long lsb = 0;
        if (f153-assertionsDisabled || data.length == 16) {
            int i;
            for (i = 0; i < 8; i++) {
    ...
    public boolean equals(Object obj) {
        boolean z = f153-assertionsDisabled;
        if (obj == null || obj.getClass() != UUID.class) {
            return f153-assertionsDisabled;
        }
        UUID id = (UUID) obj;
