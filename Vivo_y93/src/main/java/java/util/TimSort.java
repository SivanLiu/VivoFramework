...
class TimSort<T> {
    ...
    static final /* synthetic */ boolean f144-assertionsDisabled = (TimSort.class.desiredAssertionStatus() ^ 1);
    ...
    static <T> void sort(T[] a, int lo, int hi, Comparator<? super T> c, T[] work, int workBase, int workLen) {
        if (f144-assertionsDisabled || (c != null && a != null && lo >= 0 && lo <= hi && hi <= a.length)) {
            ...
            if (nRemaining >= 2) {
                ...
                if (f144-assertionsDisabled || lo == hi) {
                    ts.mergeForceCollapse();
                    if (!f144-assertionsDisabled && ts.stackSize != 1) {
    ...
    private static <T> void binarySort(T[] a, int lo, int hi, int start, Comparator<? super T> c) {
        if (f144-assertionsDisabled || (lo <= start && start <= hi)) {
            if (start == lo) {
                start++;
            }
            while (start < hi) {
                ...
                int left = lo;
                int right = start;
                if (f144-assertionsDisabled || lo <= right) {
                    while (left < right) {
                    ...
                    if (f144-assertionsDisabled || left == right) {
                        int n = start - left;
                        switch (n) {
                            case 1:
                                break;
    ...
    private static int minRunLength(int n) {
        if (f144-assertionsDisabled || n >= 0) {
            int r = 0;
            while (n >= 32) {
                r |= n & 1;
                n >>= 1;
    ...
    private void mergeAt(int i) {
        if (!f144-assertionsDisabled && this.stackSize < 2) {
            throw new AssertionError();
        } else if (!f144-assertionsDisabled && i < 0) {
            throw new AssertionError();
        } else if (f144-assertionsDisabled || i == this.stackSize - 2 || i == this.stackSize - 3) {
            int base1 = this.runBase[i];
            int len1 = this.runLen[i];
            int base2 = this.runBase[i + 1];
            int len2 = this.runLen[i + 1];
            if (!f144-assertionsDisabled && (len1 <= 0 || len2 <= 0)) {
                throw new AssertionError();
            } else if (f144-assertionsDisabled || base1 + len1 == base2) {
    ...
    private static <T> int gallopLeft(T key, T[] a, int base, int len, int hint, Comparator<? super T> c) {
        if (f144-assertionsDisabled || (len > 0 && hint >= 0 && hint < len)) {
            int lastOfs = 0;
            int ofs = 1;
            int maxOfs;
            ...
            if (f144-assertionsDisabled || (-1 <= lastOfs && lastOfs < ofs && ofs <= len)) {
                lastOfs++;
                while (lastOfs < ofs) {
                ...
                if (f144-assertionsDisabled || lastOfs == ofs) {
                    return ofs;
                }
                throw new AssertionError();
            }
    ...
    private void mergeLo(int base1, int len1, int base2, int len2) {
        if (f144-assertionsDisabled || (len1 > 0 && len2 > 0 && base1 + len1 == base2)) {
            Object a = this.a;
            ...
            if (len2 == 0) {
                ...
                while (true) {
                    ...
                    int count2 = 0;
                    do {
                        if (f144-assertionsDisabled || (len1 > 1 && len2 > 0)) {
                ...
                } else if (!f144-assertionsDisabled && len2 != 0) {
                    throw new AssertionError();
                } else if (f144-assertionsDisabled || len1 > 1) {
                    System.arraycopy(tmp, cursor1, a, dest, len1);
                } else {
                    throw new AssertionError();
                }
        ...
        if (f144-assertionsDisabled || (len1 > 0 && len2 > 0 && base1 + len1 == base2)) {
            ...
            if (len1 == 0) {
                ...
                while (true) {
                    ...
                    int count2 = 0;
                    do {
                        if (f144-assertionsDisabled || (len1 > 0 && len2 > 1)) {
