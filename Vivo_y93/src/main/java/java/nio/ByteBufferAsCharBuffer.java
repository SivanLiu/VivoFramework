...
class ByteBufferAsCharBuffer extends CharBuffer {
    ...
    static final /* synthetic */ boolean f51-assertionsDisabled = (ByteBufferAsCharBuffer.class.desiredAssertionStatus() ^ 1);
    ...
    public CharBuffer slice() {
        int pos = position();
        int lim = limit();
        if (f51-assertionsDisabled || pos <= lim) {
            int rem = pos <= lim ? lim - pos : 0;
            int off = (pos << 1) + this.offset;
            if (f51-assertionsDisabled || off >= 0) {
