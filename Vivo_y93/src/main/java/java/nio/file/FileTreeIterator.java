...
class FileTreeIterator implements Iterator<Event>, Closeable {
    ...
    static final /* synthetic */ boolean f72-assertionsDisabled = (FileTreeIterator.class.desiredAssertionStatus() ^ 1);
    private Event next;
    ...
    FileTreeIterator(Path start, int maxDepth, FileVisitOption... options) throws IOException {
        ...
        if (f72-assertionsDisabled || this.next.type() == EventType.ENTRY || this.next.type() == EventType.START_DIRECTORY) {
